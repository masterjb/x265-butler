...
type Props = {
  rows: FileRow[];
  // 50-04: fileId → blocklist_entry.id, for the file-pinned blocklisted rows of
  // THIS page only. Built server-side; absent key = no single entry to remove
  // (pattern match, or a pattern that has since been deleted).
...
export function LibraryClient({
  ...
  return (
    <PageContainer variant="data">
      ...
      <FileDetailPanel
        file={selectedFile}
        blocklistEntryId={selectedFile ? blocklistEntryIds?.[String(selectedFile.id)] : undefined}
        open={panelOpen}
