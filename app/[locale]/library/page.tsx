...
import { blocklistRepo, fileRepo, settingRepo, shareRepo } from '@/src/lib/db';
...
export default async function LibraryPage({ searchParams }: Props) {
  ...
  // 50-04: fileId → blocklist_entry.id for this page's file-pinned blocklisted
  // rows. Built by the SAME helper the API route uses.
  let blocklistEntryIds: BlocklistEntryIdMap;
  let blocklistEntryIds_ms = 0;
  ...
  try {
    ...
    // 50-04: measured like every other repo read on this page (22-01 IMP-1) —
    // an unmeasured read is a blind spot in the slow_request breakdown. Skips
    // the query entirely when no row on the page is 'blocklisted'.
    const t_bl = performance.now();
    blocklistEntryIds = buildBlocklistEntryIdMap(rows, (ids) => blocklistRepo().findByFileIds(ids));
    blocklistEntryIds_ms = performance.now() - t_bl;
    ...
    logger.info(
      {
        ...
        breakdown: {
          ...
          blocklistEntryIds: blocklistEntryIds_ms,
  ...
  return (
    <LibraryClient
      rows={rows}
      blocklistEntryIds={blocklistEntryIds}
      pagination={{
        page: query.page,
        size: query.size,
        total,
