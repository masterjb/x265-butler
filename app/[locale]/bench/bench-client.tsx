...
// 31-03: Select the Pass-2 entry that should drive the live status bar.
// Prefers any in-flight (`running`) verify so a 2nd "Verify on full file" on a
// LOWER comboId than an earlier-completed preset still mounts Pass2ProgressCard
// and targets cancel/retry correctly (old behavior picked numeric-max comboId,
// so a running low-id combo lost to a completed high-id one until reload).
//
// SR-1 (audit): scope the running-preference to `activeRunId`. pass2Map
// accumulates across selectedRunId switches (global SSE store, no reset-on-select),
// so an UN-scoped running-preference would let a verify on a foreign run hijack
// the viewed run's status bar and mis-target cancel/retry (which POST
// (viewedRunId, foreignComboId)). When activeRunId is null/undefined (unit-level
// callers, no run context) the scope is a no-op = back-compat.
//
// SR-2 (audit): pick the MAX-comboId running entry via filter+reduce, not .find().
// pass2_busy makes >1 running globally impossible, but if that invariant is ever
// violated, max-comboId keeps selection deterministic instead of depending on
// object-key iteration order.
export function selectActivePass2(
  pass2Map: Record<number, Pass2ComboState>,
  activeRunId?: number | null,
): Pass2ComboState | null {
  const entries = Object.values(pass2Map);
  if (entries.length === 0) return null;
  const running = entries.filter(
    (e) => e.status === 'running' && (activeRunId == null || e.runId === activeRunId),
  );
  if (running.length > 0) {
    return running.reduce((acc, cur) => (cur.comboId > acc.comboId ? cur : acc));
  }
  // No-running fallback: UNCHANGED historical max-comboId across ALL entries.
  // The all-complete cross-run display stays as-is (cosmetic; reload reconciles
  // from DB) — deliberately NOT scoped, per <boundaries>.
...
function BenchClientInner({
  ...
  // Pick the Pass-2 entry that drives the Stepper: prefer the in-flight verify
  // (scoped to the active run), else fall back to latest comboId (numeric max).
  const latestPass2 = selectActivePass2(pass2Map, benchRun.runId ?? selectedRunId);
