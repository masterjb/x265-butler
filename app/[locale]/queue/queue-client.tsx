...
export function QueueClient({
  ...
  // 36-02 counts-gated source (AC-5), 48-03: the seed is pulled when the store
  // reports that something is encoding.
  const activeJobs: ActiveJob[] =
    liveActiveJobs.length > 0 ? liveActiveJobs : counts.encodingJobs > 0 ? initialActiveJobs : [];
      ...
      <div className="flex items-center justify-between gap-3">
        <span className="text-sm tabular-nums text-muted-foreground">
          {t('controls.activeCount', {
            // 48-03: encoding-only. The wire field for queued+encoding keeps its
            // name and meaning, but it is not what this header claims.
            encoding: counts.encodingJobs,
