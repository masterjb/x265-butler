...
async function readStatsAndSystem(requestId: string, bag: TimingBag): Promise<StatsBundle | null> {
  ...
  try {
    ...
    const system = {
      ...
      // ISS-002 (2026-09-06): show the EFFECTIVE path, not the raw setting.
      // The raw read is empty on every install without an override, and
      // system-info-card renders empty as an em-dash — so the card claimed "no
      // cache path" for the exact population whose jobs were staging and
      // logging happily to the auto-resolved root. That em-dash is what sends
      // an operator hunting filesystem permissions.
      cachePoolPath: readEffectiveCachePathCached().effectivePath, // audit S6
...
async function readQueueStatus(
  ...
  try {
    const t_q = performance.now();
    // 48-03: same helper as /api/queue/status and the SSE first frame, so all
    // three bootstrap sources report the identical triple for a given DB state.
    const queueCounts = queueCountsSnapshot(jobRepo());
    const result = {
      ...
      paused: false,
      activeJobs: queueCounts.activeJobs,
      encodingJobs: queueCounts.encodingJobs,
