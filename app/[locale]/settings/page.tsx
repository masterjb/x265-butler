...
export default async function SettingsPage() {
  ...
  const defaultValues: FormValues = {
    ...
    // 49-05: the four fallbacks read the ONE default source instead of
    // repeating literals that drifted apart from the migration seed.
    crf_libx265: clampCrf(settings.crf_libx265, DEFAULT_CRF_BY_ENCODER.libx265),
    crf_nvenc: clampCrf(settings.crf_nvenc, DEFAULT_CRF_BY_ENCODER.nvenc),
    crf_qsv: clampCrf(settings.crf_qsv, DEFAULT_CRF_BY_ENCODER.qsv),
    crf_vaapi: clampCrf(settings.crf_vaapi, DEFAULT_CRF_BY_ENCODER.vaapi),
    ...
    // 34-02: operator-pinned GPU render node. Empty = auto (first-enumerated
    // node = byte-identical pre-34); any non-string DB shape coerces to ''.
    gpu_device: typeof settings.gpu_device === 'string' ? settings.gpu_device : '',
    // 35-02: auto-crop toggle + crop_override geometry. Default off/empty =
    // byte-identical pre-35. auto_crop reads the 'true'/'false' bool-string;
    // crop_override is a raw string (empty = auto/none).
    auto_crop: settings.auto_crop === 'true',
    crop_override: typeof settings.crop_override === 'string' ? settings.crop_override : '',
    // 43-01: force-10bit toggle. Default false (code-fallback, no seed) =
    // byte-identical pre-43 for all four encoders. Reads the 'true'/'false' string.
    force_10bit: settings.force_10bit === 'true',
    // 43-03: color-passthrough toggle. Default false (code-fallback, no seed) =
    // byte-identical. Reads the 'true'/'false' string.
    color_passthrough: settings.color_passthrough === 'true',
  };

  // 34-02 (D1=A): server-probe the live /dev/dri/renderD* nodes for the
  // GPU-Device picker — prop-fed to the client form (zero client fetch, no
  // hydration risk). Build-guarded like the detectEncoders probe; probe never
  // throws (returns [] on no /dev/dri) but `.catch(() => [])` is belt-and-braces
  // so a Settings render can never 500 on a device-probe failure.
  let renderDevices: RenderDeviceOption[] = [];
  if (process.env.NEXT_PHASE !== 'phase-production-build') {
    const probes = await probeRenderDevices().catch(() => []);
    renderDevices = probes.map((p) => ({
      path: p.path,
  ...
  // 49-05: the boot-resolved QSV ratecontrol tier drives the QSV CRF helper.
  // It stays undefined on the catch-branch below ON PURPOSE — the fallback
  // detected=['libx265'] is NOT a detection, so no tier may be claimed from it.
