...
export function OnboardingClient({
  ...
  // 34-02: operator-picked GPU render node from the HwAccel step. '' = Auto
  // (omitted from the finish-PUT → byte-identical pre-34). NOT stashed in the
  // sessionStorage draft (mirrors `detection` — re-pick is cheap, avoids a
  // v2→v3 draft migration).
  const [selectedGpuDevice, setSelectedGpuDevice] = useState('');
  ...
  const handleFinish = useCallback(
    async (values: QualityStepValues) => {
      ...
      try {
        // 34-02: gpu_device travels with the finish-PUT ONLY when the operator
        // picked a node (Auto = '' → omitted → byte-identical pre-34, AC-5).
        ...
        if (selectedGpuDevice !== '') {
          settingsBody.gpu_device = selectedGpuDevice;
        }
    ...
    [t, router, locale, pathsValues, selectedGpuDevice],
  );
    ...
    <div className="flex flex-col gap-8">
      ...
      {hydrated && step === 2 && (
        <HwAccelStep
          ...
          selectedDevice={selectedGpuDevice}
          onDeviceSelected={setSelectedGpuDevice}
        />
      )}
