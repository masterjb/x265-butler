...
export function TrashClient({
  ...
  // 32-05: `rows` is seeded from `initialRows` via useState (mount-only). When
  // pagination nav (onPageChange/onSizeChange) router.push()es a new ?page/?size,
  // force-dynamic SSR re-fetches and re-renders TrashClient with a fresh
  // initialRows — but useState ignores prop changes → list stayed stale until a
  // full reload. Re-sync on the initialRows identity. page.tsx is a Server
  // Component → it re-renders ONLY on navigation, so this ref changes only on an
  // SSR re-fetch; in-page optimistic mutations (handleRemoveRow / handleRowsRefetch)
  // do NOT change initialRows → they are preserved (AC-2). DO NOT add
  // router.refresh() anywhere — it would refire this effect mid-session and
  // clobber those optimistic mutations (audit SR-3).
  useEffect(() => {
    setRows(initialRows);
  }, [initialRows]);
