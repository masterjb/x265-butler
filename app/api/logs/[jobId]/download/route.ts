...
//   Both now live in src/lib/log/job-log-path.ts, shared with the read route.
//
// ISS-002 (2026-09-06): the logs dir is resolved through the DC-B resolver, not
// from a raw `cache_pool_path` read. The raw read returned 404 `log_not_found`
// for every job on any install without an explicit override — this is the exact
// JSON the forum reporter saw on the download link. See
