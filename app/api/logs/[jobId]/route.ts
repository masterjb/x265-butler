...
//   Both now live in src/lib/log/job-log-path.ts, shared with the download route.
//
// ISS-002 (2026-09-06): the logs dir is resolved through the DC-B resolver, not
// from a raw `cache_pool_path` read. The raw read 404'd every job on any install
// without an explicit override — the default since 24-03, and every upgrader
// after the 36-03 migration. See src/lib/encode/cache-path-access.ts.
//
...
import { requireAuth, withRenewCookie } from '@/src/lib/auth/require-auth';
...
import { JOB_ID_REGEX, resolveJobLogsDir, safeJobLogPath } from '@/src/lib/log/job-log-path';
...
export async function GET(
  ...
  // ISS-002: never null — an unset setting resolves, it does not mean "absent".
  const { logsDir } = resolveJobLogsDir();
