// 32-02: POST /api/queue/resume — clears the in-memory pause flag; dispatch
// restarts immediately (setQueuePaused(false) kicks dispatchUntilFull, so a queued
// job starts without waiting for the IDLE_POLL_MS tick). Mirrors cancel-all/route.ts
// hardening 1:1 (auth gate, 413 >16KB, 415 non-JSON, strict empty-body zod, single
...
export async function POST(request: Request): Promise<Response> {
  ...
  const log = logger.child({ requestId, route: '/api/queue/resume' });
  ...
  try {
    ...
    // the post-state, so a double-resume returns 200 { paused: false } both times.
    setQueuePaused(false);

    // Single authoritative actor-attributed audit line (SR-2: the setter logs a
    // distinct queue_pause_state_changed breadcrumb, not queue_resumed).
    log.info({ action: 'queue_resumed', actorId }, 'queue resumed');

    return jsonResponse({ paused: false, requestId }, 200);
  } catch (err) {
    log.error(
      { err: err instanceof Error ? err.stack : String(err) },
      '/api/queue/resume: unexpected error',
    );
