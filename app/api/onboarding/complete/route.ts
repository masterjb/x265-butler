...
// 50-03 (AC-9): re-exported so `src/lib/scan/media-eligibility.ts` can be held
// against the LIVING default instead of a fifth typed copy of the list. This
// route is the source a fresh install actually writes into
// `shares.extensions_csv`, so it is the anchor that notices drift. Added as a
// separate statement rather than an `export` keyword on the const above so the
// route shows zero deleted lines in the diff.
export { ONBOARDING_DEFAULT_EXT_CSV };

// 48-02 (Bundle A, door 2 — AC-4/AC-4b): `scan_root` was
