...
// 34-01 (MH-2): PUT /api/settings must drop the globalThis detection cache when
// gpu_device changes, else a curl-PUT of the device is a silent no-op until a
// separate POST /api/encoders/refresh or a container restart. Import from the
// detection SUBPATH, NOT the `@/src/lib/encode` barrel — the barrel runs
// makeDefaultDeps() at module-load (orchestrator side-effect) which breaks the
// partial-db-mock settings route tests; detection.ts has no such load side-effect.
import { invalidateEncoderCache } from '@/src/lib/encode/detection';
...
const settingsSchema = z
  .object({
    ...
    sidecar_central_path: z
      ...
      .optional(),
    // 33-02: operator-configurable originals-trash root. DELIBERATELY DIVERGES
    // from sidecar_central_path's `.min(1)` reject-empty — here empty IS valid
    // and means "auto" (track the cache stageRoot = byte-identical to pre-33-02),
    // mirroring cache_pool_path's empty=auto idiom. There is no standalone code
    // default to fall back to (trash tracks stageRoot which itself auto-resolves
    // the cache mount), so an empty string is the semantically-correct unset
    // state + the one-click revert lever. Non-empty MUST be absolute; system-root
    // + nested-under-share rejection happen in the PUT handler below.
    trash_path: z
      .string()
      .max(4096)
      .refine((v) => v === '' || v.startsWith('/'), {
        message: 'trash_path must be an absolute path starting with / (or empty for auto)',
      })
      .optional(),
    // 34-01: operator-pinned HW-encoder device node. A DRM RENDER node, NOT a
    // storage mount — so a TIGHTER shape than trash_path's `startsWith('/')`:
    // empty (auto) OR `/dev/dri/renderD<N>`. WHY tighter: accepting an arbitrary
    // absolute path would let a typo bind a non-device, and there is NO
    // forbidden/nested-share concern (the walker never scans /dev). `card<N>`
    // nodes are DISPLAY nodes, not render nodes → rejected by the renderD-only
    // regex. A zod refine failure exits at the safeParse layer as
    // `400 { error:'invalid_body', details:[...issues] }` (path∋gpu_device) —
    // gpu_device deliberately has NO manual post-safeParse block (no nested-share /
    // forbidden), so there is NO validation_failed/fieldErrors server path.
    gpu_device: z
      .string()
      .optional()
      .refine((v) => v === undefined || v === '' || /^\/dev\/dri\/renderD\d+$/.test(v), {
        message: 'gpu_device must be empty (auto) or a /dev/dri/renderD<N> device node',
      }),
    // 35-02: auto-crop (surfaces 35-01 backend). auto_crop bool-string; crop_override
    // = fixed W:H:X:Y geometry, empty = auto/none. Validated via the SAME dep-free
    // parseCropGeometry the client formSchema uses (AC-5 single-source, zero drift).
    // audit SR-1: the refine MUST trim-test (`v.trim() === ''`) NOT bare `v === ''`
    // — parseCropGeometry('  ') is null and '  ' !== '', so the bare form would
    // 400-reject a whitespace-only value at safeParse BEFORE the post-safeParse
    // empty-trim block (below) can normalize it. Trim-tolerant accepts ''/whitespace/
    // valid, rejects invalid/odd. A refine failure exits as
    // `400 { error:'invalid_body', details:[...issues] }` (like gpu_device — NO
    // manual fieldErrors block; crop has no nested-share/forbidden concern).
    auto_crop: z.enum(['true', 'false']).optional(),
    // 43-01: force 10-bit HEVC Main10 output (surfaces 43-01 backend). bool-string,
    // mirror auto_crop. Code-fallback default false → absent = byte-identical pre-43.
    force_10bit: z.enum(['true', 'false']).optional(),
    // 43-03: color passthrough (surfaces 43-03 backend). bool-string, mirror
    // force_10bit. Code-fallback default false → absent = byte-identical.
    color_passthrough: z.enum(['true', 'false']).optional(),
    crop_override: z
      .string()
      .max(32)
      .refine((v) => v.trim() === '' || parseCropGeometry(v) !== null, {
        message: 'crop_override must be empty (auto) or even W:H:X:Y geometry',
      })
      .optional(),
...
export async function PUT(request: Request): Promise<Response> {
  ...
  // 33-02: mirror the cache_pool_path empty-trim — a whitespace-only trash_path
  // persists as '' (= auto, track stageRoot) so the truthy-guarded gates below
  // (nested / forbidden) correctly skip the auto case.
  if (typeof updates.trash_path === 'string' && updates.trash_path.trim() === '') {
    updates.trash_path = '';
  }

  // 34-01: mirror the empty-trim — a whitespace-only gpu_device persists as ''
  // (= auto, first-enumerated node). The zod regex already rejects non-empty
  // junk, so this only normalizes the all-whitespace "clear" case.
  if (typeof updates.gpu_device === 'string' && updates.gpu_device.trim() === '') {
    updates.gpu_device = '';
  }
  // 35-02 (audit SR-1): mirror the empty-trim — a whitespace-only crop_override
  // persists as '' (auto/none). The trim-tolerant zod refine above lets ''/
  // whitespace through so this block runs; a valid geometry is left untouched
  // (parseCropGeometry already normalized nothing here — raw passes to the DB).
  if (typeof updates.crop_override === 'string' && updates.crop_override.trim() === '') {
    updates.crop_override = '';
  }
  ...
  try {
    ...
    // 33-02 (audit-S2 mirror): a configured trash_path becomes a real on-disk
    // tree of 30-day originals → same system-root foot-gun as cache_pool_path.
    // Reuse isForbiddenCachePath (do NOT duplicate the prefix list). The truthy
    // guard skips the empty/auto case (empty → stageRoot, already guarded).
    if (updates.trash_path && isForbiddenCachePath(updates.trash_path)) {
      log.warn(
        {
          action: 'settings_change_rejected',
          field: 'trash_path',
          code: 'forbidden_prefix',
          trashPath: updates.trash_path,
          requestId,
        },
        'forbidden trash_path prefix',
      );
      return jsonResponse({ error: 'forbidden_trash_path', requestId }, 400);
    }

    // 33-02 (audit-MH-1, RELEASE-BLOCKING): trash_path must NOT equal, be nested
    // under, OR contain any scanned share path. Phase 33 is the anti-double-work
    // phase — trash placed inside a scanned share → the walker re-ingests the
    // 30-day originals as new files → re-detect/re-encode loop (the exact bug
    // class this phase exists to kill). cache_pool_path carries this identical
    // guard because trash historically lived under stageRoot (= cache_pool_path),
    // which is itself share-nesting-guarded; making trash_path independently
    // configurable WITHOUT this guard reopens the hole. The walker has NO
    // dirname-based 'trash/' exclusion, so content_hash dedup is the only
    // fragile second-line defense — unacceptable as the SOLE protection here.
    // Merged-state check (current + updates) so a partial PUT can't sneak past.
    // BIDIRECTIONAL: reject trash nested under a share (norm.startsWith(sp+'/'))
    // AND a share nested under trash (sp.startsWith(norm+'/')) — the dangerous
    // direction here is the walker DESCENDING from a share-root INTO the trash
    // subtree, which the cache_pool_path block (one-directional) does not cover.
    const effectiveTrashPath = updates.trash_path ?? mergedSnapshot.trash_path;
    if (effectiveTrashPath) {
      const normalizedTrash = effectiveTrashPath.replace(/\/+$/, '') || '/';
      const conflict = shareRepo()
        .listAll()
        .find((s) => {
          const sp = s.path === '/' ? '/' : s.path.replace(/\/+$/, '');
          return (
            sp === normalizedTrash ||
            normalizedTrash.startsWith(sp + '/') ||
            sp.startsWith(normalizedTrash + '/') ||
            sp === '/'
          );
        });
      if (conflict) {
        log.warn(
          {
            ...
            field: 'trash_path',
            code: 'nested_under_share',
            trashPath: effectiveTrashPath,
            requestId,
            conflictingShareName: conflict.name,
            conflictingSharePath: conflict.path,
          },
          'trash_path collides with share — rejecting',
        );
        return jsonResponse(
          {
            error: 'validation_failed',
            fieldErrors: {
              trash_path: 'trash_path_nested_under_share',
            },
            conflictingShareName: conflict.name,
            requestId,
          },
    ...
    // 28-03 (R9): the cache_pool_path row-delete is folded INTO this txn so a
    // clear+update PUT commits as ONE atomic unit — a throw in the delete now
    // rolls back the key-updates too (previously the delete ran post-commit,
    // leaving a partial-write window).
    const writeAll = db.transaction((entries: [string, string][], doClear: boolean) => {
    ...
    // is row-DELETED inside the txn, never stored as the empty string (so the
    // resolver's unset-branch fires on the next read).
    const entries: [string, string][] = Object.entries(updates)
      .filter(([k, v]) => !(k === 'cache_pool_path' && v === ''))
      .map(([k, v]) => [k, String(v)]);
    writeAll(entries, clearCachePath);

    // 24-03 (AC-7): clear-to-unset audit-trail line (the row-delete itself now
    // happens inside writeAll above; logging is NOT transactional so it stays
    // out here — mirror the per-key log shape; the entries-loop below skips it
    // since it is no longer in `entries`).
    if (clearCachePath) {
      log.info(
        {
          requestId,
    ...
    const actorUsername = auth.ok && auth.mode === 'authenticated' ? auth.username : null;
    for (const [k, v] of entries) {
      ...
      log.info(
        {
          ...
          evicted,
          requestId,
          username: auth.ok && auth.mode === 'authenticated' ? auth.username : null,
        },
    ...
    // 34-01 (MH-2 / AC-8): drop the globalThis detection cache ONLY when
    // gpu_device actually changed in this PUT. oldValues['gpu_device'] is the
    // pre-write raw value (populated above for every key in `updates`); a no-op
    // PUT (value unchanged, or gpu_device not in the body) must NOT churn the
    // cache. The next bare detectEncoders() (encoders page / aggregator / next
    // dispatch) then re-resolves the node via the MH-1 single-source read (AC-7).
    if (
      typeof updates.gpu_device === 'string' &&
      updates.gpu_device !== (oldValues['gpu_device'] ?? '')
    ) {
      invalidateEncoderCache();
      log.info(
        {
          action: 'gpu_device_changed',
          oldValue: oldValues['gpu_device'] ?? null,
          newValue: updates.gpu_device,
          requestId,
        },
        'gpu_device changed — encoder detection cache invalidated',
      );
    }
