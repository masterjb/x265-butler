...
//
// ⚠ 50-04 — `:id` MEANS DIFFERENT THINGS IN POST AND DELETE:
//   POST   /api/library/<file.id>/blocklist            → :id is a file.id
//   DELETE /api/library/<blocklist_entry.id>/blocklist → :id is a blocklist_entry.id
// The two id spaces are independent AUTOINCREMENT sequences and collide from the
// first row on (measured: file ids 1,2,3 / entry ids 1,2 — entry 1 pins file 2).
// A caller that passes a file id to DELETE removes a STRANGER'S entry and flips
// THAT file to 'pending'. New callers SHOULD pass `?expectFileId=<file.id>`: the
// route then 409s instead of acting when the entry pins a different file.
...
export async function DELETE(req: Request, ctx: RouteContext): Promise<Response> {
  ...
  // 50-04 stage 2 of 3 — SYNTAX check of the optional ?expectFileId guard.
  // Runs AFTER gateAuth + the NEXT_PHASE build guard above (a 400 naming this
  // parameter is a statement about the route that an anonymous caller must not
  // get) and BEFORE findById, so a malformed value costs no DB access.
  // Absent → skipped entirely: components/blocklist/remove-entry-button.tsx
  // deletes PATTERN entries, which have no file_id at all, and must keep working
  // byte-identically.
  const expectFileIdRaw = new URL(req.url).searchParams.get('expectFileId');
  let expectFileId: number | null = null;
  if (expectFileIdRaw !== null) {
    const parsed = Number(expectFileIdRaw);
    if (!Number.isInteger(parsed) || parsed <= 0) {
      return jsonResponse({ error: 'invalid_expect_file_id', requestId }, 400);
    }
    expectFileId = parsed;
  }
  ...
  try {
    ...
    // 50-04 stage 3 of 3 — IDENTITY check, after findById and before ANY write
    // (neither setStatus nor remove has run at this point).
    //
    // WHY THIS EXISTS: `file.id` and `blocklist_entry.id` are independent
    // AUTOINCREMENT spaces and collide immediately. Measured: three files
    // (ids 1,2,3) and two entries (ids 1,2) — entry 1 pins file 2. A caller that
    // puts a FILE id in the path where an ENTRY id belongs therefore deletes a
    // STRANGER'S entry and flips THAT file to 'pending'. The POST branch of this
    // very route reads :id as file_id, which makes the confusion one typo away.
    //
    // This guards a coding mistake, NOT a race: the id column is
    // INTEGER PRIMARY KEY AUTOINCREMENT (migrations/0008_blocklist.sql), so
    // SQLite never recycles ids — a stale entryId can only 404, never land on a
    // foreign entry. Do not "simplify" this away as a redundant race guard.
    //
    // A pattern entry (file_id IS NULL) never matches a positive expectFileId,
    // so passing the parameter against a pattern is a mismatch by construction.
    if (expectFileId !== null && entry.file_id !== expectFileId) {
      log.warn(
        {
          action: 'blocklist_delete_file_mismatch',
          entryId,
          entryFileId: entry.file_id,
          expectFileId,
        },
        'blocklist DELETE rejected — entry does not pin the expected file',
      );
      return jsonResponse({ error: 'entry_file_mismatch', requestId }, 409);
    }
