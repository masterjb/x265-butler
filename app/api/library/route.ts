...
import { blocklistRepo, fileRepo, shareRepo } from '@/src/lib/db';
import { logger } from '@/src/lib/logger';
import { libraryQuerySchema, toListOptions } from '@/src/lib/api/library-query';
import { buildBlocklistEntryIdMap } from '@/src/lib/api/blocklist-entry-map';
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    // 50-04: file-pinned blocklist entry ids for this page's `blocklisted` rows.
    // Additive field — `rows` keeps its exact FileRow shape (no JOIN, no new
    // column). Skips the query entirely when the page holds no blocklisted row.
    const blocklistEntryIds = buildBlocklistEntryIdMap(rows, (ids) =>
      blocklistRepo().findByFileIds(ids),
    );
