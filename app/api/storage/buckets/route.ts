// 15-01: GET /api/storage/buckets — fixed 4-bucket size distribution.
//
// Body: buckets (4 rows, SR4 canonical empty-state = 4 zero-rows even when
// DB has 0 files) + effectiveFilters.share + computedAt + dataAsOf.
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    const buckets = storageRepo().getSizeBuckets({ shareId: shareParam });
    logQueryExecuted(ctx, ENDPOINT, { share: shareParam, rowCount: buckets.length }, startedAt);
    return jsonResponse(
      {
        buckets,
