  ...
  const stream = new ReadableStream<Uint8Array>(
    {
      start(controller) {
        ...
        // 48-03 audit M1: explicitly annotated, because JSON.stringify would pass
        // the literal through unchecked — the required `encodingJobs` field from
        // events.ts would NOT bite here, and this is the very first frame every
        // freshly connected tab reads. `paused` stays deliberately omitted (see
        // plan boundaries): Omit<> makes the omission intentional, not accidental.
        // The rows are reused for the progress replay below (AC-3) — no second
        // listActive() call in this request path.
        const counts = queueCountsFromRows(active, pending);
        const frame: Omit<Extract<EngineEvent, { type: 'queue.updated' }>, 'paused'> = {
          type: 'queue.updated',
