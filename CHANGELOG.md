...
## [2.48.0] — The job log viewer works again

amd64-only image on a Debian 13 (Trixie) base. **No new dependencies, no database migration, no
settings change required.** A single-fix release, from two independent reports of the same thing:
opening a job's log showed nothing, and downloading it returned
`{"error_code":"log_not_found"}`. Both reporters had already ruled out file permissions, and both
were right — the file system was never consulted. **Encoding behaviour is untouched.**

### Fixed

- **Job logs are readable again on any install without an explicit cache path.** The log viewer
  and the download route read the cache pool path setting directly and returned 404 when it was
  empty — before ever checking the disk. But empty is the normal state: since 2.21.0 an empty
  setting means auto-resolve, and Butler stages and logs to `/mnt/cache/x265-butler` or
  `/config/cache`. After the 2.33.0 legacy-path migration that covers almost every installation.
  Both routes now resolve the path the same way the encoder does. A log that genuinely is not on
  disk still returns 404 — but only after looking for it. An explicit path was always honoured
  and still is, so a manual workaround can stay in place.
- **The Dashboard no longer shows a dash for the cache path.** The system card renders an empty
  value as a dash, so every affected install was told it had no cache path while jobs were
  staging there. It now reports the path actually in use.

### Internal

- The same defect had shipped once before in the log-retention sweep (fixed in 2.34.0) and
  returned here because the underlying shape was still available. There is now exactly one place
  in the codebase that reads this setting, and it always returns a resolved path; an automated
  check fails the build if a raw read reappears. The test suite had been asserting the broken
  behaviour as intended, which is why it survived — that test now asserts the opposite.
