...
import { RemoveFromBlocklistAction } from './remove-from-blocklist-action';
...
import type { BlocklistEntryIdMap } from '@/src/lib/api/blocklist-entry-map';
import {
  formatBytes,
  formatBitrate,
  formatDuration,
...
export function LibraryTable({
  ...
  rows: FileRow[];
  // 50-04: fileId → blocklist_entry.id. Optional with an empty default so the
  // existing render tests keep passing unchanged; a missing map means exactly
  // "no removable entry known", which is also the correct fallback.
  blocklistEntryIds?: BlocklistEntryIdMap;
  sort: SortKey;
  dir: SortDir;
    ...
    <Table>
      ...
      <TableBody>
        {rows.map((row) => {
          ...
          // 50-04: keyed off row.status (the SERVER truth), not displayStatus —
          // an optimistic override is a local guess and must not decide whether
          // a destructive-ish action is offered.
          const blocklistEntryId = blocklistEntryIds?.[String(row.id)];
          const chipTitle =
            row.status === 'blocklisted' && blocklistEntryId === undefined
              ? t('unblocklist.chipTitle.noEntry')
              : undefined;
          return (
            <TableRow
              key={row.id}
            ...
            >
              ...
              <TableCell className="text-right">
                ...
                >
                  ...
                  {/* 50-04: the inverse of AddToBlocklistAction — the two are
                      mutually exclusive by status, so this reuses that slot. */}
                  <RemoveFromBlocklistAction file={row} entryId={blocklistEntryId} />
