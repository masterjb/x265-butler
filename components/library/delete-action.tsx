  ...
  const handleConfirm = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (res.status === 409) {
        ...
        if (code === 'delete_blocked_active_bench') {
          // 47-03 AC-13 (/ui-ux-pro-max §8 Error Recovery): the text names the
          // way out ("cancel or finish the run") but the run lives on /bench
          // while the operator is standing on /library — without this action
          // the recovery path is named yet unreachable. Same shape as
          // components/settings/paths-tab-shares.tsx:240-243 incl. its ?from=
          // provenance convention.
          toast.error(t('toast.activeBench'), {
            action: {
              label: t('toast.activeBenchAction'),
              onClick: () => router.push('/bench?from=library-delete'),
            },
          });
          return;
        }
