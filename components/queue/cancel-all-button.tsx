...
// 48-03: the counter summed activeJobs (= queued+encoding) + pendingJobs, so
// every queued row counted twice — 1994 for a 999-row queue. Now encoding+queued,
// exactly the set /api/queue/cancel-all touches.
...
export function CancelAllButton() {
  ...
  // The sum form (rather than the shorter queued+encoding field) is deliberate:
  // it survives a future semantic drift of that field and it makes visible at the
  // call site which two sets the endpoint touches.
  const ssecounter = (counts.encodingJobs ?? 0) + (counts.pendingJobs ?? 0);
