...
import { computeEtaSeconds, formatEtaShort } from '@/src/lib/format/eta';
...
export function ActiveJobRow({
  ...
  // 43-02: ETA from ffmpeg `speed`. null (hide) unless durationSeconds>0,
  // outTimeMs!=null AND speed>0 — never "—"/NaN. Refreshes ~every 30s (the
  // -stats_period cadence). Reuses the same inputs as progressPctOf + speed.
  const eta = formatEtaShort(
    computeEtaSeconds(fileDurationSeconds ?? null, job.outTimeMs, job.speed),
  );
