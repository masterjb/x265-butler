...
import { groupFixRelevant } from '@/src/lib/diagnostics/render-device-group';
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        {/* 46-02 (D2): NVIDIA GPU model + driver — adjacent to CPU (both hardware
            capability surfaces). Evidence-only: NO AggregatedWarning, no
            color-alone, no animation. Reads the SSR-seeded payload (no fetch).
            Empty case renders a source-keyed fallback (never a raw dotted key). */}
        <DiagnosticsSection title={t('nvidiaGpu.title')} icon={Cpu}>
          {nvidiaGpu.gpus.length > 0 ? (
            <dl className="grid grid-cols-[max-content_1fr] gap-x-4 gap-y-1 text-sm">
              {nvidiaGpu.gpus.map((gpu, i) => (
                <Fragment key={`${gpu.name}-${i}`}>
                  <dt className="text-muted-foreground break-all">{gpu.name}</dt>
                  <dd className="font-mono">
                    {t('nvidiaGpu.labelDriver')}: {gpu.driverVersion}
                  </dd>
                </Fragment>
              ))}
            </dl>
            ...
            <p className="text-sm text-muted-foreground">
              {t(`nvidiaGpu.source.${nvidiaGpu.source}`)}
            </p>
          )}
        </DiagnosticsSection>
          ...
          {devices.renderDevices.length === 0 ? (
            ...
            <>
                ...
                {devices.renderDevices.map((d, i) => (
                  <li key={`${d.path}-${i}`} className="break-all font-mono text-xs">
                    ...
                    {!d.exists ? (
                      ...
                      // 29-02 — the ONLY amber branch: an ACTUAL R/W failure. Never
                      // color-alone (WCAG AA): icon + ✗ + text always paired.
                      <span className="inline-flex items-center gap-1 text-amber-600 dark:text-amber-400">
                        <AlertTriangle className="size-3.5 shrink-0" aria-hidden="true" />✗{' '}
                        {t('renderDevices.accessProblem')}
                      </span>
                    ) : (
                      // RW-OK: muted reassurance. In-group keeps its line; not-in-group
                      // (rasalf: gid 18, R+W pass) gets a neutral "group not required".
                      <span className="text-muted-foreground">
                        ✓{' '}
                        {d.inRenderGroup
                          ? t('renderDevices.inGroup')
                          : t('renderDevices.accessOkNoGroup')}
                      </span>
                    )}
                  </li>
                ))}
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                {devices.renderDevices.some(groupFixRelevant)
                  ? t('renderDevices.groupNoteFix')
                  : t('renderDevices.groupNoteOk')}
              </p>
            </>
          )}
        </DiagnosticsSection>
