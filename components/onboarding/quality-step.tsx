...
// 49-05: LEAF path on purpose — a value import from profiles.ts / the encode
// barrel would pull node:os + pino into this 'use client' bundle (MH-1).
        ...
        <form onSubmit={form.handleSubmit(onComplete)} className="flex flex-col gap-5" noValidate>
          ...
          <AutoScanAwareness
            onDeepLinkClick={() => {
              ...
              // 49-05: fallbacks from the ONE default source. Import is the
              // LEAF path — this file is 'use client' and profiles.ts / the
              // barrel would drag node:os + pino into the client bundle (MH-1).
              const normalized: QualityStepValues = {
                crf_libx265: Number.isFinite(raw.crf_libx265)
                  ? raw.crf_libx265
                  : DEFAULT_CRF_BY_ENCODER.libx265,
                crf_nvenc: Number.isFinite(raw.crf_nvenc)
                  ? raw.crf_nvenc
                  : DEFAULT_CRF_BY_ENCODER.nvenc,
                crf_qsv: Number.isFinite(raw.crf_qsv) ? raw.crf_qsv : DEFAULT_CRF_BY_ENCODER.qsv,
                crf_vaapi: Number.isFinite(raw.crf_vaapi)
                  ? raw.crf_vaapi
                  : DEFAULT_CRF_BY_ENCODER.vaapi,
              };
