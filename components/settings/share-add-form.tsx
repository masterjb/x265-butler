...
export function ShareAddForm({ onAdd, isAdding = false }: ShareAddFormProps) {
  ...
  // 48-02 (audit S2): server `fieldErrors` values are CODES
  // (`path_forbidden_prefix`, `path_traversal_rejected`, `name_invalid_chars`,
  // `extensions_csv_*`), not sentences — and FormMessage renders them verbatim.
  // Render the localized sentence when a key exists, else fall back to the raw
  // value (which for a zod-native message IS the sentence, so non-code messages
  // stay byte-identical). next-intl THROWS on a missing key, so an unguarded
  // `t()` would turn an unknown server code into a client crash.
  function serverErrorMessage(code: string): string {
    try {
      return t.has(`error.${code}`) ? t(`error.${code}`) : code;
    } catch {
      return code;
    }
  }
    ...
    if (err.kind === 'validation') {
      for (const [field, message] of Object.entries(err.fieldErrors)) {
        if (
          ...
          form.setError(field, { type: 'server', message: serverErrorMessage(message) });
        }
      }
