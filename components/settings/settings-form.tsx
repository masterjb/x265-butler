      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-6">
        ...
        {tab === 'encoder' && (
          <>
            ...
            {/* 35-02 (D1=own-card): Auto-Crop — own Card after the GPU-Device card.
                Surfaces 35-01's backend auto_crop + crop_override. The id="auto-crop"
                anchor is the onboarding deep-link target (AC-6/AC-9). Default
                (off/empty) = byte-identical pre-35. */}
            <Card id="auto-crop">
              <CardHeader>
                <CardTitle>{t('section.autoCrop.title')}</CardTitle>
                <CardDescription>{t('section.autoCrop.description')}</CardDescription>
              </CardHeader>
              <CardContent>
            ...
            {/* 43-01: Force-10bit — own Card after Auto-Crop. Surfaces 43-01's
                backend force_10bit (HandBrake "encode 8-bit as 10-bit"). Default
                unchecked = byte-identical pre-43 for all four encoders (AC-1/AC-7). */}
            <Card id="force-10bit">
              <CardHeader>
                <CardTitle>{t('section.force10bit.title')}</CardTitle>
                <CardDescription>{t('section.force10bit.description')}</CardDescription>
              </CardHeader>
              <CardContent>
            ...
            {/* 43-03: Color-passthrough — own Card after Force-10bit. Surfaces
                43-03's backend color_passthrough (preserve source VUI color tags).
                Default unchecked = byte-identical for all four encoders (AC-1/AC-7). */}
            <Card id="color-passthrough">
              <CardHeader>
                <CardTitle>{t('section.colorPassthrough.title')}</CardTitle>
                <CardDescription>{t('section.colorPassthrough.description')}</CardDescription>
              </CardHeader>
              <CardContent>
