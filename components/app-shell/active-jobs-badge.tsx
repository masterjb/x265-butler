...
export function ActiveJobsBadge() {
  ...
  // 48-03 (audit M4): the bootstrap only applies while NO live value has arrived
  // yet. A `> 0` test would be wrong after the switch: encodingJobs === 0 is a
  // valid live state (between two encodes, throttled dispatch, paused queue) and
  // must not bring the stale mount-time number back. Bootstrap source
  // (/api/queue/status.encodingJobs) and live source now agree — before 48-03 the
  // badge jumped from the correct 4 to the wrong 999 on the first SSE event.
  const encodingJobs = counts.hasLiveCounts ? counts.encodingJobs : bootstrap;

  // Browser tab title prefix (audit S3: didPrefix tracking)
  useEffect(() => {
    if (encodingJobs > 0) {
      document.title = `(${encodingJobs}) ${BASE_TITLE}`;
  ...
  if (encodingJobs === 0) return null;

  return (
    <Link
      href={`/${locale}/queue`}
      aria-label={t('aria', { count: encodingJobs })}
    ...
    >
      <Badge variant="default" className="gap-1">
        ...
        <span className="font-mono tabular-nums">{encodingJobs}</span>
        <span>{t('label')}</span>
      </Badge>
    </Link>
  );
