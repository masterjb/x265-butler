...
// 48-03: exposes the full triple + the hasLiveCounts gate so AC-6/AC-7/M4 can be
// asserted without changing the format the pre-48-03 CountsDisplay tests pin.
function FullCountsDisplay() {
  const c = useQueueCounts();
  return (
    <div data-testid="full-counts">
      {`${c.activeJobs}/${c.pendingJobs}/${c.encodingJobs}/${c.hasLiveCounts}`}
    </div>
  );
}
...
describe('useActiveJobs — multi-slot SSE reducer (36-02)', () => {
  ...
  it('test_useActiveJobs_when_map_unchanged_then_returns_stable_array_ref', () => {
    ...
    send({ type: 'queue.updated', activeJobs: 1, pendingJobs: 0, encodingJobs: 1 });
    send({ type: 'queue.updated', activeJobs: 1, pendingJobs: 2, encodingJobs: 0 });
...
describe('useQueueCounts — queue.updated reducer', () => {
  ...
  it('test_useQueueCounts_when_queue_updated_then_reflects_new_counts', () => {
    renderProvider(<CountsDisplay />);
    send({ type: 'queue.updated', activeJobs: 1, pendingJobs: 3, encodingJobs: 1 });
    expect(screen.getByTestId('counts').textContent).toBe('1/3');
  });

  // --- 48-03 ---

  it('test_AC6_when_initial_then_encodingJobs_zero_and_hasLiveCounts_false', () => {
    renderProvider(<FullCountsDisplay />);
    expect(screen.getByTestId('full-counts').textContent).toBe('0/0/0/false');
  });

  it('test_AC6_when_queue_updated_then_all_three_counts_present', () => {
    // The reported v2.44.0 state: 999 active rows, 995 waiting, 4 encoding.
    renderProvider(<FullCountsDisplay />);
    send({ type: 'queue.updated', activeJobs: 999, pendingJobs: 995, encodingJobs: 4 });
    expect(screen.getByTestId('full-counts').textContent).toBe('999/995/4/true');
  });

  it('test_when_event_lacks_encodingJobs_then_store_falls_back_to_zero_without_throwing', () => {
    // Bundle skew: an old client bundle reading a frame without the field. The
    // `?? 0` in the reducer is the guard — it must not throw and must not leave
    // the field undefined.
    renderProvider(<FullCountsDisplay />);
    send({ type: 'queue.updated', activeJobs: 3, pendingJobs: 3 });
    expect(screen.getByTestId('full-counts').textContent).toBe('3/3/0/true');
  });

  it('test_M4_hasLiveCounts_stays_true_after_an_event_carrying_encodingJobs_zero', () => {
    // The regression class of this plan: encodingJobs === 0 is a NORMAL live state.
    // It must not be readable as "store still empty" — otherwise the badge falls
    // back to its stale mount-time bootstrap number.
    renderProvider(<FullCountsDisplay />);
    send({ type: 'queue.updated', activeJobs: 995, pendingJobs: 995, encodingJobs: 0 });
    expect(screen.getByTestId('full-counts').textContent).toBe('995/995/0/true');
  });
});

describe('48-03 rebootstrap — encodingJobs + hasLiveCounts (AC-7)', () => {
  beforeEach(() => {
    captured.length = 0;
    MockEventSource.mockClear();
  });
  ...
  afterEach(() => {
    // Restore the module-level !ok stub so later suites see no state drift.
    vi.stubGlobal(
      'fetch',
      vi.fn(() => Promise.resolve({ ok: false })),
    );
  ...
  async function openWithStatus(payload: object) {
    vi.stubGlobal(
      'fetch',
  ...
  it('test_AC7_rebootstrap_adopts_encodingJobs_and_sets_hasLiveCounts', async () => {
    await openWithStatus({ paused: false, activeJobs: 999, pendingJobs: 995, encodingJobs: 4 });
    expect(screen.getByTestId('full-counts').textContent).toBe('999/995/4/true');
  });

  it('test_AC7_rebootstrap_without_encodingJobs_falls_back_to_zero_without_throwing', async () => {
    // Skew guard (audit S6): a new client bundle against a still-old server.
    await openWithStatus({ paused: false, activeJobs: 5, pendingJobs: 5 });
    expect(screen.getByTestId('full-counts').textContent).toBe('5/5/0/true');
  });
});
