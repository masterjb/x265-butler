...
describe('fileRepo.listPaginated (01-04)', () => {
  ...
  // 28-01 P6 (single-round-trip COUNT(*) OVER()): SR1 + M1 parity.
  it('test_listPaginated_P6_does_not_leak___total_into_returned_rows', () => {
    // SR1: __total must be stripped — the API serializes rows straight to client.
    seedRows(repo, 3);
    const result = repo.listPaginated({ page: 1, size: 50, sort: 'size', dir: 'desc' });
    expect(result.rows).toHaveLength(3);
    expect(result.total).toBe(3);
    for (const row of result.rows) {
      expect(Object.prototype.hasOwnProperty.call(row, '__total')).toBe(false);
    }
  });

  it('test_listPaginated_P6_total_reflects_full_filtered_set_not_page_slice', () => {
    // COUNT(*) OVER() is evaluated before LIMIT/OFFSET → total is the full set.
    seedRows(repo, 25);
    const page1 = repo.listPaginated({ page: 1, size: 10, sort: 'size', dir: 'asc' });
    expect(page1.rows).toHaveLength(10);
    expect(page1.total).toBe(25);
  });

  it('test_listPaginated_P6_M1_offset_past_end_with_status_filter_returns_full_total', () => {
    // M1 fallback: an over-paginated EMPTY page carries NO __total, so
    // listPaginated must fall back to getCountStmt — the full filtered count
    // (5) must NOT collapse to 0. Mirrors the old 2-query path exactly.
    seedRows(repo, 3); // default 'pending'
    for (let i = 0; i < 5; i++) {
      repo.upsertByPath(
        baseInput({ path: `/f-${i}.mp4`, content_hash: `fa${i}`.padStart(64, '0') }),
      );
      db.prepare("UPDATE file SET status = 'failed' WHERE path = ?").run(`/f-${i}.mp4`);
    }
    const onPage = repo.listPaginated({
      page: 1,
      size: 50,
      status: 'failed',
      sort: 'size',
    ...
    expect(onPage.rows).toHaveLength(5);
    expect(onPage.total).toBe(5);

    const pastEnd = repo.listPaginated({
      page: 99,
      size: 50,
      status: 'failed',
      sort: 'size',
