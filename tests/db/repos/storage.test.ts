...
// 28-01 M5: import the REAL production trend SQL for the AC-4 EXPLAIN test.
import { TREND_30D_SQL } from '@/src/lib/db/repos/stats';
...
describe('storageRepo aggregation contract', () => {
  ...
  describe('getSharesTable', () => {
    ...
    // 28-01 P5/M2: after batching to GROUP BY share_id, a named share that
    // produces NO group row must default-on-miss to a zeroed row (byte-identical
    // to the old per-share COALESCE-zeroed result) — NOT NaN / crash / omission.
    it('28-01 M2: named share with ZERO files → fully zeroed row (no NaN, no crash)', () => {
      const a = insertShare(db, 'A', '/mnt/a');
      const empty = insertShare(db, 'Empty', '/mnt/empty');
      insertFile(db, { path: '/mnt/a/1.mkv', size_bytes: 100, codec: 'hevc', share_id: a });

      const rows = repo.getSharesTable();
      expect(rows).toHaveLength(2);
      const emptyRow = rows.find((r) => r.shareId === empty)!;
      expect(emptyRow.sharePath).toBe('/mnt/empty');
      expect(emptyRow.totalSizeBytes).toBe(0);
      expect(emptyRow.hevcPercent).toBe(0);
      expect(emptyRow.savingsBytes).toBe(0);
      expect(emptyRow.largestFolder).toBeNull();
    });

    it('28-01 M2: share with files but ZERO done-smaller savings → savingsBytes 0', () => {
      const a = insertShare(db, 'A', '/mnt/a');
      insertFile(db, { path: '/mnt/a/x/1.mkv', size_bytes: 500, codec: 'h264', share_id: a });
      // a 'done' job exists but the FILE is not done-smaller → no realized savings.
      const fileId = db.prepare("SELECT id FROM file WHERE path = '/mnt/a/x/1.mkv'").get() as {
        id: number;
      };
      insertJob(db, fileId.id, 600, 700); // bytes_out > bytes_in; file status stays 'pending'

      const rows = repo.getSharesTable();
      const aRow = rows.find((r) => r.shareId === a)!;
      expect(aRow.totalSizeBytes).toBe(500);
      expect(aRow.hevcPercent).toBe(0); // h264 → 0% HEVC
      expect(aRow.savingsBytes).toBe(0);
    });
  });

  // 28-01 AC-4 / P7: verify the stats trend-30d query rides the composite index.
  describe('trend-30d index coverage (28-01 P7)', () => {
    it('EXPLAIN QUERY PLAN over the real TREND_30D_SQL SEARCHes idx_job_status_finished_at', () => {
      const cutoff = 0;
      const plan = db.prepare('EXPLAIN QUERY PLAN ' + TREND_30D_SQL).all(cutoff) as Array<{
        detail: string;
      }>;
      const detail = plan.map((p) => p.detail).join(' | ');
      // SR4: the raw `status='done' AND finished_at>=?` WHERE is served by the
      // composite (status, finished_at) index via SEARCH (not a full SCAN).
      expect(detail).toMatch(/SEARCH .*idx_job_status_finished_at/i);
      // The co-present `USE TEMP B-TREE FOR GROUP BY` (date-bucket aggregation)
      // is EXPECTED and is NOT an index defeat — do not misclassify it as a miss.
    });
  });
