...
describe('BenchComboRepo', () => {
  ...
  // ── 47-01 (audit S1): scalar countByRun for the purge payload ────────────────
  describe('countByRun', () => {
    function makeCombo(sampleIdx: number) {
      return {
        file_id: fileId,
        encoder: 'libx265',
        preset: 'medium',
    ...
    it('test_countByRun_returns_row_count_for_populated_run', () => {
      comboRepo.createBatch(runId, [makeCombo(0), makeCombo(1), makeCombo(2)]);
      expect(comboRepo.countByRun(runId)).toBe(3);
    });

    it('test_countByRun_returns_0_for_run_without_combos', () => {
      expect(comboRepo.countByRun(runId)).toBe(0);
    });

    it('test_countByRun_returns_0_for_unknown_run', () => {
      comboRepo.createBatch(runId, [makeCombo(0)]);
      expect(comboRepo.countByRun(999_999)).toBe(0);
    });

    it('test_countByRun_is_scoped_to_the_requested_run', () => {
      const otherRunId = runRepo.create({
        mode: 'native-sweep',
        fileIds: [fileId],
        matrix: MATRIX,
      });
      comboRepo.createBatch(runId, [makeCombo(0), makeCombo(1)]);
      comboRepo.createBatch(otherRunId, [makeCombo(0)]);
      expect(comboRepo.countByRun(runId)).toBe(2);
      expect(comboRepo.countByRun(otherRunId)).toBe(1);
    });
  });
});
