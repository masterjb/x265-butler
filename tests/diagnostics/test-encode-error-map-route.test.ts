...
const { mockSettings, mockSpawn, mockDetectEncoders, mockEnsureServerInit, mockLoggerInfo } =
  vi.hoisted(() => ({
...
vi.mock('@/src/lib/encode', async () => {
  ...
  return {
    ...
    // 49-03: test-encode.ts also imports the synthetic pixel-format pin, its
    // encoder predicate and the forced-IDR verdict accessor from this barrel.
    // REAL pure values for the first two; the verdict defaults fail-OPEN, exactly
    // like the real accessor on a cold cache.
    SYNTHETIC_PROBE_PIX_FMT: profiles.SYNTHETIC_PROBE_PIX_FMT,
    usesSyntheticPixFmtPin: profiles.usesSyntheticPixFmtPin,
    isForcedIdrSupported: () => true,
  };
});
