...
describe('mapTestEncodeError — per-code coverage (≥13 incl. generic)', () => {
  ...
  const cases: Array<[string, string, string, 'error' | 'warning']> = [
    ...
    [
      // regression: a PURE too-old-driver stderr (no Required/Found API line) STILL
      // maps to nvencDriverMismatch — the new entry does not steal it.
      'nvencDriverMismatch',
...
describe('nvencApiTooNew — NVENC API-floor class (45-02)', () => {
  const API_FLOOR =
    'Driver does not support the required nvenc API version. Required: 13.1 Found: 13.0';

  it('maps the API-floor stderr to nvencApiTooNew/warning, NOT nvencDriverMismatch', () => {
    const r = mapTestEncodeError(API_FLOOR, 1, NVENC);
    expect(r).toEqual({ code: 'nvencApiTooNew', severity: 'warning' });
  });

  it('nvencApiTooNew is ordered BEFORE nvencDriverMismatch (first-match wins)', () => {
    const idx = (code: string) => TEST_ENCODE_ERROR_PATTERNS.findIndex((p) => p.code === code);
    expect(idx('nvencApiTooNew')).toBeGreaterThanOrEqual(0);
    expect(idx('nvencApiTooNew')).toBeLessThan(idx('nvencDriverMismatch'));
  });

  it('audit-S1 single-source predicate: true for canonical + variant, false for pure too-old-driver', () => {
    expect(isNvencApiTooNewStderr(API_FLOOR.toLowerCase())).toBe(true);
    // variant wording that still carries the Required/Found API pair near an nvenc token
    expect(
      isNvencApiTooNewStderr(
        '[hevc_nvenc @ 0x55] nvenc api mismatch — required: 13.1 found: 13.0'.toLowerCase(),
      ),
    ).toBe(true);
    // near-miss: pure too-old-driver, no Required/Found API line → NOT api-floor
    expect(
      isNvencApiTooNewStderr(
        'this nvenc build requires the minimum required nvidia driver'.toLowerCase(),
      ),
    ).toBe(false);
  });
});

describe('nvencHevcUnsupported — HEVC-unsupported GPU class (46-01)', () => {
  // the reporter's verbatim v2.42.0 stderr block (i7-4790K Haswell host).
  const REPORTER =
    ...
    '[vost#0:0/hevc_nvenc @ 0x55] Terminating thread with return code -22 (Invalid argument)';
  // Pascal API-floor fixture that ALSO carries the "could not open encoder" + "invalid
  // argument" tokens (SR-1): so the precedence guard proves ORDERING, not exclusivity.
  const PASCAL_ALSO_OPEN =
    'Driver does not support the required nvenc API version. Required: 13.1 Found: 13.0\n' +
    '[hevc_nvenc @ 0x55] Could not open encoder before EOF\n' +
    '[vost#0:0/hevc_nvenc @ 0x55] error code: -22 (Invalid argument)';

  it('AC-1: reporter -22 / "Could not open encoder" stderr → nvencHevcUnsupported/warning', () => {
    expect(mapTestEncodeError(REPORTER, 1, NVENC)).toEqual({
      code: 'nvencHevcUnsupported',
      severity: 'warning',
    });
  });

  it('AC-2 precedence: Pascal fixture matches BOTH predicates yet maps to nvencApiTooNew (ordering)', () => {
    const lc = PASCAL_ALSO_OPEN.toLowerCase();
    // both predicates provably fire on this fixture → the map result is an ORDERING
    // outcome, not predicate-exclusivity. A reorder of the two entries flips this RED.
    expect(isNvencHevcUnsupportedStderr(lc)).toBe(true);
    expect(isNvencApiTooNewStderr(lc)).toBe(true);
    expect(mapTestEncodeError(PASCAL_ALSO_OPEN, 1, NVENC)).toEqual({
      code: 'nvencApiTooNew',
      severity: 'warning',
    });
  });

  it('AC-2 ordering: nvencHevcUnsupported is after api_too_new + driverMismatch, and NOT the catch-net', () => {
    const idx = (code: string) => TEST_ENCODE_ERROR_PATTERNS.findIndex((p) => p.code === code);
    expect(idx('nvencHevcUnsupported')).toBeGreaterThanOrEqual(0);
    expect(idx('nvencApiTooNew')).toBeLessThan(idx('nvencHevcUnsupported'));
    expect(idx('nvencDriverMismatch')).toBeLessThan(idx('nvencHevcUnsupported'));
    // still a SPECIFIC pattern, never the last-index catch-all.
    expect(idx('nvencHevcUnsupported')).toBeLessThan(TEST_ENCODE_ERROR_PATTERNS.length - 1);
  });

  it('AC-5: qsv "(invalid argument)" WITHOUT "could not open encoder" → never nvencHevcUnsupported', () => {
    // vendor gate + phrase gate: qsv EINVAL stays on its own path.
    const r = mapTestEncodeError(
      '[hevc_qsv @ 0x55] hwaccel init failed (Invalid argument)',
      1,
      QSV,
    );
    expect(r?.code).not.toBe('nvencHevcUnsupported');
  });

  it('AC-5: predicate requires BOTH phrases (bare "could not open encoder" w/o EINVAL is false)', () => {
    expect(isNvencHevcUnsupportedStderr('could not open encoder before eof')).toBe(false);
    expect(isNvencHevcUnsupportedStderr('some other (invalid argument) failure')).toBe(false);
  });
});
...
// ─────────────────────────────────────────────────────────────────────────────
// 49-03 (audit M-1 / AC-19): the forced-IDR rejection signature.
//
// This predicate decides whether a failed CONFIRM spawn means "this runtime
// rejects the option" (→ auto-degrade to open GOPs) or "something else went
// wrong" (→ fail OPEN, keep the pin). Getting it wrong in the permissive
// direction is the expensive mistake: the confirm spawn is the THIRD GPU-session
// attempt of its probe chain and starts while the parallel vaapi probe may still
// hold the same Intel iGPU — the 39-01 self-contention shape. A transient failure
// read as an option rejection would silently undo 49-02 on a HEALTHY host for the
// whole process lifetime AND assert a runtime limit that does not exist.
// ─────────────────────────────────────────────────────────────────────────────
describe('isForcedIdrRejectedStderr — 49-03 AC-19', () => {
  const lc = (s: string): string => s.toLowerCase();

  it('matches the qsv spelling (forced_idr, UNDERSCORE)', () => {
    expect(
      isForcedIdrRejectedStderr(
        lc(
  ...
  it('matches the nvenc spelling (forced-idr, HYPHEN)', () => {
    expect(
      isForcedIdrRejectedStderr(lc('[hevc_nvenc @ 0x55] Unrecognized option forced-idr.')),
    ).toBe(true);
  });

  it('matches a bare "(Invalid argument)" only WITH the option name present', () => {
    expect(isForcedIdrRejectedStderr(lc('forced_idr ... (Invalid argument)'))).toBe(true);
    // …and NOT without it: a bare EINVAL also trails genuine device/hwaccel faults.
    expect(isForcedIdrRejectedStderr(lc('Device creation failed: -1 (Invalid argument)'))).toBe(
      false,
    );
  });

  it('does NOT match transient/unrelated failures (the fail-OPEN cases)', () => {
    for (const stderr of [
      '[hevc_qsv @ 0x55] Device busy\nConversion failed!',
      'Cannot allocate memory',
      'Error creating a MFX session: -9',
      'No VA display found',
      '', // empty tail
    ]) {
      expect(isForcedIdrRejectedStderr(lc(stderr)), stderr).toBe(false);
    }
  });

  it('the option name ALONE is not enough — an error phrase must co-occur', () => {
    expect(isForcedIdrRejectedStderr(lc('applying option forced_idr (Force IDR frames)'))).toBe(
      false,
    );
  });
});
