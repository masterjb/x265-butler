...
// 48-02 Task 1 (Bundle A, door 2) — AC-4 + AC-4b.
// The wizard route owns its OWN schema and calls shareRepo().create() directly,
// so Guard A only reaches it because `scan_root` now REUSES the exported
// `pathSchema` from shares-zod (not a second literal list, not a hand-rolled
// copy of the refines).
describe('POST /api/onboarding/complete — 48-02 forbidden-prefix guard', () => {
  beforeEach(() => {
    mockSettingSet.mockReset();
    mockEnsureServerInit.mockReset();
    mockShareListAll.mockReset();
  ...
  const REJECTED = [
    ...
    '/proc//1',
    '/mnt/../sys',
    '/mnt/user' + String.fromCharCode(0) + '/Media',
  ];

  for (const [i, bad] of REJECTED.entries()) {
    it(`test_POST_when_scan_root_alias_${i}_is_forbidden_then_400_invalid_body`, async () => {
      const res = await POST(jsonRequest({ scan_root: bad, min_size_mb: 50 }));
      expect(res.status, `${JSON.stringify(bad)} should be rejected`).toBe(400);
      const body = await res.json();
      expect(body.error).toBe('invalid_body');
      expect(mockShareCreate).not.toHaveBeenCalled();
      expect(mockSettingSet).not.toHaveBeenCalled();
  ...
  it('test_POST_when_scan_root_is_forbidden_then_rejection_comes_from_shared_pathSchema', async () => {
    // Not a second list: the SAME schema object the shares API validates
    // against carries the code, so the two doors can never drift.
    const res = await POST(jsonRequest({ scan_root: '/sys', min_size_mb: 50 }));
    expect(res.status).toBe(400);
    const body = (await res.json()) as { details: Array<{ message: string; path: string[] }> };
    const issue = body.details.find((d) => d.message === 'path_forbidden_prefix');
    expect(issue).toBeDefined();
    expect(issue?.path).toEqual(['scan_root']);
  });

  it('test_POST_when_scan_root_is_legit_then_200_and_share_created', async () => {
    mockShareCreate.mockReturnValue(sampleShare({ id: 7, path: '/mnt/user/Media' }));
    const res = await POST(jsonRequest({ scan_root: '/mnt/user/Media', min_size_mb: 50 }));
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.completed).toBe(true);
    expect(body.shareAction).toBe('created');
