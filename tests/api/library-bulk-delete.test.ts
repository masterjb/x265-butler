...
// single-delete guard reasons (active_job / active_bench / not_found), the AC-6
...
const mocks = vi.hoisted(() => ({
  ...
  mockFileIsReferencedByActiveBench: vi.fn(),
  mockFileCountBenchRefs: vi.fn(),
...
vi.mock('@/src/lib/db', () => ({
  ...
  fileRepo: () => ({
    ...
    isReferencedByActiveBench: mocks.mockFileIsReferencedByActiveBench,
    countBenchRefs: mocks.mockFileCountBenchRefs,
  }),
  default: {},
}));
...
beforeEach(() => {
  ...
  mocks.mockFileIsReferencedByActiveBench.mockReturnValue(false);
  mocks.mockFileCountBenchRefs.mockReturnValue(0);
...
describe('POST /api/library/bulk-delete', () => {
  ...
  it('partial — not_found + active_job + active_bench + deletable → 200 successCount=1', async () => {
    mocks.mockFileGetById.mockImplementation((id: number) => {
      ...
      if (id === 3) return { ...baseFile, id: 3 }; // active_bench (pre-check)
      if (id === 4) return { ...baseFile, id: 4 }; // success
      return undefined;
    });
    mocks.mockFileIsReferencedByActiveBench.mockImplementation((id: number) => id === 3);
  ...
  // AC-6 — FK constraint thrown by deleteById (race past pre-check) → active_bench (not 500).
  it('FK constraint from deleteById → active_bench (defense-in-depth)', async () => {
    mocks.mockFileGetById.mockImplementation((id: number) => ({ ...baseFile, id }));
    ...
    const body = await res.json();
    expect(body.successCount).toBe(0);
    expect(body.failed).toEqual([{ id: 1, reason: 'active_bench' }]);
  });

  // 47-03 AC-8 — a run turning active between pre-check and DELETE is caught by
  // the in-SAVEPOINT re-check. Spy: first call false (pre-check), second true.
  it('bench turns active between pre-check and delete → active_bench, no delete', async () => {
    mocks.mockFileGetById.mockImplementation((id: number) => ({ ...baseFile, id }));
    let calls = 0;
    mocks.mockFileIsReferencedByActiveBench.mockImplementation(() => {
      calls += 1;
      return calls >= 2;
    });
    const res = await POST(makeRequest({ ids: [1] }));
  ...
  // 47-03 AC-14 — audit line sums the severed bench references over the
  // SUCCESSFULLY deleted ids only.
  it('audit line reports benchRefsSevered summed over successful ids', async () => {
    mocks.mockFileGetById.mockImplementation((id: number) =>
      id === 9 ? undefined : { ...baseFile, id },
    );
    mocks.mockFileCountBenchRefs.mockImplementation((id: number) => (id === 1 ? 2 : 3));
    const res = await POST(makeRequest({ ids: [1, 2, 9] }));
    expect(res.status).toBe(200);
    expect((await res.json()).successCount).toBe(2);
    expect(mocks.mockLoggerInfo).toHaveBeenCalledWith(
      expect.objectContaining({ audit: 'bulk_library_delete', benchRefsSevered: 5 }),
      'bulk_library_delete',
    );
  });

  it('audit line reports benchRefsSevered 0 when nothing was bench-referenced', async () => {
    mocks.mockFileGetById.mockImplementation((id: number) => ({ ...baseFile, id }));
    const res = await POST(makeRequest({ ids: [1, 2] }));
    expect(res.status).toBe(200);
    expect(mocks.mockLoggerInfo).toHaveBeenCalledWith(
      expect.objectContaining({ audit: 'bulk_library_delete', benchRefsSevered: 0 }),
      'bulk_library_delete',
    );
  });
