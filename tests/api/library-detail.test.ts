...
vi.mock('@/src/lib/db', () => ({
  ...
  // 24-04 F6: the route module now also exports DELETE which pulls getDb +
  // server-init into the shared module graph. Stub both so this GET-only test
  // resolves the wider import surface (mirrors library-retry.test.ts).
  getDb: () => ({}),
  default: {},
  shareRepo: () => ({ listAll: () => [] }),
}));
