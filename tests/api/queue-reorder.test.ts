...
describe('PATCH /api/queue/reorder', () => {
  ...
  it('test_PATCH_when_valid_request_then_emits_exactly_one_queue_updated_event', async () => {
    ...
    // 48-03: the active row carries a status so encodingJobs is provably wired
    // through rather than defaulting to 0 for a statusless fixture.
    mockListActive.mockReturnValue([{ id: 99, status: 'encoding' }]);
