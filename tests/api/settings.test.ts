...
const {
  ...
  return {
    ...
    mockSet,
    // 34-01 (AC-8): the route invalidates the detection cache ONLY when
    // gpu_device changes. Mock the barrel re-export so we can assert it without
    // loading the heavy real encode barrel in this route test.
    invalidateEncoderCacheMock: vi.fn(),
...
// 34-01: the route imports invalidateEncoderCache from the detection subpath
// (NOT the barrel — see route.ts). Mock that module so we can assert the call.
vi.mock('@/src/lib/encode/detection', () => ({
  invalidateEncoderCache: invalidateEncoderCacheMock,
}));
...
describe('PUT /api/settings', () => {
  beforeEach(() => {
    ...
    invalidateEncoderCacheMock.mockReset();
    mockGetAll.mockReturnValue({ ...seedDefaults });
  ...
  // ── 34-01: gpu_device validation (AC-5) + cache invalidation (AC-8) ──────────
  // gpu_device deliberately has NO server-side fieldErrors path (no nested-share /
  // forbidden). A bad shape exits at the zod safeParse layer as
  // 400 { error:'invalid_body', details:[...issues] } with an issue path∋gpu_device.
  it('test_PUT_when_gpu_device_empty_then_200_persists_as_auto', async () => {
    const res = await PUT(jsonReq({ settings: { gpu_device: '' } }));
    expect(res.status).toBe(200);
    expect(mockSet).toHaveBeenCalledWith('gpu_device', '');
  });

  it('test_PUT_when_gpu_device_valid_renderD_node_then_200_persisted', async () => {
    const res = await PUT(jsonReq({ settings: { gpu_device: '/dev/dri/renderD129' } }));
    expect(res.status).toBe(200);
    expect(mockSet).toHaveBeenCalledWith('gpu_device', '/dev/dri/renderD129');
  });

  it('test_PUT_when_gpu_device_arbitrary_path_then_400_invalid_body_path_gpu_device', async () => {
    const res = await PUT(jsonReq({ settings: { gpu_device: '/mnt/user/foo' } }));
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toBe('invalid_body');
    expect(
      body.details.some((i: { path: (string | number)[] }) => i.path.includes('gpu_device')),
    ).toBe(true);
    expect(mockSet).not.toHaveBeenCalledWith('gpu_device', expect.anything());
  });

  it('test_PUT_when_gpu_device_bare_basename_then_400_invalid_body', async () => {
    const res = await PUT(jsonReq({ settings: { gpu_device: 'renderD129' } }));
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toBe('invalid_body');
  });

  it('test_PUT_when_gpu_device_card_node_then_400_invalid_body', async () => {
    // card<N> = display node, NOT a render node → rejected by the renderD-only regex.
    const res = await PUT(jsonReq({ settings: { gpu_device: '/dev/dri/card0' } }));
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toBe('invalid_body');
  });

  it('test_PUT_when_gpu_device_changes_then_invalidates_encoder_cache', async () => {
    // pre-write value is unset → '' ; new value differs → invalidate (AC-8).
    const res = await PUT(jsonReq({ settings: { gpu_device: '/dev/dri/renderD129' } }));
    expect(res.status).toBe(200);
    expect(invalidateEncoderCacheMock).toHaveBeenCalledTimes(1);
  });

  it('test_PUT_when_gpu_device_unchanged_then_does_not_invalidate_cache', async () => {
    // current already renderD128 → no-op PUT of the SAME value → NO invalidate.
    mockGet.mockImplementation((k: string) =>
      k === 'gpu_device' ? '/dev/dri/renderD128' : seedDefaults[k as keyof typeof seedDefaults],
    );
    const res = await PUT(jsonReq({ settings: { gpu_device: '/dev/dri/renderD128' } }));
    expect(res.status).toBe(200);
    expect(invalidateEncoderCacheMock).not.toHaveBeenCalled();
  });

  it('test_PUT_when_gpu_device_absent_from_body_then_does_not_invalidate_cache', async () => {
    const res = await PUT(jsonReq({ settings: { language: 'de' } }));
    expect(res.status).toBe(200);
    expect(invalidateEncoderCacheMock).not.toHaveBeenCalled();
  });

  // 35-02: auto_crop + crop_override zod contract (AC-1 / AC-2 server-half / AC-8).
  it('test_PUT_when_auto_crop_true_then_200_persisted', async () => {
    const res = await PUT(jsonReq({ settings: { auto_crop: 'true' } }));
    expect(res.status).toBe(200);
    expect(mockSet).toHaveBeenCalledWith('auto_crop', 'true');
  });

  it('test_PUT_when_auto_crop_out_of_enum_then_400_invalid_body', async () => {
    const res = await PUT(jsonReq({ settings: { auto_crop: 'yes' } }));
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toBe('invalid_body');
    expect(mockSet).not.toHaveBeenCalledWith('auto_crop', expect.anything());
  });

  // 43-01: force_10bit zod contract (AC-6 server-half) — mirrors auto_crop.
  it('test_PUT_when_force_10bit_true_then_200_persisted', async () => {
    const res = await PUT(jsonReq({ settings: { force_10bit: 'true' } }));
    expect(res.status).toBe(200);
    expect(mockSet).toHaveBeenCalledWith('force_10bit', 'true');
  });

  it('test_PUT_when_force_10bit_false_then_200_persisted', async () => {
    const res = await PUT(jsonReq({ settings: { force_10bit: 'false' } }));
    expect(res.status).toBe(200);
    expect(mockSet).toHaveBeenCalledWith('force_10bit', 'false');
  });

  it('test_PUT_when_force_10bit_out_of_enum_then_400_invalid_body', async () => {
    const res = await PUT(jsonReq({ settings: { force_10bit: 'yes' } }));
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toBe('invalid_body');
    expect(mockSet).not.toHaveBeenCalledWith('force_10bit', expect.anything());
  });

  // 43-03: color_passthrough zod contract (AC-6 server-half) — mirrors force_10bit.
  it('test_PUT_when_color_passthrough_true_then_200_persisted', async () => {
    const res = await PUT(jsonReq({ settings: { color_passthrough: 'true' } }));
    expect(res.status).toBe(200);
    expect(mockSet).toHaveBeenCalledWith('color_passthrough', 'true');
  });

  it('test_PUT_when_color_passthrough_false_then_200_persisted', async () => {
    const res = await PUT(jsonReq({ settings: { color_passthrough: 'false' } }));
    expect(res.status).toBe(200);
    expect(mockSet).toHaveBeenCalledWith('color_passthrough', 'false');
  });

  it('test_PUT_when_color_passthrough_out_of_enum_then_400_invalid_body', async () => {
    const res = await PUT(jsonReq({ settings: { color_passthrough: 'yes' } }));
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toBe('invalid_body');
    expect(mockSet).not.toHaveBeenCalledWith('color_passthrough', expect.anything());
  });

  it('test_PUT_when_crop_override_valid_geometry_then_200_persisted', async () => {
    const res = await PUT(jsonReq({ settings: { crop_override: '1920:800:0:140' } }));
    expect(res.status).toBe(200);
    expect(mockSet).toHaveBeenCalledWith('crop_override', '1920:800:0:140');
  });

  it('test_PUT_when_crop_override_odd_dim_then_400_invalid_body', async () => {
    const res = await PUT(jsonReq({ settings: { crop_override: '1921:801:0:0' } }));
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toBe('invalid_body');
    expect(mockSet).not.toHaveBeenCalledWith('crop_override', expect.anything());
  });

  it('test_PUT_when_crop_override_malformed_then_400_invalid_body', async () => {
    const res = await PUT(jsonReq({ settings: { crop_override: 'abc' } }));
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error).toBe('invalid_body');
  });

  it('test_PUT_when_crop_override_empty_then_200_persists_as_auto', async () => {
    const res = await PUT(jsonReq({ settings: { crop_override: '' } }));
    expect(res.status).toBe(200);
    expect(mockSet).toHaveBeenCalledWith('crop_override', '');
  });

  // audit SR-1: whitespace-only MUST be accepted by the trim-tolerant refine and
  // empty-trimmed to '' (the naive `v === ''` refine would 400 here).
  it('test_PUT_when_crop_override_whitespace_then_200_persists_as_empty', async () => {
    const res = await PUT(jsonReq({ settings: { crop_override: '   ' } }));
    expect(res.status).toBe(200);
    expect(mockSet).toHaveBeenCalledWith('crop_override', '');
  });

  it('test_PUT_when_crop_override_with_crop_prefix_then_200_persisted', async () => {
    const res = await PUT(jsonReq({ settings: { crop_override: 'crop=1920:800:0:140' } }));
    expect(res.status).toBe(200);
    expect(mockSet).toHaveBeenCalledWith('crop_override', 'crop=1920:800:0:140');
  });
