...
// ISS-002: the route resolves through the 24-03 read-surface memo (10s TTL,
// module-global). Reset it per test or the first tmpdir leaks into the rest.
import { __resetCachePathMemo } from '@/src/lib/encode/cache-path';
...
describe('GET /api/logs/[jobId] — validation', () => {
  ...
  // ISS-002 (2026-09-06) — the test that used to live here asserted the BUG:
  // "returns 404 log_not_found when cache_pool_path empty". An unset setting is
  // the DEFAULT since 24-03 and the state of every upgrader after the 36-03
  // migration, so that assertion froze a 404 for nearly every install while the
  // encoder wrote the log to the auto-resolved root. Its replacement — an unset
  // setting must RESOLVE and serve the file — needs the accessor stubbed and
  // therefore lives in its own file: tests/api/logs-jobid-unset-cache-path.test.ts.
});
