...
// 48-02 Task 1 (Bundle A) — forbidden-prefix guard on share CREATE.
// ACs: AC-1 (reject + fieldErrors.path + no row), AC-3 (legit paths + segment-
// exact), AC-4b aliasing surface on the shares door.
describe('POST /api/shares — 48-02 forbidden-prefix guard', () => {
  const FORBIDDEN = ['/', '/sys', '/proc/x', '/dev', '/etc', '/boot', '/run', '//sys', '/sys/'];

  for (const bad of FORBIDDEN) {
    it(`test_post_when_path_is_${bad.replace(/[^a-z]/gi, '_')}_then_400_path_forbidden_prefix`, async () => {
      const before = shareRepo().listAll().length;
      const res = await POST(
        new Request('http://test/api/shares', jsonPostInit(sampleCreate({ path: bad }))),
      );
      expect(res.status).toBe(400);
      const body = (await res.json()) as { error: string; fieldErrors: Record<string, string> };
      expect(body.error).toBe('validation_failed');
      expect(body.fieldErrors.path).toBe('path_forbidden_prefix');
      expect(shareRepo().listAll()).toHaveLength(before);
    });
  }

  it('test_post_when_path_traverses_into_sys_then_400_path_traversal_rejected', async () => {
    // '..' is caught by the OLDER refine, which runs first — the aliasing hole
    // is closed either way, only the reported code differs.
    const res = await POST(
      new Request('http://test/api/shares', jsonPostInit(sampleCreate({ path: '/mnt/../sys' }))),
    );
    expect(res.status).toBe(400);
    const body = (await res.json()) as { fieldErrors: Record<string, string> };
    expect(body.fieldErrors.path).toBe('path_traversal_rejected');
  ...
  // AC-3 — a naive startsWith would reject '/etcetera' and '/development'.
  it('test_post_when_path_only_looks_like_a_system_prefix_then_201', async () => {
    for (const good of ['/mnt/user/Media', '/media', '/mnt/cache/x', '/etcetera', '/development']) {
      const res = await POST(
        new Request(
          'http://test/api/shares',
          jsonPostInit(sampleCreate({ name: `S${good.replace(/\W/g, '')}`, path: good })),
        ),
      );
      expect(res.status, `${good} should be accepted`).toBe(201);
    }
  });
});
