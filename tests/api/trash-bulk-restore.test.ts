...
describe('POST /api/trash/bulk-restore', () => {
  ...
  // 28-03 (P10 MH-2): moveAcrossFilesystems is now async — a REJECTED promise
  // (the realistic EXDEV-path failure) must still be caught by the caller's
  // try/catch and mapped to fs_orphan, NOT escape as an unhandled rejection.
  it('post-commit moveAcrossFilesystems ASYNC rejection → fs_orphan (await catches it)', async () => {
    mocks.mockTrashFindById.mockReturnValue({ ...baseEntry });
    const exdev = Object.assign(new Error('EXDEV cross-device link'), { code: 'EXDEV' });
    mocks.mockMoveAcrossFs.mockRejectedValueOnce(exdev);
    const res = await POST(makeRequest({ ids: [1] }));
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.successCount).toBe(0);
