...
const {
  ...
  mockShareListAll,
  mockLogWarn,
  // 50-04: batched file-pinned blocklist lookup behind the new
  // `blocklistEntryIds` payload field.
  mockFindByFileIds,
} = vi.hoisted(() => ({
...
vi.mock('@/src/lib/db', () => ({
  ...
  blocklistRepo: () => ({
    findByFileIds: mockFindByFileIds,
  }),
  default: {},
}));
...
// 50-04 AC-7 / AC-8: the additive `blocklistEntryIds` payload field.
// The Library row only knows file.id; the unblock action needs the
// blocklist_entry.id. It travels BESIDE `rows`, never inside them — FileRow and
// the frozen `SELECT * FROM file` in listPaginated stay untouched.
describe('GET /api/library — blocklistEntryIds (50-04)', () => {
  beforeEach(() => {
    mockFindByFileIds.mockReset();
    mockFindByFileIds.mockReturnValue([]);
  });

  it('test_GET_when_blocklisted_row_has_pinned_entry_then_payload_maps_fileId_to_entryId', async () => {
    mockListPaginated.mockReturnValue({
      rows: [
        { ...sampleRow, id: 5, status: 'blocklisted' },
        { ...sampleRow, id: 6, status: 'pending' },
      ],
      total: 2,
    });
    mockCountByStatus.mockReturnValue(emptyCounts);
    ...
    expect(body.blocklistEntryIds).toEqual({ '5': 91 });
    // rows keep their exact shape — no injected column.
    expect(Object.keys(body.rows[0])).toEqual(Object.keys(sampleRow));
  });

  it('test_GET_when_blocklisted_row_has_no_pinned_entry_then_absent_from_map_not_null', async () => {
    mockListPaginated.mockReturnValue({
      rows: [{ ...sampleRow, id: 5, status: 'blocklisted' }],
      total: 1,
    });
    mockCountByStatus.mockReturnValue(emptyCounts);
    mockCountOrphaned.mockReturnValue(0);
    ...
    expect(body.blocklistEntryIds).toEqual({});
    expect('5' in body.blocklistEntryIds).toBe(false);
  });

  // AC-8: no blocklisted row on the page ⇒ no query at all.
  it('test_GET_when_no_blocklisted_row_then_findByFileIds_never_called', async () => {
    mockListPaginated.mockReturnValue({
      rows: [{ ...sampleRow, id: 5, status: 'pending' }],
      total: 1,
    });
    mockCountByStatus.mockReturnValue(emptyCounts);
    mockCountOrphaned.mockReturnValue(0);
