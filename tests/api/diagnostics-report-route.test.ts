...
const { mockAssemble, mockRender, mockEnsureServerInit, authMode } = vi.hoisted(() => ({
  mockAssemble: vi.fn<() => Promise<DiagnosticsPayload>>(),
  mockRender: vi.fn<(p: DiagnosticsPayload) => string>(),
...
vi.mock('@/src/lib/diagnostics/markdown-template', () => ({
  renderDiagnosticsMarkdown: mockRender,
}));
...
import { GET, runtime } from '@/app/api/diagnostics-report/route';
...
describe('GET /api/diagnostics-report', () => {
  beforeEach(() => {
    ...
    mockRender.mockReturnValue('## x265-butler diagnostics report\n');
  });
  ...
  it('auth_enabled=false + no cookie → 200 + text/markdown', async () => {
    const res = await GET(new Request('http://test/api/diagnostics-report'));
    expect(res.status).toBe(200);
    expect(res.headers.get('content-type')).toMatch(/text\/markdown/);
    const body = await res.text();
    expect(body).toContain('x265-butler diagnostics report');
  });
