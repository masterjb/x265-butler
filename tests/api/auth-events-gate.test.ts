...
describe('GET /api/events — static-analysis grep gate (audit S3)', () => {
  it('test_source_has_auth_gate_before_new_ReadableStream', () => {
    const src = fs.readFileSync(
      path.join(process.cwd(), 'app', 'api', 'events', 'route.ts'),
      'utf8',
    );
    // 28-03 (L6): the inline requireAuth+authGuard idiom is now the shared
    // gateAuth() helper — the auth-before-stream-allocation invariant (audit S3)
    // is unchanged, only the gate call-site moved to gateAuth(.
    const idxAuth = src.indexOf('gateAuth(');
