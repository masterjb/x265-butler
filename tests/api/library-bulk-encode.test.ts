...
describe('POST /api/library/bulk-encode', () => {
  ...
  it('AC-1 happy path — 5 eligible IDs all enqueue → 200 successCount=5, one emit', async () => {
    ...
    // 48-03: objectContaining would silently pass without the new field, so the
    // encoding-only count is asserted explicitly here.
    expect(mocks.mockEngineEmit).toHaveBeenCalledWith(
      expect.objectContaining({ type: 'queue.updated', paused: false, encodingJobs: 0 }),
    );
