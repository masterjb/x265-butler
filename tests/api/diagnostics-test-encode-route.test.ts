...
const {
  ...
  // 50-06: the settings the mocked settingRepo hands back. Empty = every
  // resolver falls to its factory default, i.e. the pre-50-06 argv.
  mockSettings: { value: {} as Record<string, string> },
  mockSpawn: vi.fn(),
  mockDetectEncoders: vi.fn<() => Promise<DetectionResult>>(),
  mockEnsureServerInit: vi.fn(),
  mockLoggerInfo: vi.fn(),
  // 49-03: default fail-OPEN, exactly like the real accessor on a cold cache.
...
vi.mock('@/src/lib/encode', async () => {
  ...
  return {
    ...
    // 50-06: the fail-open path resolves factory CRFs from this table.
    DEFAULT_CRF_BY_ENCODER: profiles.DEFAULT_CRF_BY_ENCODER,
    // 29-01: test-encode.ts now also imports PROBE_FRAME_SIZE from the barrel for
    // the testsrc size — pull the REAL value so the spawned argv stays faithful.
    PROBE_FRAME_SIZE: profiles.PROBE_FRAME_SIZE,
    // 49-03: the synthetic pixel-format pin + its encoder predicate — REAL values
    // again, so the spawned argv carries the real `-pix_fmt nv12` for qsv/nvenc.
    SYNTHETIC_PROBE_PIX_FMT: profiles.SYNTHETIC_PROBE_PIX_FMT,
    usesSyntheticPixFmtPin: profiles.usesSyntheticPixFmtPin,
    // 49-03 (D7/AC-13): the forced-IDR verdict accessor. Stubbed onto the mocked
    // detection surface — the tests below drive it to prove runTestEncode resolves
    // the SAME value buildArgs resolves.
    isForcedIdrSupported: mockIsForcedIdrSupported,
  };
});

// 50-06: runTestEncode now reads `settingRepo().getAll()` once. This file drives
// the ROUTE, and the route deliberately never injects the settingsReader seam
// (that seam is test-only, and a production caller handing in settings would be
// a bypass around the single read). So the DB is mocked HERE — without it the
// first call would open a real SQLite file and run migrations inside a unit test.
vi.mock('@/src/lib/db', () => ({
  settingRepo: () => ({ getAll: () => mockSettings.value }),
}));

vi.mock('@/src/lib/encode/ffmpeg-binary', () => ({
  ffmpegBinary: () => '/usr/bin/ffmpeg',
  ffprobeBinary: () => '/usr/bin/ffprobe',
  // 45-01: runTestEncode now resolves its binary via the encoder-aware selector.
...
// 28-03: gateAuth adds an async hop before the handler subscribes to the spawn
// 'close' event — flush generously (not a fixed 2 ticks) so the emit is never
// lost to a microtask-timing race.
...
describe('POST /api/diagnostics/test-encode', () => {
  beforeEach(() => {
    ...
    // 49-03: back to the fail-OPEN default before every spec.
    mockIsForcedIdrSupported.mockReset();
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // 49-03 AC-13 (D7): the test encode MIRRORS production. Without this, a host
  // whose confirm spawn rejected the option would have /diagnostics validating an
  // argv the production path no longer emits — the diagnosis contradicting
  // production again, one layer up from the bug this plan fixes.
  // ───────────────────────────────────────────────────────────────────────────
  describe('49-03 AC-13: the spawned argv mirrors the production resolution', () => {
    function qsvHost() {
      mockDetectEncoders.mockResolvedValue({
        detected: ['qsv', 'libx265'],
        activeFromAuto: 'qsv',
        ...
        brokenExcerpts: {},
        forcedIdrSupported: { qsv: false },
        probeEncodeDisabled: false,
      });
    }

    async function spawnedArgv(): Promise<string[]> {
      const child = makeMockChild();
      mockSpawn.mockReturnValue(child);
      const pending = POST(
        new Request('http://test/api/diagnostics/test-encode', { method: 'POST' }),
    ...
    it('verdict TRUE ⇒ the test encode carries the IDR token', async () => {
      qsvHost();
      mockIsForcedIdrSupported.mockReturnValue(true);
      const argv = await spawnedArgv();
      expect(argv).toContain('-forced_idr');
      // …and the G3 pixel-format pin rides along
      expect(argv).toContain('-pix_fmt');
      expect(argv[argv.indexOf('-pix_fmt') + 1]).toBe('nv12');
    });

    it('verdict FALSE ⇒ the test encode drops the IDR token, exactly like production', async () => {
      qsvHost();
      mockIsForcedIdrSupported.mockReturnValue(false);
      const argv = await spawnedArgv();
      expect(argv).not.toContain('-forced_idr');
      // the pin is INDEPENDENT of the verdict and must survive
      expect(argv[argv.indexOf('-pix_fmt') + 1]).toBe('nv12');
    });

    it('the verdict is read for the ENCODER THAT WAS PICKED', async () => {
      qsvHost();
      await spawnedArgv();
      expect(mockIsForcedIdrSupported).toHaveBeenCalledWith('qsv');
    });
  });
});
