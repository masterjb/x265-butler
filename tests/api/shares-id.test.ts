...
// 48-02 Task 1 (Bundle A) — AC-2: `shareUpdateSchema` reuses
// `pathSchema.optional()`, so PATCH inherits the guard with no second edit.
describe('PATCH /api/shares/[id] — 48-02 forbidden-prefix guard', () => {
  it('test_patch_when_path_is_sys_kernel_then_400_and_stored_path_unchanged', async () => {
    const { id } = createShare();
    const res = await PATCH(
      new Request(`http://test/api/shares/${id}`, jsonPatchInit({ path: '/sys/kernel' })),
      asyncParams(id),
    );
    ...
    expect(shareRepo().getById(id)?.path).toBe('/media/movies');
  });

  it('test_patch_when_path_is_aliased_root_then_400_and_stored_path_unchanged', async () => {
    const { id } = createShare();
    for (const bad of ['/', '//', '//sys', '/run/']) {
      const res = await PATCH(
        new Request(`http://test/api/shares/${id}`, jsonPatchInit({ path: bad })),
        asyncParams(id),
      );
  ...
  it('test_patch_when_path_only_looks_like_a_system_prefix_then_200', async () => {
    const { id } = createShare();
    const res = await PATCH(
      new Request(`http://test/api/shares/${id}`, jsonPatchInit({ path: '/etcetera/media' })),
      asyncParams(id),
    );
    expect(res.status).toBe(200);
    expect(shareRepo().getById(id)?.path).toBe('/etcetera/media');
  });
});
