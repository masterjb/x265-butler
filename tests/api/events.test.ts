...
describe('GET /api/events (SSE)', () => {
  ...
  it('test_GET_when_opened_then_first_frame_is_queue_updated_with_baseline', async () => {
    // 48-03 AC-17: four active rows, one of them encoding — the first frame every
    // freshly connected tab reads must carry the encoding-only count.
    mockListActive.mockReturnValue([
      { id: 1, status: 'encoding' } as JobRow,
      { id: 2, status: 'queued' } as JobRow,
      { id: 3, status: 'queued' } as JobRow,
    ]);
    ...
    expect(buffer).toMatch(/"activeJobs":3/);
    expect(buffer).toMatch(/"pendingJobs":7/);
    // AC-17: numeric encodingJobs present in the bootstrap frame …
    expect(buffer).toMatch(/"encodingJobs":1/);
    // … and `paused` still deliberately absent (Omit<> in the route annotation).
    expect(buffer).not.toMatch(/"paused"/);
    // AC-3: the rows are reused for the progress replay — listActive() is not
    // called a second time in this request path.
