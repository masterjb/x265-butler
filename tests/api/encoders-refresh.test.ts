...
describe('POST /api/encoders/refresh', () => {
  ...
  // 49-05 AC-5: the Settings form rebuilds its whole EncoderDetectionState out
  // of THIS body (settings-form.tsx:415-419). A tier missing here would silently
  // revert the QSV CRF helper to its "tier not verified" wording after every
  // encoder-tab save — a refresh that re-probed the tier and then threw it away.
  it.each(['icq-full', 'cqp'] as const)(
    'test_POST_when_tier_%s_then_body_carries_qsvRateControl',
    async (rc) => {
      mockDetectEncoders.mockResolvedValue({
      ...
      const res = await POST(new Request('http://test', { method: 'POST' }));
      const body = await res.json();
      expect(body.qsvRateControl).toBe(rc);
    },
  );

  it('test_POST_when_tier_unresolved_then_body_omits_qsvRateControl', async () => {
    const res = await POST(new Request('http://test', { method: 'POST' }));
    const body = await res.json();
    // undefined, not a manufactured default — the client then renders the
    // "tier not verified, the -global_quality fallback applies" helper.
    expect(body.qsvRateControl).toBeUndefined();
  });
});
