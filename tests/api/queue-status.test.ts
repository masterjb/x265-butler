...
describe('GET /api/queue/status', () => {
  ...
  it('test_GET_status_when_default_then_200_with_paused_false_and_counts', async () => {
    mockListActive.mockReturnValue([{ id: 1, status: 'queued' } as JobRow]);
  ...
  it('test_GET_status_when_called_then_response_includes_encodingJobs_count', async () => {
    // 48-03 (AC-14): encodingJobs no longer comes from countByStatus('encoding')
    // but from a filter over the listActive() rows, so the rows carry a status.
    mockListActive.mockReturnValue([
    ...
    // AC-14: three repo calls became two.
    expect(mockListActive).toHaveBeenCalledTimes(1);
    expect(mockCountByStatus).toHaveBeenCalledTimes(1);
  });
  ...
  it('test_GET_status_when_encoding_present_then_hasEncodingJob_true', async () => {
    // 48-03: hasEncodingJob is derived from the row-based encodingJobs now.
    mockListActive.mockReturnValue([{ id: 1, status: 'encoding' } as JobRow]);
