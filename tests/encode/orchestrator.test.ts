...
function setupOrchestrator({
  ...
  // 26-01 (F3, M1): inject a W_OK pre-flight stub to simulate a read-only source mount.
  accessSyncImpl?: typeof fs.accessSync;
  ...
  __forTests_setDeps({
    ...
    fs: {
      ...
      // 26-01 (F3, M1): accessSyncImpl override simulates a read-only source mount.
      accessSync: accessSyncImpl ?? fs.accessSync,
...
// 26-01 (F3): sidecar location mode at the commit step — integration over the
// real-fs harness. beside (default) lands beside the output; off suppresses;
// central writes a mirrored tree under sidecar_central_path; central is decoupled
// from the source-mount W_OK pre-flight (M1/AC-7); the active mode is logged (S4/AC-9).
describe('orchestrator — sidecar mode (26-01 F3)', () => {
  function besidePath(): string {
    return path.join(mediaRoot, 'movie-x265.mkv.x265-butler.json');
  }

  it('default (no setting) → beside: sidecar lands beside the output (AC-1)', async () => {
    ...
    expect(fs.existsSync(besidePath())).toBe(true);
    const t = logSpy.mock.calls.find(
      (c) => c[0]?.action === 'job_transition' && c[0]?.transition === 'encoding→done',
    );
    expect(t?.[0].sidecarMode).toBe('beside'); // S4/AC-9
  });

  it('off → NO sidecar written anywhere, encode still commits (AC-2)', async () => {
    const { fileId } = setupOrchestrator({ sourceSize: 1_000_000, outputSize: 600_000 });
    settingRepo.set('sidecar_mode', 'off');
    await loopOnce();
    expect(fileRepo.getById(fileId)?.status).toBe('done-smaller'); // still commits
    expect(fs.existsSync(besidePath())).toBe(false);
    const t = logSpy.mock.calls.find(
      (c) => c[0]?.action === 'job_transition' && c[0]?.transition === 'encoding→done',
    );
    expect(t?.[0].sidecarMode).toBe('off'); // S4/AC-9 — distinguishes from silent-fail
  });

  it('central → mirrored tree under sidecar_central_path, NOT beside (AC-3)', async () => {
    const centralRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'x265-central-'));
    try {
      const { fileId } = setupOrchestrator({ sourceSize: 1_000_000, outputSize: 600_000 });
      ...
      expect(fileRepo.getById(fileId)?.status).toBe('done-smaller');
      const finalOut = path.join(mediaRoot, 'movie-x265.mkv');
      const centralExpected = `${path.join(centralRoot, finalOut.replace(/^\/+/, ''))}.x265-butler.json`;
      expect(fs.existsSync(centralExpected)).toBe(true); // mirrored tree
      expect(fs.existsSync(besidePath())).toBe(false); // NOT beside
    } finally {
      fs.rmSync(centralRoot, { recursive: true, force: true });
    }
  });

  it('central decoupled from a read-only source mount (M1/AC-7)', async () => {
    const centralRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'x265-central-'));
    try {
      const { fileId, sourcePath } = setupOrchestrator({
        sourceSize: 1_000_000,
        outputSize: 1_200_000, // done-larger → discard branch (source-path sidecar)
        // Simulate read-only source parent: W_OK probe throws EACCES.
        accessSyncImpl: (() => {
      ...
      settingRepo.set('sidecar_central_path', centralRoot);
      await loopOnce();
      expect(fileRepo.getById(fileId)?.status).toBe('done-larger');
      // The central write targets /config-style root, NOT the read-only source
      // parent → it must still land despite the failing source-parent W_OK probe.
      const centralExpected = `${path.join(centralRoot, sourcePath.replace(/^\/+/, ''))}.x265-butler.json`;
      expect(fs.existsSync(centralExpected)).toBe(true);
      // And the source-parent-not-writable soft-degrade warn must NOT fire (that
      // gate is beside-only now).
      const notWritableWarn = warnSpy.mock.calls.find(
        (c) => c[0]?.action === 'sidecar_source_path_not_writable',
      );
      expect(notWritableWarn).toBeUndefined();
    } finally {
      fs.rmSync(centralRoot, { recursive: true, force: true });
    }
  });
...
// 36-01: source-keyed central sidecar on the done-smaller commit. WRITE-side
// mirror of the 33-01 read resolver. central + effectiveMode==='suffix' +
// source-recoverable writes a SECOND central sidecar keyed by the SOURCE path
// (in addition to the output-keyed one) so a source resurfacing at its original
// absolute path under a DB-not-authoritative scenario is skippable. Excluded:
// replace (AC-3, output-mode harness), beside/off (AC-4), suffix hard-delete (AC-7).
describe('orchestrator — source-keyed central sidecar on done-smaller (36-01)', () => {
  const SOURCE_NAME = 'movie.mp4';
  const OUTPUT_NAME = 'movie-x265.mkv';

  function sourceCentral(centralRoot: string): string {
    const src = path.join(mediaRoot, SOURCE_NAME);
    return `${path.join(centralRoot, src.replace(/^\/+/, ''))}.x265-butler.json`;
  }
  function outputCentral(centralRoot: string): string {
    const out = path.join(mediaRoot, OUTPUT_NAME);
    return `${path.join(centralRoot, out.replace(/^\/+/, ''))}.x265-butler.json`;
  }
  function sourceKeyedWrittenLog() {
    return logSpy.mock.calls.find((c) => c[0]?.action === 'source_sidecar_written');
  }

  // AC-1 + AC-6: central + suffix + done-smaller → BOTH the output-keyed AND the
  // source-keyed central sidecar land on disk, plus the source_sidecar_written
  // breadcrumb naming the source path.
  it('central + suffix → writes BOTH output- and source-keyed central sidecars + breadcrumb (AC-1/AC-6)', async () => {
    const centralRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'x265-central-'));
    try {
      const { fileId, sourcePath } = setupOrchestrator({
        sourceSize: 1_000_000,
      ...
      expect(fileRepo.getById(fileId)?.status).toBe('done-smaller');
      // Output-keyed central sidecar (the existing :2155 write) still lands.
      expect(fs.existsSync(outputCentral(centralRoot))).toBe(true);
      // 36-01: the NEW source-keyed central sidecar also lands.
      expect(fs.existsSync(sourceCentral(centralRoot))).toBe(true);

      const log = sourceKeyedWrittenLog();
      expect(log).toBeTruthy();
      expect(log?.[0].sourcePath).toBe(sourcePath);
      expect(log?.[0].fileId).toBe(fileId);
    } finally {
      fs.rmSync(centralRoot, { recursive: true, force: true });
    }
  });

  // AC-4: beside done-smaller → NO source-keyed sidecar beside the source, no breadcrumb.
  it('beside → no source-keyed sidecar, no breadcrumb (AC-4)', async () => {
    const { fileId } = setupOrchestrator({ sourceSize: 1_000_000, outputSize: 600_000 });
    settingRepo.set('sidecar_mode', 'beside');
    await loopOnce();
    expect(fileRepo.getById(fileId)?.status).toBe('done-smaller');
    // A source-keyed beside sidecar would sit next to the (trashed) source.
    expect(fs.existsSync(path.join(mediaRoot, `${SOURCE_NAME}.x265-butler.json`))).toBe(false);
    expect(sourceKeyedWrittenLog()).toBeUndefined();
  });

  // AC-4: off done-smaller → no source-keyed write anywhere, no breadcrumb.
  it('off → no source-keyed sidecar, no breadcrumb (AC-4)', async () => {
    const { fileId } = setupOrchestrator({ sourceSize: 1_000_000, outputSize: 600_000 });
    settingRepo.set('sidecar_mode', 'off');
    await loopOnce();
    expect(fileRepo.getById(fileId)?.status).toBe('done-smaller');
  ...
  // AC-7 (SR-1): central + suffix + delete_original_after_encode=true (no hardlink
  // fallback) → source permanently unlinked, NO trash row → a source-keyed sidecar
  // would orphan-for-life. Output-keyed write still lands; source-keyed suppressed.
  it('suffix hard-delete (delete_original_after_encode) → NO source-keyed sidecar, no breadcrumb (AC-7)', async () => {
    const centralRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'x265-central-'));
    try {
      const { fileId } = setupOrchestrator({ sourceSize: 1_000_000, outputSize: 600_000 });
      settingRepo.set('sidecar_mode', 'central');
      settingRepo.set('sidecar_central_path', centralRoot);
      settingRepo.set('delete_original_after_encode', 'true');
      await loopOnce();

      expect(fileRepo.getById(fileId)?.status).toBe('done-smaller');
      // Output-keyed central sidecar still lands (protects the -x265 output).
      expect(fs.existsSync(outputCentral(centralRoot))).toBe(true);
      // Source-keyed central sidecar is SUPPRESSED (orphan-for-life avoidance).
      expect(fs.existsSync(sourceCentral(centralRoot))).toBe(false);
      expect(sourceKeyedWrittenLog()).toBeUndefined();
    } finally {
      fs.rmSync(centralRoot, { recursive: true, force: true });
    }
  ...
  // AC-5 (MH-1): soft-degrade through the REAL writeSidecarResolved. Point the
  // central root UNDER a regular file → the internal recursive mkdir fails with
  // ENOTDIR → writeSidecarResolved warn-degrades (never throws) → the encode still
  // commits (done-smaller + job done). Proven via a real fs-level failure, NOT a
  // rejecting mock (a rejecting mock would propagate → failJob → AC unsatisfiable).
  it('source-keyed write soft-degrades on a real fs failure — commit still completes (AC-5)', async () => {
    // A regular file inside the (fresh, auto-cleaned) stageRoot becomes the bogus
    // central-root parent → mkdir(recursive) under it fails ENOTDIR.
    const blocker = path.join(stageRoot, 'central-blocker-file');
    fs.writeFileSync(blocker, 'not a directory');
    const { fileId, jobId } = setupOrchestrator({ sourceSize: 1_000_000, outputSize: 600_000 });
    settingRepo.set('sidecar_mode', 'central');
    // central root is UNDER a regular file → mkdir(recursive) → ENOTDIR.
    settingRepo.set('sidecar_central_path', path.join(blocker, 'sidecars'));
    await loopOnce();

    // Commit unaffected: status committed + job completed despite the write failure.
    // (If writeSidecarResolved had THROWN, the top-level catch would route to failJob
    // → status 'failed', job 'failed'. done-smaller + done proves the soft-degrade.)
    expect(fileRepo.getById(fileId)?.status).toBe('done-smaller');
    const job = db.prepare('SELECT status FROM job WHERE id = ?').get(jobId) as { status: string };
    expect(job.status).toBe('done');
    // The fs failure was real: the recursive mkdir under a regular file never
    // succeeded, so no central sidecar tree was created (write truly hit ENOTDIR).
    // (The warn itself is emitted by writeSidecarResolved's OWN module logger, not
    // deps.logger, so it is not observable on warnSpy in this harness — the commit-
    // completes invariant above is the load-bearing AC-5 proof.)
    expect(fs.existsSync(path.join(blocker, 'sidecars'))).toBe(false);
  });
});
...
describe('orchestrator — sidecar fan-out + M1 ORDER + M2 soft-degrade (05-13)', () => {
  ...
  // 28-09 AC-8 characterization (sentinel-integrity): pin the audit-M1 ORDER
  // (writeSidecar → setStatus → markCompleted → log+SSE) on the done-smaller
  // commit path. The L1 refactor moves the commit block into commitEncodeResult
  // as a cohesive unit; this RED→GREEN-on-current-code test guards against a
  // re-indent silently reordering the data-loss-sensitive commit sequence — the
  // one AC-3 invariant not pinned by the existing presence-only assertions.
  it('test_loopOnce_done_smaller_preserves_audit_M1_ORDER_sidecar_setStatus_markCompleted_emit', async () => {
    const order: string[] = [];
    const writeSidecarMock = vi.fn(async () => {
      order.push('writeSidecar');
    });
    const hashFileMock = vi.fn().mockResolvedValue('out_hash_order');
    const { fileId } = setupOrchestrator({ sourceSize: 1_000_000, outputSize: 600_000 });

    const realSetStatus = fileRepo.setStatus.bind(fileRepo);
    const setStatusSpy = vi.fn((id: number, status: string, v: number) => {
      if (status === 'done-smaller') order.push('setStatus');
      return realSetStatus(id, status as never, v);
    });
    const orderedFileRepo = { ...fileRepo, setStatus: setStatusSpy as never } as typeof fileRepo;

    const realMarkCompleted = jobRepo.markCompleted.bind(jobRepo);
    const markCompletedSpy = vi.fn((id: number, m: never) => {
      order.push('markCompleted');
      return realMarkCompleted(id, m);
    });
    const orderedJobRepo = {
      ...jobRepo,
      markCompleted: markCompletedSpy as never,
    } as typeof jobRepo;

    const eventsEmitSpy = vi.fn((ev: { type: string }) => {
      if (ev.type === 'job.completed') order.push('emit');
    });

    __forTests_setDeps({
      writeSidecar: writeSidecarMock as never,
      hashFile: hashFileMock as never,
      fileRepo: () => orderedFileRepo,
      jobRepo: () => orderedJobRepo,
      events: { emit: eventsEmitSpy } as never,
    });
    await loopOnce();

    expect(fileRepo.getById(fileId)?.status).toBe('done-smaller');
    // audit-M1 ORDER: writeSidecar FIRST, setStatus SECOND, markCompleted THIRD, emit FOURTH.
    expect(order).toEqual(['writeSidecar', 'setStatus', 'markCompleted', 'emit']);
  });
    ...
    const { jobId, fileId } = setupOrchestrator({
      ...
      ffprobeImpl: async () => ({
        ...
        streams: [
          ...
          { attachedPic: false, index: 1, codec_type: 'audio', codec_name: 'truehd' },
        ],
      }),
    });
    ...
    const { jobId } = setupOrchestrator({
      ffprobeImpl: async () => ({
        ...
        streams: [
          ...
          { attachedPic: false, index: 2, codec_type: 'audio', codec_name: 'truehd' },
        ],
      }),
    });
