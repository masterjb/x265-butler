...
const aacWithSubripProbe = (): ProbeResult =>
  probeWithStreams([
    { attachedPic: false, index: 0, codec_type: 'video', codec_name: 'h264' },
    { attachedPic: false, index: 1, codec_type: 'audio', codec_name: 'aac' },
    { attachedPic: false, index: 2, codec_type: 'subtitle', codec_name: 'subrip' },
    { attachedPic: false, index: 3, codec_type: 'subtitle', codec_name: 'subrip' },
  ]);

// 41-01: an iPhone-style source carrying a DATA stream (mebx → data:none).
const aacWithDataStreamProbe = (): ProbeResult =>
  probeWithStreams([
    { attachedPic: false, index: 0, codec_type: 'video', codec_name: 'h264' },
    { attachedPic: false, index: 1, codec_type: 'audio', codec_name: 'aac' },
    { attachedPic: false, index: 2, codec_type: 'data', codec_name: 'none' },
  ]);

// 41-01 MH-1: a clean A/V/S + attachment (font) source — nothing dropped.
const avsAttachmentProbe = (): ProbeResult =>
  probeWithStreams([
    { attachedPic: false, index: 0, codec_type: 'video', codec_name: 'h264' },
    { attachedPic: false, index: 1, codec_type: 'audio', codec_name: 'aac' },
    { attachedPic: false, index: 2, codec_type: 'subtitle', codec_name: 'ass' },
    { attachedPic: false, index: 3, codec_type: 'attachment', codec_name: 'ttf' },
  ]);

// 49-01: a source carrying embedded cover art. The cover is the LAST stream
// (source index 2) but VIDEO ordinal 1 — the shape from the forum report.
const aacWithCoverProbe = (): ProbeResult =>
  probeWithStreams([
    { attachedPic: false, index: 0, codec_type: 'video', codec_name: 'h264' },
    { attachedPic: false, index: 1, codec_type: 'audio', codec_name: 'aac' },
    { attachedPic: true, index: 2, codec_type: 'video', codec_name: 'mjpeg' },
  ]);

// 49-01 (AC-15): mp4-resolved source that falls back to MKV (truehd audio) AND
// carries a cover — the ordinals must survive the container switch.
const truehdWithCoverProbe = (): ProbeResult =>
  probeWithStreams([
    { attachedPic: false, index: 0, codec_type: 'video', codec_name: 'h264' },
    { attachedPic: false, index: 1, codec_type: 'audio', codec_name: 'truehd' },
    { attachedPic: true, index: 2, codec_type: 'video', codec_name: 'mjpeg' },
...
// 41-01 AC-9: mp4-resolved source that falls back to MKV (truehd audio) AND
// carries a data stream — the warn must still fire on the fallback path.
const truehdWithDataStreamProbe = (): ProbeResult =>
  probeWithStreams([
    { attachedPic: false, index: 0, codec_type: 'video', codec_name: 'h264' },
    { attachedPic: false, index: 1, codec_type: 'audio', codec_name: 'truehd' },
    { attachedPic: false, index: 2, codec_type: 'data', codec_name: 'none' },
...
describe('orchestrator — 05-15 match-source dispatch (AC-3 .. AC-6 + AC-12)', () => {
  // 41-01: the MKV path now runs ONE source ffprobe (incompatible-stream
  // analysis for the warn). A clean A/V source → count 0, no warn fired.
  it('test_match_source_when_clean_mkv_source_then_one_source_probe_no_incompatible_warn', async () => {
    ...
    // 41-01: EXACTLY ONE source ffprobe runs (the new MKV analysis probe).
    ...
    expect(sourceProbeCalls).toHaveLength(1);
    // Clean A/V source → no incompatible streams → count falsy + NO warn.
    expect(runEncodeSpy.mock.calls[0]?.[0]?.droppedIncompatibleStreamCount).toBeFalsy();
  ...
  // 41-01 AC-1/AC-4: MKV source WITH a data stream → count>0 threaded into
  // runEncode opts (buildArgs emits the warn).
  it('test_match_source_when_mkv_source_has_data_stream_then_count_threaded', async () => {
    const ffprobeMock: (p: string) => Promise<ProbeResult | null> = vi.fn(async () =>
      aacWithDataStreamProbe(),
    );
    const { sourcePath } = await setupDispatch({
      setting: 'match-source',
      filename: 'movie.mkv',
    ...
    expect(runEncodeSpy).toHaveBeenCalledTimes(1);
    const opts = runEncodeSpy.mock.calls[0]?.[0];
    expect(opts?.outputContainer).toBe('mkv');
    expect(opts?.droppedIncompatibleStreamCount).toBe(1);
    expect(opts?.droppedIncompatibleStreamDescriptors).toEqual(['data:none']);
    // EXACTLY ONE source ffprobe (no double probe).
    const probeCalls = (ffprobeMock as unknown as { mock: { calls: unknown[][] } }).mock.calls;
    expect(probeCalls.filter((call) => String(call[0]) === sourcePath)).toHaveLength(1);
  });

  // 41-01 MH-1: MKV source with an attachment (font) stream → NOT counted.
  it('test_match_source_when_mkv_source_has_attachment_then_count_zero_fonts_kept', async () => {
    await setupDispatch({
      setting: 'match-source',
      filename: 'movie.mkv',
      ffprobeImpl: async () => avsAttachmentProbe(),
    });
    await loopOnce();
  ...
  // 41-01 AC-9/SR-1: mp4-resolved source that FALLS BACK to MKV (truehd audio)
  // AND carries a data stream → warn-feed still fires via the post-fallback `if`,
  // and the fallback-path probe is REUSED (source ffprobe called ONCE, not twice).
  it('test_match_source_when_mp4_falls_back_to_mkv_with_data_stream_then_count_threaded_single_probe', async () => {
    const ffprobeMock: (p: string) => Promise<ProbeResult | null> = vi.fn(async () =>
      truehdWithDataStreamProbe(),
    );
    const { sourcePath } = await setupDispatch({
      setting: 'match-source',
      filename: 'movie.mp4',
    ...
    expect(runEncodeSpy).toHaveBeenCalledTimes(1);
    const opts = runEncodeSpy.mock.calls[0]?.[0];
    // truehd → mp4 fallback flipped effectiveContainer to mkv.
    expect(opts?.outputContainer).toBe('mkv');
    expect(warnSpy).toHaveBeenCalledWith(
      expect.objectContaining({ action: 'mp4_fallback_to_mkv', fallbackReason: 'audio' }),
      expect.any(String),
    );
    // The data stream is counted on the fallback path.
    expect(opts?.droppedIncompatibleStreamCount).toBe(1);
    expect(opts?.droppedIncompatibleStreamDescriptors).toEqual(['data:none']);
    // SR-1: the mp4 pre-flight probe is reused — exactly ONE source ffprobe.
    const probeCalls = (ffprobeMock as unknown as { mock: { calls: unknown[][] } }).mock.calls;
    expect(probeCalls.filter((call) => String(call[0]) === sourcePath)).toHaveLength(1);
  });
...
describe('orchestrator — 16-05 NEW-default output_suffix=-x265 (AC-5 + AC-7)', () => {
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // 49-01: cover-art ordinals are threaded from whichever probe already ran.
  // ───────────────────────────────────────────────────────────────────────────

  // AC-9 (mkv seam): pure-MKV path — the ordinals come from the LOCAL
  // mkvSourceProbe, and preflightSourceProbe stays null (41-01 MH-2).
  it('49-01 AC-9: mkv source with cover art threads the ordinals into runEncode', async () => {
    const ffprobeMock = vi.fn(async () => aacWithCoverProbe());
    await setupDispatch({
      setting: 'match-source',
      filename: 'movie.mkv',
      ffprobeImpl: ffprobeMock as () => Promise<ProbeResult | null>,
    ...
    const opts = runEncodeSpy.mock.calls[0]?.[0];
    expect(opts?.outputContainer).toBe('mkv');
    expect(opts?.attachedPicVideoOrdinals).toEqual([1]);
    expect(opts?.encodedVideoOrdinals).toEqual([0]);
  });

  // AC-9 (mp4 seam): the ordinals come from the mp4 preflight probe.
  it('49-01 AC-9: mp4 source with cover art threads the ordinals into runEncode', async () => {
    const ffprobeMock = vi.fn(async () => aacWithCoverProbe());
    await setupDispatch({
      setting: 'mp4',
      filename: 'movie.mp4',
    ...
    await loopOnce();
    const opts = runEncodeSpy.mock.calls[0]?.[0];
    expect(opts?.outputContainer).toBe('mp4');
    expect(opts?.attachedPicVideoOrdinals).toEqual([1]);
    expect(opts?.encodedVideoOrdinals).toEqual([0]);
  });

  // AC-15: the mp4 block flips effectiveContainer to mkv (truehd audio). The mkv
  // block runs AFTERWARDS as a separate top-level `if` — a single assignment
  // point would silently fall through on exactly this transition.
  it('49-01 AC-15: mp4→mkv fallback keeps the cover ordinals set exactly once', async () => {
    const ffprobeMock = vi.fn(async () => truehdWithCoverProbe());
    await setupDispatch({
      setting: 'match-source',
      filename: 'movie.mp4',
      ffprobeImpl: ffprobeMock as () => Promise<ProbeResult | null>,
    ...
    await loopOnce();
    const opts = runEncodeSpy.mock.calls[0]?.[0];
    // the container really did switch
    expect(opts?.outputContainer).toBe('mkv');
    expect(opts?.attachedPicVideoOrdinals).not.toBeUndefined();
    expect(opts?.attachedPicVideoOrdinals).toEqual([1]);
    expect(opts?.encodedVideoOrdinals).toEqual([0]);
  });

  // AC-8 companion at the dispatch layer: a source without a cover leaves BOTH
  // fields undefined so the argv stays byte-identical to pre-49.
  it('49-01 AC-8: source without cover art leaves both ordinal fields undefined', async () => {
    await setupDispatch({ setting: 'match-source', filename: 'movie.mkv' });
    await loopOnce();
    const opts = runEncodeSpy.mock.calls[0]?.[0];
    expect(opts?.attachedPicVideoOrdinals).toBeUndefined();
    expect(opts?.encodedVideoOrdinals).toBeUndefined();
  });

  // AC-11: ffprobe returning null soft-degrades — the fix is observability
  // dependent by design; a hard abort here would create a NEW failure class for
  // files that run fine today.
  it('49-01 AC-11: ffprobe null leaves the ordinals undefined and the encode runs', async () => {
    const ffprobeMock = vi.fn(async () => null);
    await setupDispatch({
      setting: 'match-source',
      filename: 'movie.mkv',
      ffprobeImpl: ffprobeMock as () => Promise<ProbeResult | null>,
    ...
    await loopOnce();
    expect(runEncodeSpy).toHaveBeenCalled();
    const opts = runEncodeSpy.mock.calls[0]?.[0];
    expect(opts?.attachedPicVideoOrdinals).toBeUndefined();
    expect(opts?.encodedVideoOrdinals).toBeUndefined();
  });
