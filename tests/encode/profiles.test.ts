...
describe('buildCodecBlock — per-encoder shape (default preset, AC-12 byte-identical for libx265/nvenc/qsv)', () => {
  ...
  it('test_buildCodecBlock_when_qsv_with_default_preset_then_includes_hevc_qsv_global_quality_lookahead', () => {
    expect(buildCodecBlock({ encoder: 'qsv', crf: 22, preset: 'slow' })).toEqual([
      '-c:v',
      'hevc_qsv',
      '-preset',
      'slow',
...
describe('buildCodecBlock — 12-03 preset override + Catalog-validator-fallback', () => {
  ...
  it('test_buildCodecBlock_when_qsv_with_veryslow_preset_then_argv_threads_veryslow_AND_non_preset_flags_byte_identical', () => {
    // AC-3 SR1: non-preset flags must remain byte-identical pre-12-03.
