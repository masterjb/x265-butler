...
// 50-01 (AC-15): the argv must reach the job log ONLY — never the ring buffer.
...
// `logger` is imported once at the top of this file (50-01) — a second import
// here is a TS2300 duplicate identifier.
...
//
// 50-02: THIS BLOCK IS NOW THE MP4 CONTRACT. The copy form survives EXACTLY on
// mp4, where the muxer preserves `attached_pic` and the 49-01 output is correct.
// On MKV the same copy produced a second real video track (M-C) — VLC crashes,
// mpv black with sound — so MKV moved to unmap + re-attach and is asserted in
// tests/encode/ffmpeg-cover-attach.test.ts. Every case below therefore pins
// `outputContainer: 'mp4'` EXPLICITLY: leaving it at the mkv default would be
// asserting the defect 50-02 removed.
// ─────────────────────────────────────────────────────────────────────────────
describe('49-01 attached-pic cover-art copy (mp4 path)', () => {
  ...
  // 50-02 REPLACES the former "works the same for both containers" assertion.
  // It no longer does, deliberately: the copy is mp4-only, mkv unmaps + attaches.
  it('AC-9 (50-02): mp4 copies the cover, mkv unmaps it — the two forks are exclusive', () => {
    const mp4 = buildArgs({
      ...base,
      encoder: 'libx265',
      outputContainer: 'mp4',
    ...
    expect(mp4).toContain('-c:v:1');
    expect(mp4[mp4.indexOf('-c:v:1') + 1]).toBe('copy');
    expect(mp4).not.toContain('-attach');
    expect(mp4.some((t) => t === '-0:v:1')).toBe(false);
    ...
    expect(mkv.some((t) => t.startsWith('-c:v:'))).toBe(false);
    expect(mkv[mkv.indexOf('-map') + 1]).toBe('0:v');
    expect(mkv).toContain('-0:v:1');
  });
...
describe('49-02 forced keyframes + closed GOP', () => {
  ...
  it('AC-4: one cover ⇒ -force_key_frames:v:0 and NEVER the bare token', () => {
    ...
    const args = buildArgs({
      ...base,
      encoder: 'libx265',
      // 50-02: mp4 — the narrowing lives on the copy path now (E3).
      outputContainer: 'mp4',
      attachedPicVideoOrdinals: [1],
      encodedVideoOrdinals: [0],
    });
...
// ── 50-01: the full argv as the first job-log line (AC-13 / AC-14 / AC-15) ────

describe('50-01 — runEncode writes the argv into the job log', () => {
  it('AC-13: the FIRST onLogChunk output carries the binary and every argument', async () => {
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const chunks: string[] = [];
    const p = runEncode({
      ...
      output: '/out.x265.mkv',
      crf: 23,
      onLogChunk: (c) => chunks.push(typeof c === 'string' ? c : c.toString('utf8')),
    });
    child.stderr.emit('data', Buffer.from('some ffmpeg noise\n'));
    child.emit('close', 0);
    await p;
    ...
    expect(first.startsWith(`ffmpeg argv: ${bin} `)).toBe(true);
    expect(first.endsWith('\n')).toBe(true);
    for (const a of args) expect(first).toContain(a);
    // The binary named in the log is the binary that was spawned — resolved once.
    expect(first).toContain(bin);
  });

  it('AC-13: the line is there even when the spawn fails instantly', async () => {
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const chunks: string[] = [];
    const p = runEncode({
      ...
      output: '/o',
      crf: 23,
      onLogChunk: (c) => chunks.push(String(c)),
    }).catch(() => null);
    child.emit('error', new Error('spawn ffmpeg ENOENT'));
    await p;
    expect(chunks[0]?.startsWith('ffmpeg argv: ')).toBe(true);
  });

  it('AC-13: a throwing onLogChunk never aborts the encode', async () => {
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const p = runEncode({
      input: '/i',
  ...
  it('AC-14: the argv itself is untouched — spawn gets exactly buildArgs(opts)', async () => {
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const opts = { input: '/in.mp4', output: '/out.x265.mkv', crf: 23 } as const;
    const expected = buildArgs({ ...opts });
    const p = runEncode({ ...opts, onLogChunk: () => {} });
    child.emit('close', 0);
    await p;
    const [, args] = spawnMock.mock.calls[0] as [string, string[]];
    expect(args).toEqual(expected);
    // No argv token was added for the logging.
    expect(args.some((a) => a.includes('ffmpeg argv'))).toBe(false);
  });

  it('AC-14: runEncode without onLogChunk behaves exactly as before', async () => {
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const expected = buildArgs({ input: '/i', output: '/o', crf: 23 });
    const p = runEncode({ input: '/i', output: '/o', crf: 23 });
    child.emit('close', 0);
    await expect(p).resolves.toMatchObject({ exitCode: 0 });
    expect(spawnMock.mock.calls[0][1]).toEqual(expected);
  });
});

describe('50-01 — the argv never reaches the ring buffer (AC-15)', () => {
  it('AC-15: no logger stream sees the argv line — job log only', async () => {
    const spies = {
      info: vi.spyOn(logger, 'info').mockImplementation(() => logger as never),
      warn: vi.spyOn(logger, 'warn').mockImplementation(() => logger as never),
      error: vi.spyOn(logger, 'error').mockImplementation(() => logger as never),
      debug: vi.spyOn(logger, 'debug').mockImplementation(() => logger as never),
      telemetry: vi.spyOn(logger, 'telemetry').mockImplementation(() => logger as never),
    };
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const chunks: string[] = [];
    ...
    // It DID reach the job-log channel …
    expect(chunks[0]?.startsWith('ffmpeg argv: ')).toBe(true);
    // … and no logger call carries it (the ring buffer feeds the copy-report).
    for (const spy of Object.values(spies)) {
      for (const call of spy.mock.calls) {
        expect(JSON.stringify(call)).not.toContain('ffmpeg argv:');
      }
    }
    vi.restoreAllMocks();
  });
