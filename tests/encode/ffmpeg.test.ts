...
import { runEncode, type ProgressEvent } from '@/src/lib/encode/ffmpeg';
...
describe('buildArgs — 12-03 uniform preset threading (AC-4 + SR1 + M5)', () => {
  ...
  it('test_buildArgs_when_encoder_qsv_with_preset_veryslow_then_argv_threads_veryslow_AND_non_preset_flags_byte_identical_SR1', async () => {
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const p = runEncode({
      input: '/i',
    ...
    // SR1: QSV non-preset block byte-identical.
    expect(args).toContain('-global_quality');
    expect(args[args.indexOf('-global_quality') + 1]).toBe('22');
    expect(args).toContain('-look_ahead');
    expect(args[args.indexOf('-look_ahead') + 1]).toBe('1');
    expect(args).toContain('-look_ahead_depth');
    expect(args[args.indexOf('-look_ahead_depth') + 1]).toBe('40');
  });
