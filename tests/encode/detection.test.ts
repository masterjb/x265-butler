...
import {
  PROBE_FRAME_SIZE,
  // 49-03: the shared synthetic pixel-format pin + its encoder predicate, so the
  // no-drift invariant covers the new pin exactly as 29-01 covers the frame size.
  SYNTHETIC_PROBE_PIX_FMT,
  usesSyntheticPixFmtPin,
  // 50-06: buildTestEncodeArgs takes the resolved preset explicitly now.
...
describe('29-01: buildProbeEncodeArgs HW-safe frame size + no drift', () => {
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // 50-06 AC-24: the BOOT PROBE stays configuration-free.
  //
  // This is a deliberate ASYMMETRY, not an oversight: the boot probe answers
  // "does this hardware work", the /diagnostics test encode answers "does MY
  // configuration run". A probe that fed the operator's CRF or preset could gate
  // a working encoder OUT of `detected[]` because of a setting — the exact
  // mis-diagnosis class 30-01 and 49-03 each had to close.
  // ───────────────────────────────────────────────────────────────────────────
  describe('50-06 AC-24: buildProbeEncodeArgs reads no operator settings', () => {
    const ENCODERS = ['libx265', 'nvenc', 'qsv', 'vaapi'] as const;

    it('its argv is unchanged by 50-06 — hard-pinned crf 28 and no keyframe token', () => {
      for (const enc of ENCODERS) {
        const argv = __forTests_buildProbeEncodeArgs(enc);
        // the probe keeps its OWN fixed quality value...
        expect(argv).toContain('28');
        // ...and never grew the tokens the test encode gained
        expect(argv).not.toContain('-force_key_frames');
        // the lavfi source stays exactly as it was: no format conversion
        const i = argv.indexOf('-i');
        expect(argv[i + 1]).toBe(`testsrc=size=${PROBE_FRAME_SIZE}:rate=1:duration=1`);
      }
    });

    it('detection.ts never imports the settings resolvers', () => {
      const raw = readFileSync('src/lib/encode/detection.ts', 'utf8');
      const code = raw.replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/.*$/gm, '');
      expect(code).not.toContain('encode-settings-resolve');
...
// ─────────────────────────────────────────────────────────────────────────────
// 49-03 AC-5: the boot probe pins `-pix_fmt nv12` for qsv/nvenc — the G3 fix.
//
// `testsrc` emits rgb24. Modern `hevc_qsv` builds list RGB in their `pix_fmts`
// table, so ffmpeg inserts NO auto-converter and hands RGB to iHD, which rejects
// it (`Current pixel format is unsupported`). The probe then labels the encoder
// `compiled-in-broken` and gates it out of detected[] — on a host whose real
// (yuv420p-sourced) QSV encodes run perfectly. The N100 forum report is exactly
// that signature: /diagnostics says broken, the encodes work.
//
// Asserted against the REAL private builder via the __forTests seam (audit S-2),
// never against an argv rebuilt by hand.
// ─────────────────────────────────────────────────────────────────────────────
describe('49-03 AC-5: probe argv pins the synthetic pixel format', () => {
  const pairIndex = (argv: string[], a: string, b: string): number => {
    for (let i = 0; i < argv.length - 1; i++) if (argv[i] === a && argv[i + 1] === b) return i;
    return -1;
  };

  it('qsv: both ratecontrol variants carry -pix_fmt nv12', () => {
    for (const rc of ['icq-full', 'cqp'] as const) {
      const argv = __forTests_buildProbeEncodeArgs('qsv', undefined, rc);
      expect(pairIndex(argv, '-pix_fmt', 'nv12')).toBeGreaterThanOrEqual(0);
      expect(argv.filter((t) => t === '-pix_fmt')).toHaveLength(1);
    }
  });

  it('nvenc: carries -pix_fmt nv12', () => {
    const argv = __forTests_buildProbeEncodeArgs('nvenc');
    expect(pairIndex(argv, '-pix_fmt', 'nv12')).toBeGreaterThanOrEqual(0);
  });

  it('libx265 and vaapi carry NO -pix_fmt token', () => {
    for (const enc of ['libx265', 'vaapi'] as const) {
      const argv = __forTests_buildProbeEncodeArgs(enc, '/dev/dri/renderD128');
      expect(argv).not.toContain('-pix_fmt');
    }
  });

  it('vaapi keeps nv12 ONLY inside its hwupload filter chain', () => {
    const argv = __forTests_buildProbeEncodeArgs('vaapi', '/dev/dri/renderD128');
    expect(argv).toContain('format=nv12,hwupload');
    expect(argv).not.toContain('-pix_fmt');
  });

  it('the lavfi source is UNCHANGED — the pin lives in the codec block (D5=B)', () => {
    for (const enc of ENCODER_IDS) {
      const argv = __forTests_buildProbeEncodeArgs(enc);
      expect(argv).toContain(`testsrc=size=${PROBE_FRAME_SIZE}:rate=1:duration=1`);
      // no `,format=nv12` grafted onto the source (variant A′ was rejected)
      expect(argv.some((t) => t.startsWith('testsrc=') && t.includes('format='))).toBe(false);
    }
  });

  it('both synthetic builders share ONE pin value (no 29-01-style drift)', () => {
    expect(SYNTHETIC_PROBE_PIX_FMT).toBe('nv12');
    for (const enc of ['qsv', 'nvenc'] as const) {
      expect(usesSyntheticPixFmtPin(enc)).toBe(true);
      const probe = __forTests_buildProbeEncodeArgs(enc);
      const test = buildTestEncodeArgs({
        encoder: enc,
        crf: 28,
        preset: DEFAULT_PRESET_BY_ENCODER[enc],
      });
      expect(pairIndex(probe, '-pix_fmt', SYNTHETIC_PROBE_PIX_FMT)).toBeGreaterThanOrEqual(0);
      expect(pairIndex(test, '-pix_fmt', SYNTHETIC_PROBE_PIX_FMT)).toBeGreaterThanOrEqual(0);
    }
    for (const enc of ['libx265', 'vaapi'] as const) {
      expect(usesSyntheticPixFmtPin(enc)).toBe(false);
    }
  });

  // AC-9: the CONFIRM spawn carries the IDR token; every TIER spawn does not.
  it('AC-9: forceIdr threads into the probe argv, and is absent by default', () => {
    const qsvTier = __forTests_buildProbeEncodeArgs('qsv', undefined, 'icq-full');
    expect(qsvTier).not.toContain('-forced_idr');
    const qsvConfirm = __forTests_buildProbeEncodeArgs('qsv', undefined, 'icq-full', true);
    expect(pairIndex(qsvConfirm, '-forced_idr', '1')).toBeGreaterThanOrEqual(0);
    // the confirm spawn keeps the SAME pin and the SAME ratecontrol as its tier
    expect(pairIndex(qsvConfirm, '-pix_fmt', 'nv12')).toBeGreaterThanOrEqual(0);
    expect(qsvConfirm).toContain('-global_quality');

    const nvTier = __forTests_buildProbeEncodeArgs('nvenc');
    expect(nvTier).not.toContain('-forced-idr');
    const nvConfirm = __forTests_buildProbeEncodeArgs('nvenc', undefined, undefined, true);
    expect(pairIndex(nvConfirm, '-forced-idr', '1')).toBeGreaterThanOrEqual(0);
    expect(pairIndex(nvConfirm, '-pix_fmt', 'nv12')).toBeGreaterThanOrEqual(0);
  });

  it('AC-9: the confirm spawn carries NO -force_key_frames (Q4)', () => {
    for (const enc of ['qsv', 'nvenc'] as const) {
      const argv = __forTests_buildProbeEncodeArgs(enc, undefined, undefined, true);
      expect(argv.some((t) => t.startsWith('-force_key_frames'))).toBe(false);
    }
  });
});
