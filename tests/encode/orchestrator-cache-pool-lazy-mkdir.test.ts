...
// This is a real-I/O integration file (tmpdir mkdtemp/rmSync per test + a full
// loopOnce dispatch). Isolated it runs in ~0.5s, but under the full parallel
// suite it can be starved past Vitest's 5s default and time out spuriously.
// Raise the per-test + hook ceiling so CPU-starvation does not flake the run.
...
describe('22-02 T1.5: orchestrator cache-pool lazy-mkdir + downstream statfs remap', () => {
  ...
  it('24-03 supersedes 22-02: empty cache_pool_path is DC-B-resolved, NOT an empty shape-error', async () => {
    // Pre-24-03 (22-02) this seeded cache_pool_path='' produced the byte-empty
    // stageRoot '' (old `settings.get() ?? DEFAULT` — '' is non-nullish) which
    // tripped assertValidStageRoot → 'invalid_cache_pool_path:empty'. 24-03's
    // resolveEffectiveCachePath `.trim()`s the empty value and routes it through
    // DC-B (probe /mnt/cache → /mnt/cache/x265-butler, else /config/cache), so
    // the orchestrator NEVER sees '' as stageRoot anymore (audit deferred-item #1;
    // the clear-to-unset PUT also DELETEs the row rather than storing ''). The
    // deterministic, environment-independent contract: the empty shape-error is
    // gone. The job either succeeds (resolved path writable) or fails with a
    // cache_pool_unavailable:* probe error — but never the empty shape-error.
    ...
    expect(job.error_msg).not.toBe('invalid_cache_pool_path:empty');
  });
