...
describe('orchestrator → engineEvents emit (02-03 Task 2)', () => {
  ...
  it('test_loopOnce_emits_queue_updated_AFTER_terminal_with_fresh_counts', async () => {
    ...
    // 48-03: encoding-only count travels on the wire and never exceeds activeJobs.
    expect(last.encodingJobs).toBe(0);
    expect(last.encodingJobs).toBeLessThanOrEqual(last.activeJobs);
  });
