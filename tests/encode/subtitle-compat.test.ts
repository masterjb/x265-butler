...
describe('analyzeStreams', () => {
  ...
  it('test_analyzeStreams_when_mixed_mov_text_and_ass_and_mp4_then_only_ass_incompatible', () => {
    const result = analyzeStreams(
      makeProbe([
        { attachedPic: false, index: 2, codec_type: 'subtitle', codec_name: 'mov_text' },
        { attachedPic: false, index: 3, codec_type: 'subtitle', codec_name: 'ass' },
      ]),
      'mp4',
    );
  ...
  it('test_analyzeStreams_when_subtitle_stream_missing_codec_name_then_skipped_no_throw', () => {
    expect(() =>
      analyzeStreams(makeProbe([{ attachedPic: false, index: 1, codec_type: 'subtitle' }]), 'mp4'),
    ).not.toThrow();
    const result = analyzeStreams(
      makeProbe([{ attachedPic: false, index: 1, codec_type: 'subtitle' }]),
      'mp4',
    );
    expect(result.hasIncompatibleSubs).toBe(false);
  });
