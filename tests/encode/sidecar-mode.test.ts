...
const { mockWriteFile, mockRename, mockUnlink, mockMkdir, mockStat, mockReadFile, mockLogger } =
  vi.hoisted(() => ({
    mockWriteFile: vi.fn(),
    mockRename: vi.fn(),
    mockUnlink: vi.fn(),
...
describe('readSidecarResolved (33-01 — read-side mirror)', () => {
  const TARGET = '/media/movies/x.mkv';
  const BESIDE = '/media/movies/x.mkv.x265-butler.json';
  const CENTRAL_PATH = '/config/x265-butler/sidecars/media/movies/x.mkv.x265-butler.json';
  const JSON_PAYLOAD = JSON.stringify(PAYLOAD);

  // path-aware fs: stat/readFile succeed only for paths present in `files`.
  function primeFs(files: Record<string, string>): void {
  ...
  it('DEFAULT_SIDECAR_CENTRAL_PATH equals the 26-01 write-default (AC-7 no drift)', () => {
    expect(DEFAULT_SIDECAR_CENTRAL_PATH).toBe('/config/x265-butler/sidecars/');
    expect(DEFAULT_SIDECAR_CENTRAL_PATH).toBe(CENTRAL);
  });

  it('central → reads the central-resolved path FIRST (AC-1/AC-2)', async () => {
    primeFs({ [CENTRAL_PATH]: JSON_PAYLOAD }); // ONLY central exists, no beside
    const r = await readSidecarResolved(TARGET, 'central', CENTRAL);
    expect(r).toMatchObject({ schema: 'x265-butler/v2', source: { contentHash: 'a'.repeat(64) } });
    expect(mockReadFile).toHaveBeenCalledWith(CENTRAL_PATH, 'utf-8');
  });

  it('central MISS → falls back to a beside read (D1=B, AC-3)', async () => {
    primeFs({ [BESIDE]: JSON_PAYLOAD }); // NO central sidecar, beside exists
    const r = await readSidecarResolved(TARGET, 'central', CENTRAL);
    expect(r).toMatchObject({ schema: 'x265-butler/v2' });
    expect(mockReadFile).toHaveBeenCalledWith(BESIDE, 'utf-8');
  });

  it('central miss + no beside → null (both absent)', async () => {
    primeFs({});
    const r = await readSidecarResolved(TARGET, 'central', CENTRAL);
    expect(r).toBeNull();
  });

  it('beside → reads ONLY the beside path, never the central path (AC-4)', async () => {
    primeFs({ [BESIDE]: JSON_PAYLOAD, [CENTRAL_PATH]: JSON_PAYLOAD });
    const r = await readSidecarResolved(TARGET, 'beside', CENTRAL);
    expect(r).toMatchObject({ schema: 'x265-butler/v2' });
    expect(mockReadFile).toHaveBeenCalledWith(BESIDE, 'utf-8');
    expect(mockReadFile).not.toHaveBeenCalledWith(CENTRAL_PATH, 'utf-8');
  });

  it('off → reads ONLY the beside path (byte-identical to pre-33-01, AC-4)', async () => {
    primeFs({ [BESIDE]: JSON_PAYLOAD, [CENTRAL_PATH]: JSON_PAYLOAD });
    const r = await readSidecarResolved(TARGET, 'off', CENTRAL);
    expect(r).toMatchObject({ schema: 'x265-butler/v2' });
    expect(mockReadFile).toHaveBeenCalledWith(BESIDE, 'utf-8');
    expect(mockReadFile).not.toHaveBeenCalledWith(CENTRAL_PATH, 'utf-8');
  });

  it('central traversal-guard throw → soft-degrades to beside + warns (AC-5)', async () => {
    primeFs({ '/../../etc/passwd.x265-butler.json': JSON_PAYLOAD });
    // `/../../etc/passwd` central-resolves to an escaping path → resolveSidecarTarget throws.
    const r = await readSidecarResolved('/../../etc/passwd', 'central', CENTRAL);
    // soft-degrade: falls back to beside read (which here resolves + finds the file)
    expect(r).toMatchObject({ schema: 'x265-butler/v2' });
    expect(mockLogger.warn).toHaveBeenCalledWith(
      expect.objectContaining({ action: 'sidecar_read_resolve_failed', mode: 'central' }),
      expect.any(String),
    );
  });
});
