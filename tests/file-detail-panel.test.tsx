...
// 28-02 R8: mock authFetch so the in-panel fetches are controllable. Default is a
// never-resolving promise → ContainerFallbackCard / EncodeInfoSection stay hidden
// (identical to the pre-mock behavior where real authFetch failed in jsdom and the
// sections silently hid). The crash-smoke test overrides it with a deferred promise.
const { authFetchMock } = vi.hoisted(() => ({ authFetchMock: vi.fn() }));
vi.mock('@/components/auth/auth-fetcher', () => ({
  authFetch: authFetchMock,
  markLogoutClicked: vi.fn(),
}));
...
describe('FileDetailPanel', () => {
  beforeEach(() => {
    // Default: in-flight forever → panel sub-fetch sections hidden (pre-mock parity).
  ...
  // 28-02 R8 — AC-4 CRASH SMOKE (necessary-not-sufficient). NOT a regression pin:
  // React 19 (react@^19.2.5) makes setState-after-unmount a SILENT no-op (no
  // warning emitted), so a "console.error not called with unmount-warning"
  // assertion would be GREEN with OR without the isMountedRef guard → tautological.
  // The REAL R8 verification is STRUCTURAL (grep, in the plan's <verification>).
  // This case only proves the in-flight-unmount-then-settle path does not throw /
  // leave an unhandled rejection.
  it('test_FileDetailPanel_when_unmounted_mid_container_override_save_then_no_throw', async () => {
    let resolveSave: (v: unknown) => void = () => {};
    authFetchMock.mockImplementationOnce(
      () =>
        new Promise((r) => {
          resolveSave = r as (v: unknown) => void;
        }),
    );
    ...
    // Make the container-override select dirty so Save enables, then click it.
    const select = screen.getByRole('combobox');
    await act(async () => {
      fireEvent.change(select, { target: { value: 'mp4' } });
    });
    const saveBtn = screen.getByRole('button', { name: /save container override/i });
    await act(async () => {
      fireEvent.click(saveBtn);
    });

    // Unmount while the PATCH is in flight, THEN settle the request → the handler's
    // post-await finally setters run against an unmounted component. isMountedRef
    // guards them; on React 19 they'd be silent no-ops regardless — assert no throw.
    unmount();
    await act(async () => {
    ...
    expect(true).toBe(true); // reached = no thrown error / no unhandled rejection
  });
});
