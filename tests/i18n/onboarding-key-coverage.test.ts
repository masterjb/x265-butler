...
describe('onboarding i18n key coverage (16-03 audit)', () => {
  ...
  const NEW_NAMESPACES_20_03 = [
    ...
    // 49-05 (AC-15): 6 → 7. The explainer gained exactly ONE key,
    // `notTransferable` — the sentence that the uniform 0..51 axis is a UI
    // convention, not a claim that the same number means the same quality on
    // another encoder. Everything else 20-03 froze (SCALE_MAX, gradient,
    // markerPercent, crfExplainerOpened, the kill-switch) is untouched.
    { prefix: 'onboarding.step5.crfExplainer.', count: 7 },
  ] as const;
