...
import type { NvidiaGpuSource } from '@/src/lib/diagnostics/types';
...
describe('diagnostics i18n key-coverage', () => {
  ...
  // 46-02 (D2, AC-8, SR-2): nvidiaGpu.source must enumerate EVERY NvidiaGpuSource
  // union member in BOTH locales — a missing key would render a raw dotted
  // `nvidiaGpu.source.<x>` on the surface. The `Record<NvidiaGpuSource, true>`
  // literal makes a future 6th union member a COMPILE error here (must extend
  // this list), and the runtime assert catches a missing translation.
  it('46-02: diagnostics.nvidiaGpu.source enumerates all NvidiaGpuSource members (EN + DE)', () => {
    const EXPECTED: Record<NvidiaGpuSource, true> = {
      present: true,
      binary_missing: true,
      no_gpu: true,
      timeout: true,
    ...
    const enSource = (en as { diagnostics: { nvidiaGpu: { source: Record<string, string> } } })
      .diagnostics.nvidiaGpu.source;
    const deSource = (de as { diagnostics: { nvidiaGpu: { source: Record<string, string> } } })
      .diagnostics.nvidiaGpu.source;
    expect(Object.keys(enSource).sort()).toEqual(expectedKeys);
    expect(Object.keys(deSource).sort()).toEqual(expectedKeys);
    // every value is a non-empty string (no blank fallback).
    for (const v of Object.values(enSource)) expect(v.length).toBeGreaterThan(0);
    for (const v of Object.values(deSource)) expect(v.length).toBeGreaterThan(0);
  });
});
