...
vi.mock('@/src/lib/encode', () => ({
  ...
  // 24-03: page.tsx computes the cache block via the Cached resolver.
...
describe('Settings hash-anchor handler — 20-01 (AC-12 / S10)', () => {
  ...
  // 35-02 AC-9 — new #auto-crop branch lands on the encoder tab (the Auto-Crop
  // Card lives in the non-default tab==='encoder' block; without this branch the
  // onboarding deep-link is dead). Mirrors the #encoder-config branch.
  it('test_settings_when_hash_auto_crop_then_encoder_tab_aria_selected_true', async () => {
    await act(async () => {
      await renderPageWithHash('#auto-crop');
    });
    await waitFor(() => {
      const encoderTab = screen.getByRole('tab', { name: /encoder/i });
      expect(encoderTab).toHaveAttribute('aria-selected', 'true');
  ...
  // 44-01 (MH-1) — the load-bearing #auto-crop anchor must RESOLVE to a mounted
  // element nested inside the consolidated Encoding-Profile card, not merely
  // switch the tab. The aria-selected assertion above is invariant to whether the
  // anchor exists (it would pass even if id="auto-crop" were deleted); THIS test
  // is what actually locks the deep-link target after the 3-card → 1-card reorg.
  // Mirrors the composition test's querySelector + not.toBeNull() shape.
  it('test_settings_when_hash_auto_crop_then_anchor_resolves_inside_encoding_profile_card', async () => {
    let container: HTMLElement;
    await act(async () => {
      ({ container } = await renderPageWithHash('#auto-crop'));
    });
    await waitFor(() => {
      const encoderTab = screen.getByRole('tab', { name: /encoder/i });
      expect(encoderTab).toHaveAttribute('aria-selected', 'true');
    });
    const anchor = container!.querySelector('#auto-crop');
    expect(anchor).not.toBeNull();
    // The anchor div sits inside the single Encoding-Profile card, alongside the
    // other two video toggles — proving the consolidation, not three separate cards.
    const profileCard = (anchor as HTMLElement).closest('[data-slot="card"], .rounded-xl');
    expect(profileCard).not.toBeNull();
    expect(profileCard?.querySelector('#force-10bit')).not.toBeNull();
    expect(profileCard?.querySelector('#color-passthrough')).not.toBeNull();
  });

  it('test_settings_when_hash_auto_crop_then_request_animation_frame_scheduled', async () => {
