...
const {
  ...
  // 48-03: widened from `vi.fn(() => null)` — AC-19 needs to hand the hook a real
  // ActiveJob-shaped object, which the null-only inferred return type rejected.
  mockUseActiveJob: vi.fn<() => Record<string, unknown> | null>(() => null),
  // 48-03: the hoisted default fixes the return type of every mockReturnValue below.
...
describe('DashboardClient', () => {
  ...
  it('test_AC11_when_nothing_encoding_but_995_queued_then_idle_branch_not_active_branch', () => {
    // Pre-48-03 the card branched on activeJobs (= queued+encoding): with 995
    // waiting jobs it entered the active branch, found no activeJob and fell
    // through to the empty coffee branch.
    mockUseActiveJob.mockReturnValue(null);
    mockUseQueueCounts.mockReturnValue({
      activeJobs: 995,
      pendingJobs: 995,
    ...
    render(
      wrap(
        <DashboardClient
          initialStats={FIXTURE_STATS}
          initialQueueStatus={{ ...FIXTURE_QUEUE, activeJobs: 995, pendingJobs: 995 }}
          initialEncoders={FIXTURE_ENCODERS}
          requestId="test-req"
        />,
      ),
    );
    expect(screen.getByText('Idle — 995 jobs queued')).toBeTruthy();
    expect(screen.queryByText('Queue empty')).toBeNull();
  });

  it('test_AC20_when_no_live_value_yet_then_SSR_seed_drives_the_idle_branch', () => {
    // audit M5: the SSR seed was dead code since 03-04 (`??` never fires on a
    // required number initialised to 0). Gated on hasLiveCounts it finally works.
    mockUseActiveJob.mockReturnValue(null);
    mockUseQueueCounts.mockReturnValue({
      activeJobs: 0,
      pendingJobs: 0,
  ...
  it('test_AC19_drain_transition_card_moves_from_active_to_idle_when_encodes_finish', () => {
    // 4 encoding, 995 queued → active branch; encodes finish (encodingJobs 4 → 0,
    // pendingJobs stays 995) → idle branch, NOT the empty coffee branch.
    mockUseActiveJob.mockReturnValue({
      jobId: 1,
      fileId: 1,
      filePath: '/m/a.mkv',
      pct: 10,
    ...
    const { rerender } = render(
      wrap(
        <DashboardClient
          initialStats={FIXTURE_STATS}
          initialQueueStatus={{ ...FIXTURE_QUEUE, activeJobs: 999, pendingJobs: 995 }}
          initialEncoders={FIXTURE_ENCODERS}
          requestId="test-req"
        />,
      ),
    );
    expect(screen.queryByText('Idle — 995 jobs queued')).toBeNull();
