...
describe('ShareAddForm', () => {
  ...
  it('test_submit_when_server_400_validation_then_field_errors_inline', async () => {
    ...
    await waitFor(() => expect(submit).not.toBeDisabled());
    await userEvent.click(submit);
    // 48-02 (AC-20): the code is now TRANSLATED before it reaches setError —
    // pre-48-02 this asserted the raw token `name_invalid_chars`, which is
    // exactly the cryptic-error state audit-S2 flagged.
    await waitFor(() =>
      expect(screen.getByText(/may only contain letters, numbers/i)).toBeInTheDocument(),
    );
  });
...
// 48-02 (audit S2 → AC-20): mirrors the ShareEditForm coverage — the add form
// carries the SAME validation branch and must not render the raw code either.
describe('ShareAddForm — 48-02 server-error-code translation (AC-20)', () => {
  async function submitWithValidationError(fieldErrors: Record<string, string>) {
    const onAdd = vi.fn<(b: unknown) => Promise<ShareSaveResult>>(async () => ({
      ok: false,
  ...
  it('test_submit_when_server_returns_path_forbidden_prefix_then_translated_sentence_shown', async () => {
    await submitWithValidationError({ path: 'path_forbidden_prefix' });
    await waitFor(() => {
  ...
  it('test_submit_when_server_returns_name_invalid_chars_then_translated_sentence_shown', async () => {
    await submitWithValidationError({ name: 'name_invalid_chars' });
    await waitFor(() => {
      expect(screen.getByText(/may only contain letters, numbers/i)).toBeInTheDocument();
    });
  });

  it('test_submit_when_server_returns_unknown_code_then_raw_code_shown_and_no_crash', async () => {
    await submitWithValidationError({ path: 'some_future_server_code' });
    await waitFor(() => {
