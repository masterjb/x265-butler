...
describe('SettingsForm composition pin (28-10 L2 split, AC-7)', () => {
  ...
  it('test_settingsForm_when_split_then_every_card_threads_its_signature_node', async () => {
    ...
    // 44-01 (G1): Encoding-Profile card — the three video toggles (auto-crop,
    // force-10bit, colour) are grouped under ONE card titled "Encoding profile",
    // each retaining its element-id anchor on a mounted sub-section div.
    expect(screen.getByText('Encoding profile')).toBeInTheDocument();
    const autoCropAnchor = container.querySelector('#auto-crop');
    const force10bitAnchor = container.querySelector('#force-10bit');
    const colorAnchor = container.querySelector('#color-passthrough');
    expect(autoCropAnchor).not.toBeNull();
    expect(force10bitAnchor).not.toBeNull();
    expect(colorAnchor).not.toBeNull();
    // All three anchors live inside the SAME card (single consolidated region).
    const profileCard = (autoCropAnchor as HTMLElement).closest('[data-slot="card"], .rounded-xl');
    expect(profileCard).not.toBeNull();
    expect(profileCard?.contains(force10bitAnchor)).toBe(true);
    expect(profileCard?.contains(colorAnchor)).toBe(true);
