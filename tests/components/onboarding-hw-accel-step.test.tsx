...
// 34-02: HwAccelStep now ALSO fetches GET /api/encoders/render-devices for the
// device picker (a SEPARATE effect from the /api/encoders/refresh detection
// probe). The shared mock must answer that URL with [] so existing detection
// assertions + callCount-based mocks (which count ONLY the refresh probe) stay
// valid. Tests that exercise the picker stub their own device-list response.
function isRenderDevicesUrl(url: unknown): boolean {
  return typeof url === 'string' && url.includes('/api/encoders/render-devices');
}

function mockFetchOnce(payload: DetectionWithWarnings | 'error', status = 200) {
  return vi.fn(async (url: string) => {
    if (isRenderDevicesUrl(url)) {
...
describe('HwAccelStep — React.StrictMode dev-double-mount (AC-15)', () => {
  ...
  it('test_hw_accel_step_useEffect_fires_fetch_exactly_once_under_strict_mode', async () => {
    ...
    await screen.findByRole('region', { name: /Software encoding active/i });
    // Even with StrictMode double-mount, probeFiredRef guards the detection
    // probe to exactly one /api/encoders/refresh call. 34-02: the device-list
    // probe is a SEPARATE once-guarded fetch, so filter by URL rather than count
    // all calls.
    const refreshCalls = fetchMock.mock.calls.filter(
      (c) => !isRenderDevicesUrl(c[0]) && String(c[0]).includes('/api/encoders/refresh'),
    );
...
describe('HwAccelStep — 34-02 GPU device picker', () => {
  const DEVICES = [
    {
  ...
  function mockFetchWithDevices(
    detection: DetectionWithWarnings,
    devices: unknown,
    deviceStatus = 200,
  ) {
    return vi.fn(async (url: string) => {
  ...
  it('test_picker_renders_when_render_devices_fetch_returns_nodes', async () => {
    vi.stubGlobal('fetch', mockFetchWithDevices(makePayload({ detected: ['libx265'] }), DEVICES));
    render(
      wrap(
        <HwAccelStep
          cachedDetection={null}
    ...
    expect(await screen.findByLabelText('Render device')).toBeInTheDocument();
  });

  it('test_picker_NOT_rendered_when_render_devices_fetch_is_empty', async () => {
    vi.stubGlobal('fetch', mockFetchWithDevices(makePayload({ detected: ['libx265'] }), []));
    render(
      wrap(
        <HwAccelStep
          cachedDetection={null}
    ...
    // Branch renders (proves the component mounted + settled), picker does not.
    await screen.findByRole('region', { name: /Software encoding active/i });
    expect(screen.queryByLabelText('Render device')).toBeNull();
  });

  it('test_picker_NOT_rendered_when_render_devices_fetch_errors', async () => {
    vi.stubGlobal(
      'fetch',
      mockFetchWithDevices(makePayload({ detected: ['libx265'] }), 'boom', 500),
    );
    render(
      wrap(
        <HwAccelStep
  ...
  it('test_picker_selecting_a_node_calls_onDeviceSelected_with_full_path', async () => {
    ...
    const trigger = await screen.findByLabelText('Render device');
    await user.click(trigger);
    await user.click(await screen.findByRole('option', { name: 'renderD129' }));
    await waitFor(() => {
      expect(onDeviceSelected).toHaveBeenCalledWith('/dev/dri/renderD129');
    });
  });

  it('test_picker_visible_on_nvidia_branch_above_continue_anyway', async () => {
    vi.stubGlobal(
      'fetch',
      mockFetchWithDevices(
        makePayload({ detected: ['libx265'], warnings: [{ code: 'nvenc_no_runtime' }] }),
        DEVICES,
      ),
    );
    ...
    // nvidia branch suppresses the generic Continue (in-panel Continue-Anyway).
    await screen.findByRole('region', { name: /NVIDIA plugin missing/i });
    // The picker still renders (orthogonal to the vendor branch).
    expect(await screen.findByLabelText('Render device')).toBeInTheDocument();
  });
});
