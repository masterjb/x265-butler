...
const {
  ...
  // 48-03: the counter is encodingJobs + pendingJobs now (was activeJobs +
  // pendingJobs, which double-counted every queued row).
...
describe('CancelAllButton — P2 confirm flow (AlertDialog removed)', () => {
  ...
  // 48-03 AC-12
  it('test_AC12_counter_counts_every_row_once_999_not_1994', () => {
    // 4 encoding + 995 queued. Pre-48-03 the counter read activeJobs (999, i.e.
    // queued+encoding) + pendingJobs (995) = 1994 for a 999-row queue — a
    // trust-destroying number in a destructive confirm.
    mockUseQueueCounts.mockReturnValue({
      activeJobs: 999,
      pendingJobs: 995,
      encodingJobs: 4,
    ...
    expect(mockShowUndoToast).toHaveBeenCalledTimes(1);
    const args = mockShowUndoToast.mock.calls[0]![0] as unknown as { message: string };
    expect(args.message).toMatch(/999 jobs/i);
    expect(args.message).not.toMatch(/1994/);
  });
