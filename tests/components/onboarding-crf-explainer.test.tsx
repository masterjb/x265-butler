...
/*
 * 49-05 AC-15 — the explainer is EXTENDED, not rebuilt.
 *
 * The whole point of the 49-05 change here is one added sentence: the uniform
 * 0..51 axis is a UI convention, not a statement that the same number means the
 * same quality on another encoder. Everything 20-03 froze must stay frozen, so
 * this block asserts both halves: the new line is present, and the marker /
 * gradient / event / kill-switch behaviour is untouched (covered by the blocks
 * above, which still run unchanged).
 */
describe('CRFExplainer — 49-05 non-transferability line (AC-15)', () => {
  beforeEach(() => {
    mockLoggerInfo.mockClear();
    vi.unstubAllEnvs();
  });
  ...
  it.each(['en', 'de'] as const)('renders the added line in %s', (locale) => {
    const messages = locale === 'en' ? en : de;
    render(wrap(<CRFExplainer encoder="qsv" currentValue={26} />, locale));
    expect(
      screen.getByText(messages.onboarding.step5.crfExplainer.notTransferable),
    ).toBeInTheDocument();
  });

  it('the frozen 20-03 surface is still there (SCALE_MAX 51 marker math)', () => {
    render(wrap(<CRFExplainer encoder="qsv" currentValue={26} />));
    const marker = screen.getByTestId('crf-explainer-marker');
    // 26 / 51 — proves SCALE_MAX is unchanged at 51.
    expect(marker.style.left).toBe(`${(26 / 51) * 100}%`);
  });
});
