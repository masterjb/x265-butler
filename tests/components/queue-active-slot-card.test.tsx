...
describe('ActiveJobsPanel — multi-row (36-02 D1=B)', () => {
  ...
  // 43-02 (AC-5): ETA renders inline when derivable, hidden when not.
  it('renders ETA when outTimeMs + speed + durationSeconds are present', () => {
    // 30s done of 120s at 2x → (120 − 30) / 2 = 45s.
    const jobs = [activeJobFix({ jobId: 10, fileId: 1, outTimeMs: 30_000, speed: 2.0 })];
    render(
      wrap(<ActiveJobsPanel jobs={jobs} metaById={{ 1: metaFix({ durationSeconds: 120 }) }} />),
    );
    expect(screen.getByText(/ETA 45s/)).toBeInTheDocument();
  });

  it('hides ETA when speed is null', () => {
    const jobs = [activeJobFix({ jobId: 10, fileId: 1, outTimeMs: 30_000, speed: null })];
    render(
      wrap(<ActiveJobsPanel jobs={jobs} metaById={{ 1: metaFix({ durationSeconds: 120 }) }} />),
    );
  ...
  it('hides ETA when durationSeconds is null', () => {
    const jobs = [activeJobFix({ jobId: 10, fileId: 1, outTimeMs: 30_000, speed: 2.0 })];
    render(
      wrap(<ActiveJobsPanel jobs={jobs} metaById={{ 1: metaFix({ durationSeconds: null }) }} />),
    );
    expect(screen.queryByText(/ETA/)).toBeNull();
  });
});
