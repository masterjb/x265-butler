...
describe('LibraryDeleteAction P2 undo-toast flow (AC-7)', () => {
  ...
  // 47-03 AC-13: the active-bench toast names cause AND way out, and carries an
  // action that actually reaches the run (it lives on /bench, not /library).
  it('test_LibraryDeleteAction_when_409_active_bench_then_activeBench_toast_with_bench_action', async () => {
    vi.useFakeTimers();
    stubFetch({ ok: false, status: 409, jsonBody: { error: 'delete_blocked_active_bench' } });
    render(wrap(<LibraryDeleteAction file={baseFile} />));
    fireEvent.click(screen.getByRole('button', { name: /Delete entry/i }));
    await act(async () => {
      await vi.advanceTimersByTimeAsync(10_000);
    ...
    expect(mockToastError).toHaveBeenCalledWith(
      expect.stringMatching(/benchmark run that is still active/i),
      expect.objectContaining({
        action: expect.objectContaining({ label: 'View benchmark runs' }),
      }),
    );
    // The action navigates to the bench page with the ?from= provenance marker.
    const opts = mockToastError.mock.calls.at(-1)![1] as {
      action: { onClick: () => void };
    };
    opts.action.onClick();
    expect(mockRouterPush).toHaveBeenCalledWith('/bench?from=library-delete');
    expect(mockRouterRefresh).not.toHaveBeenCalled();
  });

  // A TERMINAL bench run no longer produces a 409 at all — the route answers 200
  // and the component takes the plain success path.
  it('test_LibraryDeleteAction_when_terminal_bench_ref_then_success_path', async () => {
    vi.useFakeTimers();
    stubFetch({ ok: true, status: 200, jsonBody: { deleted: true } });
    render(wrap(<LibraryDeleteAction file={baseFile} />));
    fireEvent.click(screen.getByRole('button', { name: /Delete entry/i }));
    await act(async () => {
      await vi.advanceTimersByTimeAsync(10_000);
