...
describe('EncoderStep — AutoCropAwareness callout (35-02 AC-6)', () => {
  ...
  // 37-02 AC-5: awareness body sets the no-op-is-expected expectation.
  it('body conveys that a no-bars file is encoded unchanged and that is expected', async () => {
    const fetchStub = multiUrlFetch({
      '/api/bench/recommendation': {
        status: 200,
        body: { recommendations: { nvenc: { crf: 23, preset: 'medium' } } },
    ...
    expect(
      screen.getByText(
        /A file without black bars is encoded unchanged — that is the expected result, not an error\./,
      ),
    ).toBeInTheDocument();
  });
});
