...
describe('OnboardingClient — wizard state machine', () => {
  ...
  // 34-02: a picked GPU device travels with the finish-PUT body.
  it('test_onboardingPage_when_device_picked_then_finish_PUT_includes_gpu_device', async () => {
    const user = userEvent.setup();
    const calls: { url: string; init: RequestInit }[] = [];
    const fetchMock = vi.fn(async (url: string, init: RequestInit = {}) => {
      calls.push({ url, init });
      if (url === '/api/encoders/render-devices') {
        return new Response(
          JSON.stringify([
            {
      ...
      if (url === '/api/settings')
        return new Response(JSON.stringify({ ok: true }), { status: 200 });
      if (url === '/api/onboarding/complete') {
        return new Response(JSON.stringify({ completed: true, requestId: 'rid' }), { status: 200 });
      }
      return new Response('not-found', { status: 404 });
    });
    vi.stubGlobal('fetch', fetchMock);
    render(wrap(<OnboardingClient initialSettings={FIXTURE_SETTINGS} locale="en" />));
    fireEvent.click(screen.getByRole('button', { name: /Get started/i }));
    // HwAccel step: pick the Arc node from the device picker, then Continue.
    await screen.findByRole('heading', { name: /Hardware acceleration/i });
    ...
    const putCall = calls.find((c) => c.url === '/api/settings' && c.init.method === 'PUT');
    expect(putCall).toBeDefined();
    const body = JSON.parse(String(putCall!.init.body)) as { settings: Record<string, string> };
    expect(body.settings.gpu_device).toBe('/dev/dri/renderD129');
  });
