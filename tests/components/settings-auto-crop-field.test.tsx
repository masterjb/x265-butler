...
describe('AutoCropField (35-02)', () => {
  ...
  // 37-02 AC-5: helper sets the no-op-is-expected expectation.
  it('helper conveys that a no-bars file is encoded unchanged and that is expected', () => {
    render(wrap(<Harness />));
    expect(
      screen.getByText(
        /Files without black bars are left unchanged — that is the expected result, not an error\./,
      ),
    ).toBeTruthy();
  });
