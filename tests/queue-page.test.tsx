...
describe('QueuePage (Server + Client integration)', () => {
  ...
  it('test_QueuePage_when_only_queued_active_then_no_phantom_active_row', async () => {
    ...
    // 48-03: activeJobs still counts the queued row (unchanged wire meaning), but
    // the seed gate now reads encodingJobs — nothing is encoding, so no phantom row.
    mockUseQueueCounts.mockReturnValue({
      activeJobs: 1,
      pendingJobs: 0,
      encodingJobs: 0,
  ...
  it('test_AC8_header_shows_encoding_only_count_and_never_the_word_active', async () => {
    // The reported v2.44.0 state: 4 encoding, 995 queued. The header used to read
    // "999 active · 995 queued".
    mockUseQueueCounts.mockReturnValue({
      activeJobs: 999,
      pendingJobs: 995,
      encodingJobs: 4,
    ...
    await renderPage();
    expect(screen.getByText('4 encoding · 995 queued')).toBeInTheDocument();
    expect(screen.queryByText(/999 active/)).toBeNull();
    expect(screen.queryByText(/\bactive\b ·/)).toBeNull();
  });

  it('test_AC13_seed_gate_reads_encodingJobs_not_activeJobs', async () => {
    // liveActiveJobs empty, encodingJobs 0, activeJobs 995 (all queued) → the panel
    // gets an empty job list, so the idle headline shows instead of a phantom row.
    mockListActive.mockReturnValue([jobFix({ id: 30, file_id: 7, status: 'queued' })]);
    mockUseActiveJobs.mockReturnValue([]);
    mockUseQueueCounts.mockReturnValue({
      activeJobs: 995,
      pendingJobs: 995,
    ...
    mockGetById.mockReturnValue({ id: 7, path: '/m/z.mkv', duration_seconds: 60, size_bytes: 10 });
    await renderPage();
    expect(screen.getByText(en.queue.active.idleHeadline)).toBeInTheDocument();
    expect(screen.queryByText(en.queue.active.panelTitle.replace('{count}', '1'))).toBeNull();
  });
