...
describe('SettingsPage — Encoder tab (Plan 03-03)', () => {
  ...
  // 43-01 (AC-7 / SR-1): force-10bit checkbox renders, defaults unchecked, and
  // its value flows into the PUT /api/settings payload. The default-unchecked
  // assertion is the UI-side guarantee of the AC-1 byte-identical contract.
  it('test_encoderTab_when_rendered_then_force10bit_switch_present_and_default_unchecked', async () => {
    ...
    // Default-unchecked = byte-identical pre-43 for fresh installs.
  ...
  // 43-03 AC-6: color-passthrough switch renders default-unchecked + round-trips
  ...
  it('test_encoderTab_when_colorPassthrough_toggled_then_PUT_payload_carries_color_passthrough_true', async () => {
    ...
    // AC-6: default-unchecked (byte-identical default).
    expect(sw).toHaveAttribute('aria-checked', 'false');
    await act(async () => {
      fireEvent.click(sw); // unchecked → checked, makes form dirty
    });
