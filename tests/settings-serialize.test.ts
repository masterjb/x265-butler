...
// 43-01: force_10bit bool → 'true'/'false' (mirrors auto_crop). Absent = byte-
// identical pre-43 (AC-6 serialize-half).
describe('serializeForApi — force_10bit (Plan 43-01)', () => {
  it('test_serializeForApi_when_force_10bit_true_then_string_true', () => {
    expect(serializeForApi({ force_10bit: true })).toEqual({ force_10bit: 'true' });
  });

  it('test_serializeForApi_when_force_10bit_false_then_string_false', () => {
    expect(serializeForApi({ force_10bit: false })).toEqual({ force_10bit: 'false' });
  });

  it('test_serializeForApi_when_force_10bit_absent_then_not_in_output', () => {
    const out = serializeForApi({ encoder: 'qsv' });
    expect(out.force_10bit).toBeUndefined();
  });
});

// 43-03: color_passthrough bool → 'true'/'false' (mirrors force_10bit). Absent =
// byte-identical (AC-6 serialize-half).
describe('serializeForApi — color_passthrough (Plan 43-03)', () => {
  it('test_serializeForApi_when_color_passthrough_true_then_string_true', () => {
    expect(serializeForApi({ color_passthrough: true })).toEqual({ color_passthrough: 'true' });
  });

  it('test_serializeForApi_when_color_passthrough_false_then_string_false', () => {
    expect(serializeForApi({ color_passthrough: false })).toEqual({ color_passthrough: 'false' });
  });

  it('test_serializeForApi_when_color_passthrough_absent_then_not_in_output', () => {
    const out = serializeForApi({ encoder: 'qsv' });
    expect(out.color_passthrough).toBeUndefined();
  });
});
