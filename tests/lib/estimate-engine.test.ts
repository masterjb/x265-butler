...
describe('runEstimate (13-04 T2)', () => {
  ...
  // 28-04 (L7): hash-failure file lands in eligible (size-only, null duration) AND
  // the warn still fires — proving the extracted classifyEstimateEntry hash-failed
  // branch matches the old inline catch byte-for-byte.
  it('case 16 (L7): hashFile rejects → eligible (size-only, null duration) + warn fires', async () => {
    mockWalk.mockImplementation(makeWalker([entry('/m/a.mp4', 7_000_000_000)]));
    mockHash.mockRejectedValueOnce(new Error('I/O read error'));
    const result = await runEstimate(
      { rootPath: '/m', extensions: ['mp4'], minSizeMb: 1, encoder: 'libx265' },
      makeRepos(),
    );
    ...
    expect(result.skipBuckets.sidecar).toBe(0);
    expect(result.skipBuckets.blocklist).toBe(0);
    expect(result.savings.totalBytes).toBe(7_000_000_000);
    // duration unknown → no with-duration contribution
    expect(result.encodeTime.withDurationCount).toBe(0);
    // ffprobe never reached — hash failed first
    expect(mockFfprobe).not.toHaveBeenCalled();
    const warnCall = mockLoggerWarn.mock.calls.find(
      (c) => typeof c[1] === 'string' && c[1].includes('hash failed'),
    );
    expect(warnCall).toBeTruthy();
  });
