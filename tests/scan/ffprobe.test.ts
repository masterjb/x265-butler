...
import { ffprobe, parseFrameRate, countVideoPackets } from '@/src/lib/scan/ffprobe';
...
describe('ffprobe', () => {
  ...
  it('test_ffprobe_when_format_missing_optional_fields_then_returns_nulls', async () => {
    ...
    expect(result).toEqual({
      ...
      color: { space: null, primaries: null, transfer: null, range: null },
      hdr10: { masterDisplay: null, maxCll: null },
      streams: [{ attachedPic: false, index: 0, codec_type: 'video', codec_name: 'hevc' }],
    });
  });
  ...
  // 49-01 (AC-1): the attached_pic disposition flag rides the -show_streams JSON
  // that ffprobe already returns — no new argv flag, no second spawn.
  it('test_ffprobe_when_attached_pic_disposition_then_sets_attachedPic_per_stream', async () => {
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const promise = ffprobe('/path/to/cover.mkv');
    child.stdout.emit(
      'data',
      Buffer.from(
        JSON.stringify({
          streams: [
            // disposition present, attached_pic 0 → false
            {
              codec_type: 'video',
              codec_name: 'h264',
              width: 1920,
            ...
            // disposition object entirely absent → false
            { codec_type: 'audio', codec_name: 'aac' },
            // the cover: attached_pic 1 → true
            {
              codec_type: 'video',
              codec_name: 'mjpeg',
              width: 600,
    ...
    child.emit('close', 0);
    const result = await promise;
    expect(result?.streams?.map((s) => s.attachedPic)).toEqual([false, false, true]);
    // tags are UPPER-normalized like format tags, and undefined when absent.
    expect(result?.streams?.[2].tags).toEqual({ FILENAME: 'cover.jpg', MIMETYPE: 'image/jpeg' });
    expect(result?.streams?.[1].tags).toBeUndefined();
  });

  it('test_ffprobe_when_attached_pic_present_then_argv_unchanged', async () => {
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const promise = ffprobe('/path/to/cover.mkv');
    child.stdout.emit(
      ...
      Buffer.from(
        JSON.stringify({
          streams: [{ codec_type: 'video', codec_name: 'h264', disposition: { attached_pic: 1 } }],
          format: {},
        }),
      ),
    );
    child.emit('close', 0);
    await promise;
    // AC-1: exactly one spawn, and the argv carries no new flag — the
    // disposition object is part of what -show_streams already emits.
    expect(spawnMock).toHaveBeenCalledOnce();
    const args = spawnMock.mock.calls[0][1] as string[];
    expect(args).toContain('-show_streams');
    expect(args.some((a) => a.includes('disposition'))).toBe(false);
  });
});

// ── 50-01: frame-gate probe layer ─────────────────────────────────────────────

describe('parseFrameRate (50-01, AC-11)', () => {
  it.each([
    ['24/1', 24],
    ...
    ['0/1', undefined],
    ['  25/1  ', 25],
  ])('test_parse_frame_rate_when_%s_then_expected', (raw, expected) => {
    expect(parseFrameRate(raw as string | undefined)).toBe(expected);
  });

  it('test_parse_frame_rate_never_returns_nan_or_infinity', () => {
    for (const raw of ['0/0', 'N/A', '', 'x/y', '1/0', 'NaN', 'Infinity', '1/', '/1']) {
      const v = parseFrameRate(raw);
      expect(v === undefined || (Number.isFinite(v) && v > 0)).toBe(true);
    }
  });

  it('test_ffprobe_when_avg_frame_rate_present_then_surfaced_on_stream', async () => {
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const promise = ffprobe('/path/to/video.mkv');
    child.stdout.emit(
      'data',
      Buffer.from(
        JSON.stringify({
          ...
          format: { duration: '30.023' },
        }),
      ),
    );
    child.emit('close', 0);
    const result = await promise;
    expect(result?.streams?.[0].avgFrameRate).toBeCloseTo(23.976, 3);
    // '0/0' (the audio sentinel) must NOT become 0 or NaN.
    expect(result?.streams?.[1].avgFrameRate).toBeUndefined();
    // AC-1 style guard: no new ffprobe argument was added for this field.
    const args = spawnMock.mock.calls[0][1] as string[];
    expect(args.some((a) => a.includes('avg_frame_rate'))).toBe(false);
  });
});

describe('countVideoPackets (50-01, AC-12)', () => {
  it('test_count_video_packets_when_happy_path_then_returns_packet_count', async () => {
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const promise = countVideoPackets('/stage/out.mkv');
    child.stdout.emit('data', Buffer.from('17\n'));
    child.emit('close', 0);
  ...
  it('test_count_video_packets_when_called_then_argv_is_exact', async () => {
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const promise = countVideoPackets('/stage/out with spaces.mkv');
    child.stdout.emit('data', Buffer.from('720\n'));
    child.emit('close', 0);
    await promise;
    expect(spawnMock).toHaveBeenCalledOnce();
    expect(spawnMock.mock.calls[0][0]).toBe('ffprobe');
    expect(spawnMock.mock.calls[0][1]).toEqual([
      ...
      '-of',
      'csv=p=0',
      '/stage/out with spaces.mkv',
    ]);
    expect((spawnMock.mock.calls[0][2] as { shell?: unknown }).shell).toBeUndefined();
  });

  // audit MH-2: the ordinal must reach the selector — a hardcoded v:0 would count
  // the cover stream on an output whose cover is video ordinal 0.
  it('test_count_video_packets_when_video_ordinal_1_then_selects_v1', async () => {
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const promise = countVideoPackets('/stage/out.mkv', { videoOrdinal: 1 });
    child.stdout.emit('data', Buffer.from('720\n'));
    child.emit('close', 0);
    await promise;
    const args = spawnMock.mock.calls[0][1] as string[];
  ...
  it.each([[-1], [1.5], [NaN]])(
    'test_count_video_packets_when_ordinal_%s_then_null_and_no_spawn',
    async (ordinal) => {
      await expect(countVideoPackets('/stage/out.mkv', { videoOrdinal: ordinal })).resolves.toBe(
        null,
      );
  ...
  it('test_count_video_packets_when_spawn_errors_then_null', async () => {
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const promise = countVideoPackets('/nope/missing.mkv');
    child.emit('error', new Error('spawn ffprobe ENOENT'));
    await expect(promise).resolves.toBe(null);
  });

  it('test_count_video_packets_when_nonzero_exit_then_null', async () => {
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const promise = countVideoPackets('/stage/out.mkv');
  ...
  it('test_count_video_packets_when_output_unparsable_then_null', async () => {
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const promise = countVideoPackets('/stage/out.mkv');
  ...
  it('test_count_video_packets_when_empty_output_then_null', async () => {
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const promise = countVideoPackets('/stage/out.mkv');
    child.emit('close', 0);
  ...
  it('test_count_video_packets_when_timeout_then_null_and_killed', async () => {
    const child = new FakeChild();
    spawnMock.mockReturnValueOnce(child);
    const promise = countVideoPackets('/stage/out.mkv', { timeoutMs: 5 });
