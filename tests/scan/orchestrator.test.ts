...
// 48-02 (AC-15): the prune list is ABSOLUTE, a fixture tree is not.
import {
...
describe('runScan', () => {
  ...
  // 33-01 (audit-SR-4b): the scan call site must thread sidecar_mode/sidecar_central_path
  // into runSkipPipeline so central-mode operators get loop-protection on scan (AC-6
  // scan side). End-to-end: a central sidecar (matching the disk hash) → file is skipped.
  it('test_runScan_when_mode_central_then_central_sidecar_skips_the_file', async () => {
    const centralRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'scan-central-skip-'));
    try {
      settingRepoFn().set('sidecar_mode', 'central');
      settingRepoFn().set('sidecar_central_path', centralRoot);
      ...
      // disk hash is mocked to 'h'*64 (beforeEach) → central sidecar must carry it.
      const payload: SidecarV2 = {
        ...
        source: { filename: 'a.mp4', contentHash: 'h'.repeat(64), sizeBytes: ABOVE_MIN },
        output: { filename: 'a.x265.mkv', contentHash: 'o'.repeat(64), sizeBytes: 100 },
        encoder: 'libx265',
        quality: { mode: 'crf', value: 23 },
        outcome: 'done-smaller',
      };
      await writeSidecarResolved(filePath, payload, 'central', centralRoot);

      await runScan({ rootPath: tmpdir, extensions: ['mp4'], minSizeMb: 1 }, repo);

      // central sidecar reached the pipeline → row marked skipped-sidecar (NOT pending/queued).
      const row = repo.findByContentHash('h'.repeat(64));
...
describe('runScan multi-share (14-02)', () => {
  ...
  it('test_runScan_when_one_share_throws_then_others_continue', async () => {
    ...
    expect(result.byShare![0]).toEqual({
      ...
      filesUnchanged: 0,
      filesFailed: 0,
      // 48-01 (AC-9): the zeroed failure entry carries the directory counters
      // too, so byShare.length === shares.length stays type-uniform.
      // 48-02 added the fifth (dirsSkippedSystemPrefix).
      dirsVisited: 0,
      dirsSkippedCycle: 0,
...
// ── 48-01: directory-integrity counters + scan-integrity store ───────────────
describe('runScan scan-integrity (48-01)', () => {
  let tmpdir: string;
  let db: Db;
  let repo: FileRepo;
  let shares: ShareRepo;
  ...
  beforeEach(() => {
    tmpdir = fs.mkdtempSync(path.join(os.tmpdir(), 'scan-integrity-'));
    db = new Database(':memory:');
    db.pragma('foreign_keys = ON');
    migrate(db);
    db.prepare('DELETE FROM shares').run();
  ...
  it('test_scanIntegrityStore_before_first_scan_then_null', () => {
    // AC-11: "never scanned" must be distinguishable from "scanned, 0 skips".
    expect(getScanIntegrity()).toBeNull();
  });

  it('test_runScan_when_multi_share_then_byShare_and_toplevel_counters_agree', async () => {
    // AC-8: every byShare entry carries its OWN walk's counters and the
    // top-level counters are the sum across them.
    const aDir = path.join(tmpdir, 'a');
    const bDir = path.join(tmpdir, 'b');
    writeSized(path.join(aDir, 'sub1', 'x.mp4'), ABOVE_MIN);
    writeSized(path.join(bDir, 'sub1', 'sub2', 'y.mp4'), ABOVE_MIN);
    shares.create({
      name: 'A',
      path: aDir,
      min_size_mb: 1,
    ...
    expect(result.byShare).toHaveLength(2);
    expect(result.byShare![0].dirsVisited).toBe(2); // a + a/sub1
    expect(result.byShare![1].dirsVisited).toBe(3); // b + b/sub1 + b/sub1/sub2
    expect(result.dirsVisited).toBe(result.byShare!.reduce((acc, s) => acc + s.dirsVisited, 0));
    expect(result.dirsSkippedCycle).toBe(0);
    expect(result.dirsSkippedUnreadable).toBe(0);
    expect(result.dirsSkippedMaxDepth).toBe(0);

    // AC-10: the FILE-axis invariant is untouched by the new directory axis.
    expect(
      result.filesAdded + result.filesUpdated + result.filesUnchanged + result.filesFailed,
    ).toBe(result.filesScanned);
    expect(result.filesScanned).toBe(2);
  ...
  it('test_runScan_when_share_throws_then_store_row_marked_failed', async () => {
    // AC-9 + audit-added M1: a zeroed row alone cannot be told apart from an
    // empty share — the failed flag and sharesFailed carry that information.
    const goodDir = path.join(tmpdir, 'good');
    writeSized(path.join(goodDir, 'c.mp4'), ABOVE_MIN);
    shares.create({
      name: 'Bad',
      path: '/nonexistent-share-path-48-01',
      min_size_mb: 1,
      extensions_csv: 'mp4',
      max_depth: null,
    });
    ...
    expect(result.byShare).toHaveLength(2);
    expect(result.byShare![0].dirsVisited).toBe(0);

    const snap = getScanIntegrity();
    expect(snap).not.toBeNull();
    expect(snap!.outcome).toBe('complete'); // runScan itself did NOT throw
    expect(snap!.sharesFailed).toBe(1);
    expect(snap!.byShare.map((s) => ({ name: s.name, failed: s.failed }))).toEqual([
      { name: 'Bad', failed: true },
      { name: 'Good', failed: false },
    ]);
    // audit-added M4: opts.rootPath is pro-forma in multi-share mode.
    expect(snap!.rootPath).toBeNull();
  });

  it('test_runScan_when_single_share_then_store_carries_rootPath_and_timestamps', async () => {
    // AC-11 + AC-16 + M4 (single-share arm): rootPath IS meaningful here, and
    // both timestamps are present so overlapping scans stay reconstructable.
    writeSized(path.join(tmpdir, 'sub', 'a.mp4'), ABOVE_MIN);

    await runScan({ rootPath: tmpdir, extensions: ['mp4'], minSizeMb: 1 }, repo);

    const snap = getScanIntegrity()!;
    expect(snap.outcome).toBe('complete');
    expect(snap.rootPath).toBe(tmpdir);
    expect(snap.dirsVisited).toBe(2); // root + sub
    expect(snap.sharesFailed).toBe(0);
    expect(snap.byShare).toEqual([
      {
        shareId: null,
        name: '(fallback)',
        rootPath: tmpdir,
    ...
    expect(Date.parse(snap.startedAtIso)).not.toBeNaN();
    expect(Date.parse(snap.finishedAtIso)).not.toBeNaN();
    expect(Date.parse(snap.finishedAtIso)).toBeGreaterThanOrEqual(Date.parse(snap.startedAtIso));
  });

  it('test_runScan_when_single_share_throws_then_store_says_failed_and_rethrows', async () => {
    // AC-15: the boot auto-scan path SWALLOWS a runScan throw
    // (reconcile.ts:69-77), so without the failure snapshot the diagnostics
    // surface would keep presenting the previous successful scan as current.
    writeSized(path.join(tmpdir, 'sub', 'a.mp4'), ABOVE_MIN);
    await runScan({ rootPath: tmpdir, extensions: ['mp4'], minSizeMb: 1 }, repo);
    expect(getScanIntegrity()!.dirsVisited).toBe(2); // stale evidence candidate

    const deadRoot = path.join(tmpdir, 'gone-48-01');
    await expect(
      runScan({ rootPath: deadRoot, extensions: ['mp4'], minSizeMb: 1 }, repo),
    ).rejects.toThrow(/not accessible/);

    const snap = getScanIntegrity()!;
    expect(snap.outcome).toBe('failed');
    expect(snap.sharesFailed).toBe(1);
    expect(snap.rootPath).toBe(deadRoot);
    expect(snap.byShare).toEqual([]);
    // AC-11/AC-15: fully overwritten — NOT the previous run's numbers.
    expect(snap.dirsVisited).toBe(0);
  });
});

// ── 48-02 (D1=C): the prune counter must reach the diagnostics surface ───────
describe('runScan system-prefix prune propagation (48-02)', () => {
  let tmpdir: string;
  let db: Db;
  let repo: FileRepo;
  let shares: ShareRepo;
  ...
  beforeEach(() => {
    tmpdir = fs.mkdtempSync(path.join(os.tmpdir(), 'scan-prune-'));
    db = new Database(':memory:');
    db.pragma('foreign_keys = ON');
    migrate(db);
    db.prepare('DELETE FROM shares').run();
  ...
  it('test_runScan_when_share_root_encloses_system_dirs_then_counter_reaches_result_and_store', async () => {
    // AC-9: WalkStats → PerShareCounters → ScanResult(.byShare) →
    // ScanIntegritySnapshot. The reporter's shape: one share enclosing the
    // pseudo-filesystems.
    const shareRoot = path.join(tmpdir, 'root');
    for (const sysdir of ['proc', 'sys', 'dev', 'run']) {
      writeSized(path.join(shareRoot, sysdir, 'trap.mp4'), ABOVE_MIN);
    }
    writeSized(path.join(shareRoot, 'mnt', 'movie.mp4'), ABOVE_MIN);
    __setPruneSystemPrefixesForTests(
      ['proc', 'sys', 'dev', 'run'].map((d) => path.join(shareRoot, d)),
    );
    shares.create({
      name: 'Everything',
      path: shareRoot,
    ...
    expect(result.filesScanned).toBe(1); // only the /mnt file survived
    expect(result.dirsSkippedSystemPrefix).toBe(4);
    expect(result.byShare![0].dirsSkippedSystemPrefix).toBe(4);

    const snap = getScanIntegrity()!;
    expect(snap.dirsSkippedSystemPrefix).toBe(4);
    expect(snap.byShare[0].dirsSkippedSystemPrefix).toBe(4);
    expect(snap.systemPrefixSamples).toHaveLength(4);
    // AC-18: the deliberate prune does NOT masquerade as unintended loss.
    expect(snap.dirsSkippedCycle).toBe(0);
    expect(snap.dirsSkippedUnreadable).toBe(0);
  });

  it('test_runScan_when_nothing_pruned_then_counter_is_zero_not_absent', async () => {
    // AC-9: an ABSENT field would be indistinguishable from an older build.
    writeSized(path.join(tmpdir, 'sub', 'a.mp4'), ABOVE_MIN);
    const result = await runScan({ rootPath: tmpdir, extensions: ['mp4'], minSizeMb: 1 }, repo);
    expect(result.dirsSkippedSystemPrefix).toBe(0);
    expect(getScanIntegrity()!.dirsSkippedSystemPrefix).toBe(0);
    expect(getScanIntegrity()!.systemPrefixSamples).toEqual([]);
  });

  it('test_runScan_when_kill_switch_zero_then_counter_stays_zero_and_files_are_scanned', async () => {
    // AC-8 end-to-end through the orchestrator, not just the walker.
    process.env.SCAN_PRUNE_SYSTEM_PATHS = '0';
    __resetSystemPruneMemoForTests();
    const shareRoot = path.join(tmpdir, 'root');
    writeSized(path.join(shareRoot, 'proc', 'trap.mp4'), ABOVE_MIN);
    __setPruneSystemPrefixesForTests([path.join(shareRoot, 'proc')]);
    shares.create({
      name: 'Everything',
      path: shareRoot,
      min_size_mb: 1,
    ...
    const result = await runScan({ rootPath: '<ignored>', extensions: [], minSizeMb: 0 }, repo);
    expect(result.filesScanned).toBe(1); // the trap file WAS scanned
    expect(result.dirsSkippedSystemPrefix).toBe(0);
    expect(getScanIntegrity()!.dirsSkippedSystemPrefix).toBe(0);
  });
});
