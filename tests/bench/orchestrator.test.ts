...
describe('BenchOrchestrator._executeRun (native-sweep)', () => {
  ...
  // 28-08 AC-3: P1 (parallel encode) is DROPPED — the encode loop must stay serial.
  // Anti-regression pin: combo-2's encode never starts until combo-1's settles, so
  // mockEncode is never concurrently in-flight (max-in-flight === 1). RED-proof:
  // wrapping the encode loop in Promise.all makes both increment before either
  // decrements → max === 2 (verified locally then reverted).
  it('dispatches encodeForBench strictly serially — max in-flight === 1 (P1 dropped)', async () => {
    const { benchRunRepo, benchComboRepo } = makeMockRepos();
    // Two combos in the same sample (file 10, sample_idx 0).
    benchComboRepo.listPendingByRun.mockReturnValue([
      makeComboRow({ id: 1 }),
      makeComboRow({ id: 2 }),
    ]);
    benchRunRepo.findById
    ...
    let inFlight = 0;
    let maxInFlight = 0;
    mockEncode.mockImplementation(async () => {
      inFlight++;
      maxInFlight = Math.max(maxInFlight, inFlight);
      // AUTO-resolve after yielding one tick (NOT an externally-held deferred —
      // that would deadlock a serial loop since combo-2 never dispatches).
      await Promise.resolve();
      inFlight--;
      return { sizeBytes: 100_000, encodeSec: 1.5 };
    });
  ...
  // 28-08 AC-7 (audit MH-1): the shared completedCombos counter (moved into a
  // threaded `progress` ref) must bump EXACTLY once per combo on BOTH the success
  // and the mid-encode-failure path — never twice (helper++ AND caller++), never
  // missed. The single-combo `> 0` sentinel above cannot catch a double/missed
  // increment; this success+failure pair pins monotonic +1 → final === total.
  it('progress counter bumps exactly once per combo — success + failure pair', async () => {
    // bench.progress is throttled to 1Hz per run (_emitProgress); two combos in the
    // same tick would collapse to a single emit. Advance the fake clock >1s on each
    // encode so BOTH increments are observable.
    vi.useFakeTimers();
    try {
      const { benchRunRepo, benchComboRepo } = makeMockRepos();
      benchComboRepo.listPendingByRun.mockReturnValue([
      ...
      // combo-1 encode resolves, combo-2 encode REJECTS — exercises both the
      // success bump (:670) and the mid-encode-failure bump (:693).
      let call = 0;
      mockEncode.mockImplementation(async () => {
        vi.advanceTimersByTime(2000); // clear the 1Hz progress throttle
        call++;
        if (call === 1) return { sizeBytes: 100_000, encodeSec: 1.5 };
      ...
      const unsub = engineEvents.subscribe((ev) => {
        if (ev.type === 'bench.progress') {
          completed.push(ev.completedCombos);
          totals.push(ev.totalCombos);
        }
      });
      ...
      // Exactly twice, monotonic +1, final === totalCombos === 2. A double-bump
      // (helper++ AND caller++) → [2, 4]; a missed failure-bump → [1, 1] or [1].
