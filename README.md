...
- Dense, dark-mode-first dashboard with live SSE progress, concurrent-job visibility, cumulative savings stats
- Optional username/password auth (off by default), EN/DE i18n via next-intl
- Self-diagnostics page with hardware-encoder probe, render-node permission evidence, and copyable bug-report
...
- **`:X.Y.Z`** — Exact-semver pin (e.g. `ghcr.io/masterjb/x265-butler:2.21.0`). Reproducible, frozen at the build that created it. Recommended for production-stability operators.
...
> - CA template: Edit Container → set `Repository` to `ghcr.io/masterjb/x265-butler:2.21.0`
> - `docker-compose.yml`: `image: ghcr.io/masterjb/x265-butler:2.21.0`
