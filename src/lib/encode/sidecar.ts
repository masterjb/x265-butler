...
export const SIDECAR_SUFFIX = '.x265-butler.json';
export const SIDECAR_TMP_SUFFIX = '.x265-butler.json.tmp';

// 33-01 (audit-SR-1): SINGLE source of truth for the central-path default.
// The 26-01 WRITE side defaults sidecar_central_path to this same string
// (encode/orchestrator.ts:770 + scan/orchestrator.ts:125). The 33-01 READ side
// (skip-pipeline default + readSidecarResolved fallback) MUST consume THIS const,
// never a re-typed literal — read-default drift from write-default = silent
// central-read-miss = the re-queue bug returns (AC-7).
export const DEFAULT_SIDECAR_CENTRAL_PATH = '/config/x265-butler/sidecars/';
...
export async function readSidecar(targetPath: string): Promise<SidecarPayload | null> {
  return readSidecarAtPath(sidecarPathFor(targetPath));
}

/**
 * 33-01: read-side resolver — mirror of writeSidecarResolved (write side).
 * Returns the sidecar payload for `targetPath` honoring `mode`:
 *   - off / beside / (anything not 'central') → beside read (byte-identical to
 *     pre-33-01 readSidecar) — covers the off + beside + unset cases.
 *   - central → resolve the mirrored central path via resolveSidecarTarget and
 *     read it FIRST; on a central miss (no file) D1=B falls back to a beside read
 *     so a beside→central mode-switcher's existing library is still protected.
 *
 * AC-5: a resolveSidecarTarget traversal-guard throw soft-degrades (warn +
 * beside fallback) and NEVER propagates into the skip-pipeline.
 */
export async function readSidecarResolved(
  targetPath: string,
  mode: SidecarMode,
  centralRoot: string,
): Promise<SidecarPayload | null> {
  if (mode !== 'central') {
    return readSidecarAtPath(sidecarPathFor(targetPath));
  }

  let centralPath: string | null;
  try {
    centralPath = resolveSidecarTarget(targetPath, 'central', centralRoot);
  } catch (err) {
    // AC-5: traversal-guard throw — soft-degrade, fall through to beside.
    logger.warn(
      {
        action: 'sidecar_read_resolve_failed',
        targetPath,
        mode,
        err: err instanceof Error ? err.message : String(err),
      },
      'central sidecar target resolution failed — falling back to beside read',
    );
    return readSidecarAtPath(sidecarPathFor(targetPath));
  }

  // resolveSidecarTarget returns the suffixed FINAL path → feed it to
  // readSidecarAtPath directly (do NOT re-append SIDECAR_SUFFIX).
  if (centralPath !== null) {
    const hit = await readSidecarAtPath(centralPath);
    if (hit) return hit;
  }
  // D1=B: central miss → beside fallback (AC-3).
  return readSidecarAtPath(sidecarPathFor(targetPath));
}

/**
 * 33-01: read + size-cap + schema-version gate for a FINAL sidecar path (suffix
 * already appended). Extracted verbatim from readSidecar so both the beside
 * helper and readSidecarResolved share ONE implementation. Returns null on
 * missing / oversize / unparseable / unknown-schema.
 */
async function readSidecarAtPath(sidecarPath: string): Promise<SidecarPayload | null> {
