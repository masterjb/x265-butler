...
export {
  ...
  // 49-03: the forced-IDR verdict accessor. Consumed by the diagnostics
  // test-encode runner so it resolves the SAME value the production buildArgs
  // resolves (D7/AC-13) — otherwise the test encode would validate an argv the
...
export {
  ...
  PROBE_FRAME_SIZE,
  // 49-03: shared synthetic-probe pixel-format pin + the predicate for the
  ...
  // 49-05: per-encoder factory CRF defaults. Re-exported through profiles.ts
  ...
  // this barrel pulls orchestrator + detection (audit MH-1).
  DEFAULT_CRF_BY_ENCODER,
} from './profiles';
export type { CodecBlockInput, EncodeProfileInput } from './profiles';
// 50-06: the shared settings resolvers — ONE source for "which encoder / which
...
// 50-02 additive: the cover-art attach surface. attached-pic.ts is the PURE half
