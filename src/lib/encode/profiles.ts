...
export const PROFILE_BUILDERS: Record<EncoderId, ProfileBuilder> = {
  ...
  qsv: (crf, preset, _devicePath, qsvRateControl) =>
    qsvRateControl === 'cqp'
      ? ['-c:v', 'hevc_qsv', '-preset', resolvePreset('qsv', preset), '-q:v', String(crf)]
      : [
          '-c:v',
          'hevc_qsv',
          '-preset',
...
export function buildCodecBlock(input: CodecBlockInput): string[] {
  ...
  return builder(input.crf, input.preset, input.devicePath, input.qsvRateControl);
}
