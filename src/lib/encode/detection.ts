...
// audit-added M1: the excerpt MUST be the TAIL of stderr — ffmpeg prints the
// fatal line last; a head-slice would capture only banner/config noise and
// DESTROY the evidence this plan exists to surface. Collapse internal whitespace
// runs first so the 240-char budget carries signal, not blank lines.
function tailExcerpt(stderr: string | undefined): string {
  if (!stderr) return '';
  return stderr.replace(/\s+/g, ' ').trim().slice(-240);
}
...
async function probeVaInfo(driPresent: boolean): Promise<VaInfoProbeResult> {
  ...
  const out = r.stdout;
  const hasIhd = out.includes('iHD');
  const hasI965 = out.includes('i965');
  const hasEncEntrypoints = out.includes('VAEntrypointEncSlice');
  if (hasIhd) {
    return { qsv: true, vaapi: false };
  }
  if (hasEncEntrypoints && hasI965) {
    // Legacy Intel iGPU — i965 path; VAAPI works, AV1 unavailable.
    return {
      qsv: false,
      vaapi: true,
      warning: {
  ...
  if (hasEncEntrypoints) {
    // Mesa / AMD path — no warning.
    return { qsv: false, vaapi: true };
  }
  // Driver loaded but no encode entrypoints; treat as dri-present-no-driver
  // when /dev/dri was visible.
  if (driPresent) {
    return {
      qsv: false,
      vaapi: false,
