...
async function resolveVaapi(): Promise<{
  vaapiDevice?: string;
  qsv: boolean;
  vaapi: boolean;
  warning?: DetectionWarning;
  ...
  return { vaapiDevice, qsv: probe.qsv, vaapi: probe.vaapi, warning: probe.warning };
}
...
async function runDetection(): Promise<DetectionResult> {
  ...
  const [nv, va] = await Promise.all([resolveNvenc(), resolveVaapi()]);
  ...
  if (probeEncodeDisabled) {
    ...
    const settled = await Promise.all(
      candidates.map((enc) =>
        probeEncodeFunctional(enc, enc === 'vaapi' ? vaapiDevice : undefined).then((res) => ({
          enc,
          res,
        })),
      ),
...
export function __forTests_buildProbeEncodeArgs(encoder: EncoderId, devicePath?: string): string[] {
  return buildProbeEncodeArgs(encoder, devicePath);
}
