...
// 45-02 (audit-S1): SINGLE-SOURCE the NVENC API-floor stderr match. This module
// (a PURE, dep-free dictionary — imports only ./types) is cycle-safe to pull in.
import {
  isNvencApiTooNewStderr,
  isNvencHevcUnsupportedStderr,
  // 49-03 (audit M-1): the forced-IDR rejection signature lives in the SAME
  // single-source dictionary — never a second inline regex here.
  isForcedIdrRejectedStderr,
} from '@/src/lib/diagnostics/test-encode-error-map';
// 49-03: the confirm stage only fires when the pin is actually enabled. keyframe.ts
// is a leaf (imports only ../logger), so this edge is acyclic.
import { closedGopEnabled } from './keyframe';
...
export type DetectionWarningCode =
  ...
  // 45-02: nvenc probe stderr shows the compile-baked NVENC API floor exceeds the
  // GPU's ("Required: 13.1 Found: 13.0") — Pascal/Maxwell on their last driver;
  // distinct from encoder_runtime_broken (a NOT-updatable hardware-generation
  // limit, not a generic runtime break).
  | 'nvenc_api_too_new'
  // 46-01: nvenc probe open fails EINVAL with no more-specific reason → GPU has no
  // HEVC-NVENC encode block (pre-Maxwell-2). Distinct hardware-generation limit;
  // stays compiled-in-broken.
  | 'nvenc_hevc_unsupported'
  // 49-03 (Q2=A): the encoder WORKS, but this runtime rejects the closed-GOP
  // (`-forced_idr` / `-forced-idr`) option 49-02 emits in production. Butler has
  // auto-degraded THIS encoder to open GOPs. Deliberately NOT
  // `encoder_runtime_broken` (precedent 45-02 / 46-01: a distinct, narrower cause
  // gets its own code) — the encoder is functional and STAYS in detected[]; only
  // one option is unavailable.
  | 'forced_idr_unsupported';
...
export interface DetectionResult {
  ...
  // 49-03 (Q1=B): per-encoder verdict of the forced-IDR CONFIRM spawn. Stable-shape
  // style mirroring `brokenExcerpts`: the object is ALWAYS present, and a key exists
  // ONLY where the confirm spawn produced a definitive answer.
  //   true    = the confirm spawn ran and exited 0 → the runtime accepts the option.
  //   false   = the confirm spawn exited non-zero AND its stderr carried the option-
  //             rejection signature → auto-degrade this encoder to open GOPs.
  //   ABSENT  = nothing was proven (kill-switch, probe-inconclusive, timeout/ENOENT,
  //             encoder ∉ {qsv,nvenc}, closed-GOP disabled, unclassifiable stderr).
  //             Read via isForcedIdrSupported() ⇒ fail-OPEN true (D4).
  // PER-ENCODER, NOT GLOBAL: on a dual-GPU unRAID host (Intel iGPU + NVIDIA dGPU —
  // not exotic) a single boolean would let a qsv rejection cross-degrade nvenc, and
  // in the parallel fan-out the last writer would decide.
  forcedIdrSupported: Partial<Record<EncoderId, boolean>>;
}
...
// 49-03 (G3): pin `-pix_fmt nv12` for qsv/nvenc. The lavfi source stays UNCHANGED
// — the knob belongs in the codec block, only the SETTER is this builder (D5=B).
// Without the pin, `testsrc`'s rgb24 reaches a modern `hevc_qsv` that advertises
// RGB in `pix_fmts`, so ffmpeg inserts no converter and iHD rejects it → this
// probe reports `compiled-in-broken` on a host whose real encodes work.
// 49-03 (AC-9): `forceIdr` threads through for the CONFIRM spawn only — every
// tier spawn leaves it false, so their argv stays byte-identical to v2.46.0.
...
interface ProbeEncodeOutcome {
  ...
  qsvRateControl?: QsvRateControl;
  // 49-03: verdict of the forced-IDR confirm spawn. undefined = nothing proven
  // (fail-OPEN, D4). Rides on this outcome exactly the way qsvRateControl does, so
  // the parallel fan-out in runDetection stays untouched and the spawn budget
  // stays inside the per-candidate chain (AC-16 structurally, not by assertion).
  forcedIdrSupported?: boolean;
  // 49-03: the `forced_idr_unsupported` warning, kept SEPARATE from `warning`
  // above — that one belongs to the compiled-in-broken path, which runDetection
  // only reads when `detected` is false. A confirm rejection leaves the encoder
  // detected AND functional, so its warning needs its own carrier.
  forcedIdrWarning?: DetectionWarning;
}
...
async function probeEncodeOnce(
  ...
  // 45-01: nvenc probe uses the jellyfin binary (else Pascal probe fails on BtbN
  // → false-negative gates nvenc out). Binary-selection ONLY; error-mapping = 45-02.
  const r = await runEncodeProbe(
    ffmpegBinaryFor(encoder),
  ...
  if (r.cause === 'EXIT_NONZERO') {
    ...
    // 45-02: the nvenc probe stderr may carry the compile-baked API floor
    // ("Required: 13.1 Found: 13.0"). Classify it as nvenc_api_too_new (a
    // hardware-generation limit) rather than the generic encoder_runtime_broken.
    // Match the RAW pre-tailExcerpt stderr via the SINGLE-SOURCE predicate (never
    // a second inline regex — audit-S1 maintenance invariant). outcome STAYS
    // 'compiled-in-broken' + detected false: the binary genuinely can't encode
    // on this GPU; only the warning CODE differs.
    // 46-01: chain the nvenc HEVC-unsupported classifier AFTER api_too_new so the
    // Pascal API-floor signature (whose string also contains "could not open encoder")
    // STILL wins first. Compute the lowercased tail once. Same SINGLE-SOURCE predicates
    // as the test-encode map — the two classifiers can never diverge.
    const lcTail = (r.stderrTail ?? '').toLowerCase();
    const code: DetectionWarningCode =
      encoder === 'nvenc' && isNvencApiTooNewStderr(lcTail)
        ? 'nvenc_api_too_new'
        : encoder === 'nvenc' && isNvencHevcUnsupportedStderr(lcTail)
          ? 'nvenc_hevc_unsupported'
          : 'encoder_runtime_broken';
    return {
...
async function probeEncodeTiers(
  encoder: EncoderId,
  devicePath?: string,
): Promise<ProbeEncodeOutcome> {
  if (encoder !== 'qsv') {
...
// ── 49-03: the forced-IDR CONFIRM stage (D3=D-C) ────────────────────────────
// 49-02 shipped `-forced_idr 1` (qsv) / `-forced-idr 1` (nvenc) into production
// UNPROBED: no probe, no test-encode and no bench path ever emitted the token. On
// a oneVPL/iHD or NVENC runtime that rejects it, /diagnostics stays green while
// 100 % of real jobs die. That is the 25-02 shape INVERTED — back then the
// diagnosis was too pessimistic, now it is too optimistic.
//
// This spawn closes that gap the SELF-DEGRADING way (D-C), not the
// gate-the-encoder-out way (D-A): a rejecting host keeps hardware encoding and
// merely loses the closed-GOP pin. D-A would have turned "all QSV jobs die
// loudly" into "everything runs silently, orders of magnitude slower".
//
// Runs at most ONCE per {qsv,nvenc} chain, AFTER the untouched 30-01 tiering, and
// only when there is something to confirm. Spawn budget per chain: qsv 2 (worst
// case 3), nvenc 2, vaapi 1, libx265 0. The chains run in parallel, so the boot
// worst case is the MAXIMUM of one chain (3 × ENCODE_PROBE_TIMEOUT_MS = 30 s, up
// from 20 s) — and since 39-01 the dispatch awaits detectEncoders(), so the first
// encode after boot waits for exactly that. A separate, shorter confirm timeout is
// deliberately DEFERRED (AUDIT D-1).
async function confirmForcedIdrSupport(
  encoder: EncoderId,
  devicePath: string | undefined,
  qsvRateControl: QsvRateControl | undefined,
): Promise<Pick<ProbeEncodeOutcome, 'forcedIdrSupported' | 'forcedIdrWarning'>> {
  const r = await runEncodeProbe(
    ffmpegBinaryFor(encoder),
    // Carries the SAME pixFmt pin and the SAME tier-won ratecontrol as the spawn
    // before it, plus the IDR token — and deliberately NO `-force_key_frames`
    // (Q4): that is a generic ffmpeg option no runtime rejects, so taking it along
    // would blend two possible causes into one exit code.
    buildProbeEncodeArgs(encoder, devicePath, qsvRateControl, true),
  );

  if (r.ok) return { forcedIdrSupported: true };

  if (r.cause === 'EXIT_NONZERO') {
    const lcTail = (r.stderrTail ?? '').toLowerCase();
    if (isForcedIdrRejectedStderr(lcTail)) {
      // PROVEN rejection → auto-degrade THIS encoder only. outcome stays
      // 'functional', the encoder stays in detected[] (AC-10).
      return {
        forcedIdrSupported: false,
        // audit SR4 / 25-01: `detail` carries a RAW ffmpeg excerpt — rendered as
        // opaque text, NEVER through next-intl/ICU (a stray `{`/`}` throws). The
        // `${encoder}: ` prefix is MANDATORY: on a dual-GPU host qsv AND nvenc can
        // both emit this identical code, and the copy-report must tell them apart
        // (precedent: the encoder_runtime_broken branch in probeEncodeOnce).
        forcedIdrWarning: {
          code: 'forced_idr_unsupported',
          severity: 'warn',
          detail: `${encoder}: ${tailExcerpt(r.stderrTail)}`,
        },
      };
    }
    // NOT classifiable (device busy, OOM, GPU-session contention with the parallel
    // vaapi probe, driver hiccup, a wording we have not seen) → FAIL OPEN (D4):
    // no verdict, no warning, production keeps its pin. Believing a transient
    // failure here would undo 49-02 on a HEALTHY host for the whole process
    // lifetime and claim a runtime limit that does not exist — the very
    // false-negative class this plan exists to remove, rebuilt one level up.
    // ONE warn with the RAW, unstripped tail (precedent encoder_probe_broken,
    // 25-01 audit-F1): tailExcerpt is LOSSY, and the signature list above must
    // stay tunable from field logs.
    logger.warn(
      {
        action: 'encoder_forced_idr_confirm_unclassified',
        encoder,
        rawStderr: r.stderrTail ?? '',
      },
      'forced-idr confirm failed with an unrecognised stderr — failing OPEN (pin kept)',
    );
    return {};
  }

  // TIMEOUT / ENOENT / ERROR → nothing was proven either way. Fail OPEN, and emit
  // NO warning: there is no evidence to report.
  return {};
}

// 49-03: the 30-01 tiering (above, unchanged) plus the forced-IDR confirm stage.
// The gate is all three of: the encoder probed 'functional', the closed-GOP pin is
// actually enabled, and the encoder is one that emits an IDR token at all
// (libx265 pins via `-x265-params open-gop=0`, which needs no runtime probe;
// vaapi emits no IDR token whatsoever by 49-02 design). Miss any one and the spawn
// count is byte-identical to v2.46.0 (AC-8, AC-18).
async function probeEncodeFunctional(
  encoder: EncoderId,
  devicePath?: string,
): Promise<ProbeEncodeOutcome> {
  const tiered = await probeEncodeTiers(encoder, devicePath);

  if (tiered.outcome !== 'functional') return tiered;
  if (!closedGopEnabled()) return tiered;
  if (encoder !== 'qsv' && encoder !== 'nvenc') return tiered;

  const confirm = await confirmForcedIdrSupport(encoder, devicePath, tiered.qsvRateControl);
  return { ...tiered, ...confirm };
}
...
async function runDetection(gpuDevice?: string): Promise<DetectionResult> {
  ...
  // 49-03 (Q1=B): per-encoder forced-IDR verdict. Stays EMPTY unless a confirm
  // spawn produced a definitive answer — an absent key means "not proven", which
  // isForcedIdrSupported() reads as fail-OPEN true (D4). Under the probe-encode
  // kill-switch or with the closed-GOP pin disabled it therefore stays `{}` (AC-18).
  const forcedIdrSupported: Partial<Record<EncoderId, boolean>> = {};
  ...
  if (probeEncodeDisabled) {
    ...
    for (const { enc, res } of settled) {
      ...
      // 49-03: persist the confirm verdict. Only a DEFINITIVE answer writes a key —
      // undefined stays absent so the accessor falls open (D4/AC-11). The warning
      // rides alongside because a rejection leaves the encoder detected+functional,
      // so the `else` branch below (which owns res.warning) never sees it.
      if (res.forcedIdrSupported !== undefined) forcedIdrSupported[enc] = res.forcedIdrSupported;
      if (res.forcedIdrWarning) warnings.push(res.forcedIdrWarning);
      if (res.detected) {
        detected.push(enc);
  ...
  logger.info(
    {
      ...
      qsvRateControl,
      // 49-03 (audit M-2 / AC-20): the confirm verdict rides on the EXISTING line
      // — no second line, AC-16 untouched. Without it the evidence would live only
      // in /api/diagnostics, i.e. only for whoever calls it; the copy-report embeds
      // the ring-buffer, and the ring-buffer sees only ≥ `telemetry` (25), never
      // `debug` (20) (the 22-01 → 38-02 dark-surface bug). Emitted
      // even when empty, so "{}" is positive evidence that nothing was proven.
      forcedIdrSupported,
      probeDurationMs,
...
// 49-03 (D4/AC-11): the ONE place that reads the forced-IDR verdict out of the
// detection cache, mirroring getActiveQsvRateControl above so real-encode leaves
// (profiles.ts) stay pure and no caller reaches into globalThis itself.
//
// FAIL-OPEN — the default is `true`, and that direction is deliberate. Absent a
// key we have PROVEN NOTHING: cold process, kill-switch, probe-inconclusive,
// confirm timeout, an encoder that emits no IDR token, or an unclassifiable
// stderr. Fail-CLOSED would silently strip the closed-GOP pin — i.e. silently
// revert 49-02 — on every host whose probe is disabled or slow, and nobody would
// notice until the playback artefacts came back. Only a POSITIVE `false`, written
// by a confirm spawn whose stderr carried the rejection signature, degrades.
export function isForcedIdrSupported(encoder: EncoderId): boolean {
  return globalThis.__x265butler_encoder_cache?.forcedIdrSupported?.[encoder] ?? true;
}
...
// 49-03 (audit S-2): forwards `forceIdr` for the same reason MH-2 forwarded
// qsvRateControl — AC-5/AC-9 must assert against the REAL private builder, never
// against an argv the test rebuilds by hand (a divergence between the two would
// otherwise stay invisible). The `pixFmt` pin needs no parameter: the builder
// derives it from the encoder itself.
export function __forTests_buildProbeEncodeArgs(
  ...
  return buildProbeEncodeArgs(encoder, devicePath, qsvRateControl, forceIdr);
}
