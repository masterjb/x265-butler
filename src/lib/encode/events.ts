...
export type EngineEvent =
  ...
  | {
      ...
      // 43-02: ffmpeg speed multiplier, forwarded for the per-job ETA.
      speed: number | null;
  ...
  | {
      ...
      bytesOut: number;
      durationMs: number;
      // 26-01 (F3, audit-S4): active sidecar location mode at this commit. In
      ...
      // failed" post-incident. Optional for back-compat with pre-26-01 fixtures.
      sidecarMode?: 'off' | 'beside' | 'central';
      // 26-02 (F5): EFFECTIVE per-file output mode at this commit. 'replace' =
      ...
      // Optional for back-compat with pre-26-02 fixtures.
  ...
  | {
      type: 'queue.updated';
      /**
       * queued + encoding. Name AND meaning are frozen since 02-03 — external SSE
       * consumers rely on it. 48-03 evaluated a rename to `totalJobs` and rejected
       * it (CONTEXT option C: large test churn, no user-visible gain). The UI no
       * longer renders this field as "active"; it renders `encodingJobs`.
       */
      activeJobs: number;
      pendingJobs: number;
      /** ONLY status='encoding' (48-03, R3). Required field: tsc forces every emit site. */
      encodingJobs: number;
      paused: boolean;
    }
