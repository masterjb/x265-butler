...
async function processOne(
  ...
  // file would break the OTHER link (a Sonarr/Radarr "Use Hardlinks" import
  // shares one inode across library + download dirs). The fallback writes the
  // '-x265' sibling and leaves the original UNTOUCHED (not trashed, not
  // unlinked) — zero data-loss, the other link survives. A second TOCTOU
  // re-probe runs at the commit step (S1) for a link created DURING the encode.
  ...
  if (effectiveMode === 'replace') {
    ...
    try {
      ...
      // a genuinely vanished source is surfaced by the STAGE symlink step).
      statOk = false;
  ...
  // 26-02: replace derives the BARE-extension basename path (no '-x265' label);
  // suffix keeps the existing label-style path. outputPathFor honors the
  // resolved suffix (legacy override OR post-fallback container-derived).
  // 26-02: `let` because the commit-step TOCTOU hardlink fallback (S1) reassigns
  // this to the suffix path so the downstream sidecar write + forensic logs
  // follow the ACTUAL committed path. The suffix branch never reassigns it.
  let finalOutputPath =
    effectiveMode === 'replace'
      ? deps.staging.replaceOutputPathFor(file.path, effectiveContainer)
      : deps.staging.outputPathFor(file.path, resolvedSuffix);
  // 26-02 (F5): for replace SAME-EXT the finalOutputPath IS the original (exists
  // by definition — it is the legitimate replace target, trashed before rename
  // in the commit step). Skip the collision guard in that case ONLY. Replace
  // DIFF-EXT (avi→mkv) still guards: refuse if 'movie.mkv' already exists.
