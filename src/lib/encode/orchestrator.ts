...
import {
  ...
  // that collapses when the video track does (M5/M-A: avg_frame_rate does not).
  countVideoPackets as defaultCountVideoPackets,
  type ProbeResult,
  type ProbeStream,
  type SourceColor,
  type SourceHdr10,
...
// 35-01: auto-crop pre-pass + shared geometry normalizer (override + detect).
import { detectCrop as defaultDetectCrop, parseCropGeometry } from './cropdetect';
...
import {
  ...
  // 50-02: the attach half — cover descriptors + the `-metadata:s:t` base index.
  describeAttachedPictures,
  countAttachmentStreams,
  type CoverStream,
} from './attached-pic';
// 50-02: the extraction leaf. Injected through EngineDeps.extractCovers? (E8) so
// none of the twelve existing orchestrator-*.test.ts dep literals have to change.
import {
  extractCovers as defaultExtractCovers,
  coverAttachEnabled,
  type CoverAttachment,
} from './cover-extract';
// 50-01: the output frame-count integrity gate (pure evaluation + kill-switch).
import { evaluateFrameGate, frameGateEnabled } from './frame-gate';
...
// 50-06: the shared settings resolvers. The /diagnostics test encode calls the
// SAME three functions, so a pinned encoder / per-encoder CRF / per-encoder
// preset can no longer mean two different things in two places.
import {
...
export type EngineDeps = {
  ...
  // 35-01: auto-crop cropdetect pre-pass (DI for test seams; production wires
  // the default from cropdetect.ts). Only invoked when auto_crop is on AND no
  // valid crop_override is set.
  detectCrop: typeof defaultDetectCrop;
  // 50-01 (E7): the frame-gate packet counter. OPTIONAL — production
  // makeDefaultDeps ALWAYS wires it, and the call site falls back to the default
  // import. Precedent: 31-02 `fs.utimesSync?`. Keeping it optional avoids
  // touching the 15 `ffprobe:`-dep literals across 12 legacy test files; those
  // tests still spawn NOTHING, because the gate resolves duration+fps FIRST and
  // no legacy fixture carries `avgFrameRate`.
  countVideoPackets?: typeof defaultCountVideoPackets;
  // 50-02 (E8): the cover extractor. OPTIONAL for the same reason as
  // countVideoPackets (50-01 E7 / 31-02 fs.utimesSync?): production
  // makeDefaultDeps ALWAYS wires it and the call site falls back to the default
  // import, so no legacy test dep literal needs touching. Those tests spawn
  // NOTHING either way — the extraction only runs on an MKV encode whose probe
  // reported a cover, which no legacy fixture does.
  extractCovers?: typeof defaultExtractCovers;
};
...
function makeDefaultDeps(): EngineDeps {
  return {
    ...
    detectCrop: defaultDetectCrop,
    countVideoPackets: defaultCountVideoPackets,
    extractCovers: defaultExtractCovers,
  };
}
...
function resolveEncoderFor(
  ...
  // 50-06: the validity decision moved into the shared leaf so the /diagnostics
  // test encode resolves the SAME pin. The WARN below stays HERE — it carries a
  // `jobId`, which a pure resolver has no business knowing.
  const requestedRaw = settingsAll.encoder;
  const resolvedRequest = resolveRequestedEncoder(requestedRaw);
  let requested: EncoderId | 'auto';
  if (resolvedRequest !== 'invalid') {
...
function readSettings(deps: EngineDeps): {
  ...
  // 33-02: operator-configurable originals-trash root. Code-fallback (no
  // migration): empty/unset → stageRoot = byte-identical to pre-33-02 (trash
  // on the cache pool). A non-empty value routes the 30-day originals-trash to
  // the chosen mount (e.g. the array). Clearing the field is the one-click
  // revert lever (mirrors cache_pool_path's empty=auto idiom, NOT sidecar's
  // .min(1)-reject — there is no standalone default to fall back to).
  trashPath: string;
  // 34-01: operator-pinned /dev/dri/renderD* node for HW encoders. Code-fallback
  // empty/unset → '' (auto = first node = byte-identical). No migration; read
  // fresh per dispatch though the actual consumer is detection (detectEncoders
  // reads gpu_device from the setting repo itself — see 34-01 MH-1). Surfaced
  // here so the orchestrator/diagnostics can READ the configured value.
  gpuDevice: string;
  // 35-01: auto-crop / black-bar removal. Both code-default OFF/empty (no
  // migration) → off/unset is byte-identical to pre-35 for all four encoders.
  //  autoCrop      — per-file cropdetect pre-pass (default false).
  //  cropOverride  — operator fixed `W:H:X:Y` escape-hatch (default ''). When a
  //                  valid override is set it WINS over auto-detect; an invalid
  //                  non-empty value fails open (uncropped + warn). Parsed/guarded
  //                  at the dispatch resolve (the 35-02 UI only edits the string).
  autoCrop: boolean;
  cropOverride: string;
  // 43-01: force 10-bit HEVC Main10 output. Code-fallback default false (no
  // migration) → unset/'false' is byte-identical to pre-43 for all four encoders
  // (mirrors autoCrop). Threaded ONLY into the production runEncode dispatch.
  force10bit: boolean;
  // 43-03: preserve source VUI color tags. Code-fallback default false (no
  // migration) → unset/'false' byte-identical (mirrors force10bit). Threaded
  // ONLY into the production runEncode dispatch.
  ...
  const stageRoot = resolveEffectiveCachePath(settings.get('cache_pool_path')).effectivePath;
  // 33-02: empty/whitespace trash_path → track stageRoot (auto-cache).
  const rawTrash = settings.get('trash_path');
  return {
    stageRoot,
    ...
    // 33-02: code-fallback — empty/unset = stageRoot (byte-identical), else the
    // configured array/mount root. No migration; resolved fresh per dispatch.
    trashPath: rawTrash && rawTrash.trim() !== '' ? rawTrash : stageRoot,
    // 34-01: code-fallback — empty/unset = '' (auto = first-enumerated node =
    // byte-identical). Detection resolves the override itself; this is the
    // read-through for the orchestrator/diagnostics SURFACE.
    gpuDevice: settings.get('gpu_device') ?? '',
    // 35-01: code-fallback — off/empty = byte-identical to pre-35. No migration.
    autoCrop: settings.get('auto_crop') === 'true',
    cropOverride: settings.get('crop_override') ?? '',
    // 43-01: code-fallback — unset/'false' = byte-identical to pre-43. No migration.
    force10bit: settings.get('force_10bit') === 'true',
    // 43-03: code-fallback — unset/'false' = byte-identical. No migration.
    colorPassthrough: settings.get('color_passthrough') === 'true',
  };
}
...
function resolveEncodeParams(
  ...
  // 50-06: the resolution moved into the shared leaf (the /diagnostics test encode
  // resolves the SAME value). `settings.crf` is handed in explicitly so the
  // delegation is byte-identical BY CONSTRUCTION rather than by argument — the
  // leaf would otherwise re-derive the fallback from `settingsAll.default_crf`,
  // which reads the same row but through a different accessor.
  const crfDispatch = resolveCrfForEncoder(activeEncoder, settingsAll, settings.crf);
  ...
  // 50-06: same delegation as the CRF above. The leaf hands back the raw stored
  // value so the "was set but invalid" warn condition below keeps its exact
  // meaning (absent ≠ garbage).
  const resolvedPreset = resolvePresetForEncoder(activeEncoder, settingsAll);
  const perEncoderPresetRawDispatch = resolvedPreset.raw;
  const presetValid = resolvedPreset.source === 'settings';
  const presetDispatch = resolvedPreset.preset;
  const presetSource: 'settings' | 'fallback' = resolvedPreset.source;
  // 43-01 (AC-9): force10bit on the canonical per-job dispatch resolved-evidence
  // line so an ON encode leaves a reconstructable audit trail ("why is my output
  // 10-bit / larger") WITHOUT re-deriving from argv — same code-fallback read as
  // readSettings (settingsAll['force_10bit']==='true'). OFF emits force10bit:false
  // → same trail as pre-43. No new log call.
  const force10bitDispatch = settingsAll['force_10bit'] === 'true';
  // 43-03 (AC-9): colorPassthrough on the same resolved-evidence line — one field,
  // no new log call, for post-incident reconstruction ("why does my output carry
  // BT.2020 tags"). OFF emits colorPassthrough:false → same trail as pre-43-03.
  const colorPassthroughDispatch = settingsAll['color_passthrough'] === 'true';
  deps.logger.info(
    {
      jobId: job.id,
      encoder: activeEncoder,
      presetResolved: presetDispatch,
      presetSource,
      force10bit: force10bitDispatch,
      colorPassthrough: colorPassthroughDispatch,
    },
...
interface ContainerCompat {
  ...
  // 41-01: count + descriptors of MKV-incompatible (data/unknown) source streams
  // dropped by the buildArgs `-map 0:v -map 0:a? -map 0:s? -map 0:t?` whitelist.
  ...
  // preflightSourceProbe (null on pure-MKV by MH-2 design). Both undefined when
  // the source carries no cover ⇒ argv byte-identical to pre-49.
  attachedPicVideoOrdinals: ReadonlyArray<number> | undefined;
  encodedVideoOrdinals: ReadonlyArray<number> | undefined;
  // 50-02: the SAME covers, described well enough to extract and re-attach them,
  // plus the number of attachment-typed source streams (the `-metadata:s:t:<n>`
  // base index). Both come from the SAME probe as attachedPicVideoOrdinals — the
  // dual mp4/mkv seam below, never preflightSourceProbe (null on pure-MKV by
  // MH-2 design) and never a second ffprobe. sourceAttachmentCount is
  // `undefined` when NO probe ran at all; a source WITHOUT attachments yields 0,
  // and buildArgs treats those two states very differently (E9).
  coverStreams: ReadonlyArray<CoverStream> | undefined;
  sourceAttachmentCount: number | undefined;
  ...
  // 43-03 (AC-7 / MH-2): source VUI color tags for passthrough, extracted from
  // whichever probe ran (mp4 sourceProbe OR mkv mkvSourceProbe) — a SEPARATE
  // field, NEVER read from preflightSourceProbe (null on pure-MKV by design).
  // undefined when color_passthrough is OFF → byte-identical (no extraction).
  sourceColor: SourceColor | undefined;
  // 43-04 (AC-7 / MH-2): source HDR10 static metadata for passthrough, from the
  // SAME dual-path probe as sourceColor (mp4 sourceProbe OR mkv mkvSourceProbe) —
  // NEVER preflightSourceProbe. undefined when color_passthrough OFF (D3=A fold —
  // HDR10 rides the same Setting). byte-identical when undefined.
  sourceHdr10: SourceHdr10 | undefined;
}
...
async function resolveContainerAndCompat(
  ...
  // would silently fall through on that transition.
  let attachedPicVideoOrdinals: ReadonlyArray<number> | undefined;
  let encodedVideoOrdinals: ReadonlyArray<number> | undefined;
  // 50-02: same dual-seam discipline as the two above — set at BOTH the mp4 and
  // the mkv block, because the mp4 block can flip effectiveContainer to 'mkv'
  // (the 05-15 audio fallback) after it has already run.
  let coverStreams: ReadonlyArray<CoverStream> | undefined;
  let sourceAttachmentCount: number | undefined;
  ...
  // 43-03 (AC-7): resolved source color for passthrough — set ONLY when the
  // setting is ON, from whichever probe ran. Stays undefined when OFF → no
  // extraction, byte-identical.
  let sourceColor: SourceColor | undefined;
  // 43-04 (AC-7): source HDR10 static metadata — same dual-path + same gate as
  // sourceColor (D3=A: the existing colorPassthrough Setting ALSO gates HDR10).
  let sourceHdr10: SourceHdr10 | undefined;
  if (effectiveContainer === 'mp4') {
    preflightSourceProbe = await deps.ffprobe(file.path);
    const sourceProbe = preflightSourceProbe;
    if (sourceProbe) {
      // 43-03: extract color on the mp4 probe (mp4 path keeps the source VUI tags
      // too). If the source later falls back to mkv, the mkv block re-reads the
      // SAME probe — idempotent.
      if (settings.colorPassthrough) sourceColor = sourceProbe.color;
      // 43-04: extract HDR10 on the SAME mp4 probe under the SAME gate (idempotent
      // re-read on a mp4→mkv fallback).
      if (settings.colorPassthrough) sourceHdr10 = sourceProbe.hdr10;
      // 49-01: cover-art seam on the mp4 probe. NOT gated by any setting — a
      // cover kills the job outright, so the fix is unconditional. Both fields
      // are set TOGETHER or left undefined together (see EncodeOptions).
      const attachedPicMp4 = analyzeAttachedPictures(sourceProbe);
      if (attachedPicMp4.hasAttachedPictures) {
        attachedPicVideoOrdinals = attachedPicMp4.videoOrdinals;
        encodedVideoOrdinals = attachedPicMp4.encodedVideoOrdinals;
        // 50-02: the attach descriptors from the SAME probe. Carried even on the
        // mp4 path because the audio fallback below can flip the container to
        // mkv — and then the mkv branch of buildArgs needs them (AC-25). While
        // the container stays mp4 they are never used: the orchestrator gates
        // the extraction on effectiveContainer === 'mkv' (AC-29).
        coverStreams = describeAttachedPictures(sourceProbe);
      }
      // Set UNCONDITIONALLY (not inside the cover branch): a valid 0 must stay
      // distinguishable from "no probe ran" — buildArgs drops the covers on the
      // latter instead of guessing index 0 (E9/M-H).
      sourceAttachmentCount = countAttachmentStreams(sourceProbe);
  ...
  // 41-01 (D5/D6): MKV source pre-flight for the `incompatible_streams_dropped`
  // warn. A SEPARATE top-level `if` AFTER the mp4 block (NOT `else if`) — a
  // match-source / explicit-mp4 source that falls back to MKV inside the mp4
  // block flips effectiveContainer to 'mkv', and an `else if` would never
  // re-evaluate → the warn would be silently skipped on that path (the MAPPING
  // is still correct via buildArgs; only the observability is lost). SR-1: REUSE
  // the probe already taken on the fallback path — no second ffprobe.
  if (effectiveContainer === 'mkv') {
    // MH-2: read into a LOCAL var — do NOT assign back to the hoisted
    // `preflightSourceProbe`. It feeds the cropdetect duration clamp (:1613) and
    // the data-loss-critical sidecar-V3 + commit path (:1680), and MUST stay
    // null on the pure-MKV path; assigning here would silently change cropdetect
    // + sidecar behavior for EVERY MKV encode (an unscoped commit-path change the
    // boundaries forbid).
    // The probe feeds the warn ONLY (observability) — it MUST NOT fail the
    // encode. ffprobe-null AND a thrown ffprobe both soft-degrade (no warn); the
    // unconditional MKV whitelist in buildArgs keeps the mapping correct either
    // way. A throwing source ffprobe still surfaces downstream at the verify
    // stage (output probe), so the job's error prefix is unchanged.
    let mkvSourceProbe: ProbeResult | null = preflightSourceProbe;
    if (mkvSourceProbe === null) {
      try {
        mkvSourceProbe = await deps.ffprobe(file.path);
      } catch {
        mkvSourceProbe = null;
      }
    }
    if (mkvSourceProbe) {
      const incompat = analyzeIncompatibleStreams(mkvSourceProbe, 'mkv');
      if (incompat.hasIncompatible) {
      ...
      // probe set above (idempotent).
      if (settings.colorPassthrough) sourceColor = mkvSourceProbe.color;
      // 43-04: the pure-MKV HDR10 seam — same LOCAL-var source, same gate.
      if (settings.colorPassthrough) sourceHdr10 = mkvSourceProbe.hdr10;
      // 49-01: the pure-MKV cover-art seam — same LOCAL-var source. On a mp4→mkv
      // fallback this re-analyzes the same probe set above (idempotent), which is
      // exactly why a single seam is not enough: on the pure-MKV path the mp4
      // block never runs, and on the fallback path effectiveContainer changed
      // after the mp4 block already executed.
      const attachedPicMkv = analyzeAttachedPictures(mkvSourceProbe);
      if (attachedPicMkv.hasAttachedPictures) {
        attachedPicVideoOrdinals = attachedPicMkv.videoOrdinals;
        encodedVideoOrdinals = attachedPicMkv.encodedVideoOrdinals;
        // 50-02: the pure-MKV attach seam. On a mp4→mkv fallback this re-derives
        // from the SAME probe object the mp4 block already used — idempotent, no
        // second ffprobe, so expectation and mapping cannot diverge (AC-25).
        coverStreams = describeAttachedPictures(mkvSourceProbe);
      }
      sourceAttachmentCount = countAttachmentStreams(mkvSourceProbe);
    }
  }
...
async function processOne(
  ...
  const stage = await runEncodeStage(deps, job, file, settings, expectedFileVersion, {
    ...
    attachedPicVideoOrdinals,
    encodedVideoOrdinals,
    // 50-02: the same covers as extractable descriptors + the attach base index.
    coverStreams,
    sourceAttachmentCount,
    audioPerStreamTargets,
    finalOutputPath,
    // 35-01 (SR-1): thread source duration for the cropdetect offset clamp.
    sourceDurationSeconds: preflightSourceProbe?.durationSeconds ?? undefined,
    // 43-03: resolved source color (undefined when passthrough OFF → byte-identical).
    sourceColor,
    // 43-04: resolved source HDR10 (undefined when passthrough OFF → byte-identical).
    sourceHdr10,
  });
  switch (stage.kind) {
    case 'fail':
  ...
  const verify = await verifyOutput(
    ...
    expectedFileVersion,
    workDir,
    // 50-01 (audit SR-2): forward the encode's evidence so a gate-failed job
    // carries log_tail + the real exit code, exactly like encode_nonzero_exit.
    ...
    if (verdict === 'done-smaller') {
      ...
      if (effectiveMode === 'replace') {
        ...
        if (toctouHardlink) {
          ...
          // 33-02 (SR-1/AC-9): trash_path is operator-configurable but NOT
          // PUT-probed for writability (a freshly-typed array path may not exist
          // until mkdir-at-commit). A bad/unmounted/non-creatable trash_path
          // would otherwise fail EVERY encode at commit with an opaque generic
          // failure pointing nowhere. Emit a DEDICATED diagnostic naming the
          // resolved trashPath BEFORE re-throwing into the top-level failJob
          ...
          try {
            ...
            deps.logger.error(
              {
                action: 'trash_move_failed',
                jobId: job.id,
                fileId: file.id,
                originalPath: file.path,
                trashPath,
                err: trashErr instanceof Error ? trashErr.message : String(trashErr),
              },
              'trash move failed (replace mode) — check trash_path setting; original left intact',
            );
            throw trashErr;
          }
        ...
        if (replaceHardlinkFallback) {
          ...
          try {
            ...
            deps.logger.error(
              ...
              'trash move failed (suffix mode) — check trash_path setting',
            );
            throw trashErr;
          }
      ...
      // 36-01: source-keyed central sidecar — WRITE-side mirror of the 33-01 read
      // resolver and the discard-branch source write. The output-keyed write above
      // protects the -x265 output; this protects a SOURCE that resurfaces at its
      // original absolute path (restore-from-trash / relocation / rsync) under a
      // DB-not-authoritative scenario. CENTRAL ONLY + effectiveMode==='suffix' +
      // source-still-recoverable:
      //   - beside is excluded — suffix-default trashes the source BEFORE this point
      //     (orphan write) and beside cannot survive relocation by design;
      //   - effectiveMode!=='suffix' (replace) is excluded — same-ext would duplicate
      //     the output write (file.path === finalOutputPath); diff-ext trashed the source;
      //   - the suffix hard-DELETE sub-case (settings.deleteOriginalAfterEncode &&
      //     !replaceHardlinkFallback, :2058-2087) is excluded (SR-1): the operator chose
      //     permanent deletion (fs.unlink, NO trash row) → the source can NEVER resurface
      //     from trash → a source-keyed sidecar there is an orphan-for-life, the SAME
      //     orphan-avoidance rule that excludes beside. The two surviving sub-cases
      //     (replaceHardlinkFallback kept-source :2047, default trash :2088) BOTH leave a
      //     recoverable source → guard is meaningful.
      // Reuses the SAME sidecarPayload (V3 envelope; source block already describes the
      // source). writeSidecarResolved carries its own warn-never-throw soft-degrade — so
      // the breadcrumb below is an ATTEMPT signal: a paired action='sidecar_write_failed'
      // warn (same jobId, targetPath===file.path) from writeSidecarResolved NEGATES it.
      const sourceHardDeleted = settings.deleteOriginalAfterEncode && !replaceHardlinkFallback;
      if (
        sidecarPayload &&
        settings.sidecarMode === 'central' &&
        effectiveMode === 'suffix' &&
        !sourceHardDeleted
      ) {
        await deps.writeSidecarResolved(
        ...
        deps.logger.info(
          {
            action: 'source_sidecar_written',
            jobId: job.id,
            fileId: file.id,
            sourcePath: file.path,
          },
          'source-keyed central sidecar write attempted (done-smaller, suffix, source-recoverable) — relocation/DB-loss re-queue guard; a paired sidecar_write_failed warn negates this',
        );
      }
...
interface RunEncodeStageCtx {
  ...
  attachedPicVideoOrdinals: ReadonlyArray<number> | undefined;
  encodedVideoOrdinals: ReadonlyArray<number> | undefined;
  // 50-02: cover descriptors → the extraction (MKV only), and the attachment
  // count → EncodeOptions.sourceAttachmentCount (the `-metadata:s:t` base).
  coverStreams: ReadonlyArray<CoverStream> | undefined;
  sourceAttachmentCount: number | undefined;
  audioPerStreamTargets: ReadonlyArray<AudioAutoTranscodeTarget> | undefined;
  finalOutputPath: string;
  // 35-01 (SR-1): source duration (from preflightSourceProbe) so detectCrop can
  // clamp the cropdetect seek offset on short sources. undefined → detectCrop's
  // own offset-0 retry covers it.
  sourceDurationSeconds: number | undefined;
  // 43-03: resolved source color for passthrough → EncodeOptions.color. undefined
  // when color_passthrough OFF → byte-identical (no color flags emitted).
  sourceColor: SourceColor | undefined;
  // 43-04: resolved source HDR10 static metadata → EncodeOptions.hdr10. undefined
  // when color_passthrough OFF → byte-identical (no master-display/max-cll). Also
  // feeds the hdr10Present audit field on the preflight_statfs_probe log (AC-9).
  sourceHdr10: SourceHdr10 | undefined;
}
...
async function runEncodeStage(
  ...
  deps.logger.info(
    {
      ...
      // 43-04 (AC-9): SOURCE-HDR10-presence audit field on an EXISTING
      // runEncodeStage-scope info log (sourceHdr10 is in scope here, unlike the
      // dispatch_preset_resolved log at :1082 which runs pre-probe). Asserts the
      // SOURCE carried HDR10 static metadata — NOT that the output retained the
      // SEI (HW passthrough is unverifiable in-process per AC-6). false when
      // color_passthrough OFF (sourceHdr10 undefined) → byte-identical trail.
      hdr10Present: !!(sourceHdr10?.masterDisplay || sourceHdr10?.maxCll),
    },
  ...
  // 35-01: resolve the effective auto-crop ONCE, before the encode dispatch.
  // Precedence (D1): a valid crop_override WINS → else auto_crop cropdetect →
  // else none. Both off/empty → crop undefined → byte-identical pre-35 (AC-1).
  let crop: string | undefined;
  let cropSource: 'auto' | 'override' | undefined;
  // 37-02: capture WHY the auto-crop path resolved to no crop, so the no-op is
  // surfaced explicitly (reason-tagged crop_no_op below) instead of staying silent.
  let cropNoOpReason: 'full_frame' | 'inconclusive' | undefined;
  let cropNoOpDetected: string | null = null;
  const ovr = parseCropGeometry(settings.cropOverride);
  if (settings.cropOverride.trim() !== '' && ovr === null) {
    // AC-5: invalid (incl. odd-dim) override → fail-open uncropped + warn.
    deps.logger.warn(
      {
        action: 'crop_override_invalid',
        jobId: job.id,
        fileId: file.id,
        value: settings.cropOverride,
      },
      'crop_override malformed — encoding uncropped',
    );
  }
  if (ovr) {
    // Override wins — detect is skipped entirely (AC-3).
    crop = ovr;
    cropSource = 'override';
  } else if (settings.autoCrop) {
    const detected =
      (await deps.detectCrop(file.path, {
        durationSeconds: sourceDurationSeconds,
        signal: controller.signal,
      })) ?? undefined;
    // AC-4 (MANDATORY full-frame guard, audit MH-1): cropdetect prints the FULL
    // frame on a no-bars source, so an unguarded detected crop yields a spurious
    // no-op `-vf` that breaks the byte-identical default. Drop a crop equal to
    // the full source frame. file.width/height live on the FileRow already (no
    // threading). Guard skipped ONLY when dims are null (unscanned edge) → then
    // conservatively keep the detected crop (rare; documented).
    cropNoOpDetected = detected ?? null;
    if (detected) {
      const [w, h, x, y] = detected.split(':').map(Number);
      const isFullFrame =
        file.width != null &&
        file.height != null &&
        w === file.width &&
      ...
      if (!isFullFrame) {
        ...
        // detected a geometry, but it equals the full source frame → no bars.
        cropNoOpReason = 'full_frame';
      }
    } else {
      // detect failed / no usable crop line.
      cropNoOpReason = 'inconclusive';
    }
  }
  if (crop) {
    // AC-8 (audit SR-3): record the resolved crop so an operator can later
    // reconstruct WHY a committed output has non-source dimensions.
    deps.logger.info(
      {
        action: 'crop_applied',
        jobId: job.id,
  ...
  } else if (settings.autoCrop && !ovr) {
    // 37-02: auto_crop was ON but resolved to NO crop (no bars / inconclusive).
    // 35-01 dropped this silently → operator perceived auto-crop as "broken".
    // Surface it once per job, reason-tagged, parallel to crop_applied. Fires
    // ONLY on the auto-crop no-crop path: never when an override won (crop set)
    // and never when auto_crop is off (AC-4). Co-fires with crop_override_invalid
    // on a malformed override + auto_crop ON (AC-6) — two distinct facts.
    deps.logger.info(
      {
        action: 'crop_no_op',
        reason: cropNoOpReason,
        jobId: job.id,
        fileId: file.id,
        detected: cropNoOpDetected,
        srcWidth: file.width,
        srcHeight: file.height,
      },
      cropNoOpReason === 'full_frame'
        ? 'auto-crop: no black bars detected — encoding full frame'
        : 'auto-crop: detection inconclusive — encoding full frame',
    );
  }

  // ───────────────────────────────────────────────────────────────────────────
  // 50-02: extract the embedded cover art so it can be re-attached as a real
  // matroska AttachedFile instead of riding along as a second video track.
  //
  // POSITION: after STAGE (the workDir must exist) and after the crop pre-pass
  // (the AbortController is live, so a cancel here kills the extraction child),
  // before the encode dispatch. The INPUT is `workDir/input` — the staging
  // symlink, never `file.path`: the staging indirection is the contract of the
  // whole encode path, and reading the source directly would bypass it.
  //
  // MKV ONLY (AC-29). On mp4 the muxer preserves `attached_pic`, so 49-01's copy
  // already produces a correct file; extracting there would spawn a child and
  // write files nobody attaches. The kill-switch is not even read on that path.
  // ───────────────────────────────────────────────────────────────────────────
  let coverAttachments: ReadonlyArray<CoverAttachment> | undefined;
  const coverCount = coverStreams?.length ?? 0;
  if (effectiveContainer === 'mkv' && coverCount > 0) {
    if (!coverAttachEnabled()) {
      // E1=B: the kill-switch fallback is Opt B — DROP the cover. Never back to
      // the 49-01 copy form: that is a measured-broken output (M-C), not a safe
      // state. buildArgs still emits `-map -0:v:N`, so the cover leaves the
      // video set; it simply is not re-attached.
      deps.logger.warn(
        {
        ...
        'cover art: ENCODE_COVER_ATTACH_DISABLED=1 — cover dropped from the output',
      );
    } else {
      const extract = deps.extractCovers ?? defaultExtractCovers;
      coverAttachments = await extract(path.join(workDir, 'input'), workDir, coverStreams ?? [], {
        signal: controller.signal,
        log: deps.logger,
        jobId: job.id,
      });
      if ((coverAttachments?.length ?? 0) < coverCount) {
        // The difference is a SILENT cover loss the operator would otherwise
        // only notice by opening the file. extractCovers already warned per
        // cover with a reason; this line carries the total.
        deps.logger.warn(
          {
            action: 'cover_attach_partial',
            jobId: job.id,
            fileId: file.id,
            coverCount,
            attachedCount: coverAttachments?.length ?? 0,
          },
          'cover art: fewer covers attached than the source carried',
        );
      }
    }
  }
  ...
  // 50-01 (audit MH-1): chunks that arrive BEFORE the stream is open used to be
  // DROPPED. The old comment claimed "resolved promise handles late writes" —
  // the code never did that. runEncode is fully synchronous from buildArgs to
  // spawn, so its argv line fires in the SAME tick as deps.runEncode(...), while
  // the .then microtask below cannot possibly have run yet ⇒ jobLogStream is
  // still null ⇒ the line was lost in EVERY job. (stderr chunks survived only
  // because they arrive much later.) A test mocking onLogChunk would be green
  // and the production log still empty — hence AC-13b asserts the FILE.
  // The buffer is CAPPED (AC-13c): if the stream never opens, it must not grow
  // for the whole encode. This repairs the chunk-loss class in general, not just
  // the argv line — every future early write inherits it. The 05-03 T1.F
  // contract is untouched: nothing is awaited on the hot path.
  const preOpenChunks: Array<Buffer | string> = [];
  let preOpenBytes = 0;
  const PRE_OPEN_CAP_BYTES = 64 * 1024;
  const logStreamPromise = openJobLogStream(String(job.id), settings.stageRoot)
    .then((s) => {
      jobLogStream = s;
      if (s) {
        try {
          for (const c of preOpenChunks) s.write(c);
        } catch {
          // a log write must never touch the encode
        }
      }
  ...
  try {
    encodeResult = await deps.runEncode({
      ...
      signal: controller.signal,
      // 35-01: resolved auto-crop (override-wins → detect → none). undefined →
      // byte-identical pre-35. Threaded ONLY here (NOT into probe/bench).
      crop,
      // 43-01: force 10-bit Main10 output. Threaded ONLY on the production encode
      // (NOT into probe/bench/test-encode, which build their OWN args = stay 8-bit
      // by construction, SR-4). false → byte-identical pre-43 (AC-1).
      force10bit: settings.force10bit,
      // 43-03: source VUI color passthrough. Threaded ONLY on the production
      // encode (probe/bench/test-encode build their OWN args, never color — SR-3).
      // undefined when passthrough OFF → byte-identical pre-43-03 (AC-1/AC-8).
      color: sourceColor,
      // 43-04: source HDR10 static metadata → libx265 -x265-params merge (HW rides
      // ffmpeg auto SEI passthrough). Threaded ONLY on the production encode (bench/
      // probe/test-encode build their OWN args, never hdr10 — AC-8). undefined when
      // passthrough OFF → byte-identical pre-43-04 (AC-1).
      hdr10: sourceHdr10,
      ...
      attachedPicVideoOrdinals,
      encodedVideoOrdinals,
      // 50-02: the extracted covers + the `-metadata:s:t` base index. Both are
      // consumed ONLY by the MKV branch of buildArgs; on mp4 they are absent
      // anyway (no extraction runs there) and would be ignored if present.
      coverAttachments,
      sourceAttachmentCount,
      ...
      onLogChunk: (chunk) => {
        if (jobLogStream) {
          jobLogStream.write(chunk);
          return;
        }
        // 50-01 (MH-1): the stream is not open yet — buffer, capped. Over the
        // cap the chunk is dropped rather than the buffer growing unbounded.
        const n = typeof chunk === 'string' ? Buffer.byteLength(chunk) : chunk.length;
        if (preOpenBytes + n > PRE_OPEN_CAP_BYTES) return;
        preOpenBytes += n;
        preOpenChunks.push(chunk);
      },
      onProgress: (ev) => {
        deps.logger.debug(
...
// 50-01 (audit MH-2): resolve the ONE video stream the frame gate reasons about
// — the first video-typed stream that is NOT cover art. It yields BOTH halves of
// the comparison: its `avgFrameRate` feeds the expectation and its ORDINAL feeds
// `-select_streams v:<n>`, so expected and actual provably describe the same
// stream. A hardcoded `v:0` would, on an output whose cover sits at video ordinal
// 0 (the order `-map 0:v` inherits from such a source), count the single cover
// packet against an expectation taken from a different track — every such encode
// would fail. Cover detection is delegated to isAttachedPictureStream (49-01),
// the ONE place that decides this; the raw `attachedPic` field would NOT do,
// because M1 measured that the Matroska muxer does not write the attached_pic
// disposition back into our own output.
// Ordinals count video-typed streams only — the same counting rule as
// analyzeAttachedPictures, and the rule ffmpeg's `v:N` selector uses.
function resolveGateVideoStream(
  streams: ReadonlyArray<ProbeStream> | undefined,
): { videoOrdinal: number; avgFrameRate: number | undefined } | null {
  let ordinal = 0;
  for (const s of streams ?? []) {
    if (s.codec_type !== 'video') continue;
    if (!isAttachedPictureStream(s)) {
      return { videoOrdinal: ordinal, avgFrameRate: s.avgFrameRate };
    }
    ordinal += 1;
  }
  return null;
...
// post-encode check + (50-01) the output frame-count integrity gate. External
// cancel → {kind:'cancelled'}; failJob → {kind:'fail'}.
async function verifyOutput(
  ...
  expectedFileVersion: number,
  workDir: string,
  // 50-01 (audit SR-2): the encode's own exit code + ffmpeg log tail, so a
  // gate-failed job carries the SAME evidence the encode_nonzero_exit path has
  // always carried. The reporter's proof for this entire phase IS that tail
  // (`frame= 17 fps=0.3 …`); a gate that kills a job and throws the evidence
  // away would defeat goal 2 of the phase.
  encodeExitCode: number,
  encodeLogTail: string | null,
  ...
  // ── 50-01: output frame-count integrity gate ────────────────────────────────
  // E4 — the placement is deliberate and must NOT be moved up. Running the gate
  // AFTER the external-cancel check keeps an operator's cancel intent winning:
  // an externally cancelled job whose output happens to be broken stays
  // `cancelled`, it does not become `failed`. CONTEXT's "before the verdict is
  // formed" is satisfied SEMANTICALLY — the verdict computed above is a pure
  // calculation that the caller only ever consumes AFTER verifyOutput returns,
  // i.e. never on {kind:'fail'}.
  //
  // v2.46.0 booked 17 of 371 722 video frames as `done-smaller`: ffprobe parsed
  // the output and the file had a size, so both existing sieves passed and the
  // size ratio alone picked the bucket.
  if (frameGateEnabled()) {
    // E1=C: the expectation's DURATION comes from the file row (scan time), the
    // fps from the output probe. Zero extra spawns. When the DB row carries no
    // duration we fall back to the OUTPUT duration — and then the gate inherits
    // the M-B blind spot (a mux truncated as a whole shortens duration and count
    // together, so the ratio still looks fine). Deliberate: a gate that catches
    // one of the two shapes beats no gate, and the source is named in the log.
    const durationSource: 'file-row' | 'output-fallback' =
      file.duration_seconds != null ? 'file-row' : 'output-fallback';
    const durationSeconds = file.duration_seconds ?? probe.durationSeconds;

    const gateStream = resolveGateVideoStream(probe.streams);
    const avgFrameRate = gateStream?.avgFrameRate;
    const videoOrdinal = gateStream?.videoOrdinal ?? 0;

    // The pre-check runs the SAME rules as the real evaluation, in the binding
    // order duration → fps → count. A `no_duration` / `no_frame_rate` answer
    // means the count spawn is never launched (E7: this is why legacy tests,
    // whose fixtures carry no avgFrameRate, spawn nothing at all). Only
    // `count_failed` proves duration+fps are usable and the demux is worth it.
    const pre = evaluateFrameGate({ durationSeconds, avgFrameRate, actualPackets: null });
    if (pre.kind === 'skip' && pre.reason !== 'count_failed') {
      deps.logger.warn(
        {
          action: 'output_frame_gate_skipped',
          jobId: job.id,
          fileId: file.id,
          reason: pre.reason,
          durationSource,
        },
        'output frame gate skipped — job continues (fail-OPEN)',
      );
    } else {
      const countVideoPackets = deps.countVideoPackets ?? defaultCountVideoPackets;
      const actualPackets = await countVideoPackets(stageOut, { videoOrdinal });
      const gate = evaluateFrameGate({ durationSeconds, avgFrameRate, actualPackets });

      if (gate.kind === 'skip') {
        // E6: the count run itself failed (timeout / ENOENT / non-zero / junk).
        // A failed CHECK is no evidence of a broken output — fail-OPEN.
        deps.logger.warn(
          {
            action: 'output_frame_gate_skipped',
            jobId: job.id,
      ...
      } else if (gate.kind === 'pass') {
        // 49-04 telemetry tier (25): reaches the ring buffer and therefore the
        // copy-report, without shouting on stdout once per finished encode.
        deps.logger.telemetry(
          {
            action: 'output_frame_gate_passed',
            jobId: job.id,
            fileId: file.id,
          ...
          'output frame gate passed',
        );
      } else {
        // audit SR-1: the FAIL path gets the FULL structured line, and it gets it
        // BEFORE failJob. The skip path logs everything, the pass path logs
        // everything — the one path an operator actually reports must not be the
        // thin one. `warn` (not `telemetry`) so it shows in the container log too.
        deps.logger.warn(
          {
            action: 'output_frame_gate_failed',
            jobId: job.id,
            fileId: file.id,
            expected: gate.expected,
            threshold: gate.threshold,
          ...
          'output frame gate FAILED — the encoded video track is short; failing the job',
        );
        failJob(
          deps,
          { job, file, expectedFileVersion, workDir },
          // The numbers belong in error_msg: queue + library render it verbatim,
          // so this is where the operator sees HOW far off the output was.
          `output_frame_gate:${gate.actual}/${Math.round(gate.expected)}`,
          encodeExitCode,
          encodeLogTail,
        );
        return { kind: 'fail' };
