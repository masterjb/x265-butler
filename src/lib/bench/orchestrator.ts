...
import type { EncoderId, QsvRateControl } from '../encode/profiles';
...
// 49-05: the DISPLAY name of the encoder-native quality flag, stored on the
// bench_combo row at CREATION time and rendered by the four /bench components.
// It is NOT an argv source — buildCodecBlock owns the flag dispatch
// (vmaf.ts:185), proven by tests/encode/bench-argv-isolation.test.ts.
//
// `hevc_nvenc` corrected '-cq' → '-qp': PROFILE_BUILDERS.nvenc emits
// `-rc constqp -qp <crf>` (profiles.ts) and has never emitted `-cq`. Without
// this, the same release would name "-qp (constqp)" in the Settings helper and
// print `-cq=<n>` two clicks away for the same encoder.
//
// `hevc_qsv` is NOT static — the flag depends on the tier the boot probe
// resolved (30-01), so it is looked up per row-creation below.
...
// 49-05: resolve the display name for ONE encoder at row-CREATION time.
//
// WHY creation and not rendering: the tier is not stored on the row. Resolving
// it in the render layer would reinterpret OLD rows with TODAY's host tier —
// that would not fix the historical rows, it would make them lie. New rows come
// out correct, existing rows stay exactly as they were: no migration, and the
// four bench render components are untouched.
//
// The `else` branch is not a stopgap: getActiveQsvRateControl() itself falls
// back to 'icq-full' on an empty cache (detection.ts:1160) and the encode really
// does emit `-global_quality` there. If that fallback is ever flipped, BOTH
// places have to move together.
export function resolveNativeQualityParam(
  encoder: string,
  qsvTier: QsvRateControl = getActiveQsvRateControl(),
): string {
  if (encoder === 'hevc_qsv') return qsvTier === 'cqp' ? '-q:v' : '-global_quality';
  return NATIVE_QUALITY_PARAM[encoder] ?? '-crf';
}
...
// 11-02-FIX (UAT-001): per-combo progress anchors. Phase windows of the overall
// 0-100% bar. Audit M3: phase change bypasses the 1Hz throttle (immediate emit).
// (28-08 L3: lifted from _executeRun-inline → module const for _makeComboProgressEmitter.)
const BENCH_PHASE_ANCHORS = {
  'sample-extraction': [0, 10],
  encode: [10, 70],
  vmaf: [70, 95],
  pareto: [95, 100],
} as const;
type BenchPhase = keyof typeof BENCH_PHASE_ANCHORS;
...
export class BenchOrchestrator {
  ...
  // 47-01: a run with status='complete' can STILL be busy — runFullFileVerify()
  // writes pass-2 metrics into that run's bench_combo rows long after the run itself
  // completed (see runFullFileVerify above / pass2InFlight lifecycle at :143-145 / :276-278).
  // Purging such a run would delete rows an in-flight encode is about to write to.
  isPass2ActiveForRun(runId: number): boolean {
    return this.isPass2Running && this.pass2InFlight?.runId === runId;
  }

  // 47-01: the SINGLE policy seam for bench-run purge (row-delete, NOT cancel).
  //
  // MUST STAY SYNCHRONOUS — correctness contract, not style:
  //  (a) TOCTOU: the guard-read (findById) and the deleteById are safe only because no
  //      `await` sits between them, so no scheduler tick can flip pending→running
  //      mid-purge. The sibling app/api/library/[id]/route.ts:105-118 needed an explicit
  //      re-read-inside-transaction precisely because its path IS async. A single `await`
  //      here silently reopens that race.
  //  (b) Composition: better-sqlite3 transactions are synchronous — POST /api/bench/bulk-delete
  //      calls this inside a per-id SAVEPOINT. An async purgeRun cannot compose there.
  //
  // Throws Error with `.code` ∈ {'run_not_found','active_run','active_pass2'} — same
  // convention the file already uses (cancelPass2 / 'pass2_busy'); no new error class.
  purgeRun(runId: number): { combosDeleted: number } {
    const run = this.benchRunRepo.findById(runId);
    if (!run) {
      const err = new Error(`bench_run ${runId} not found`);
      (err as Error & { code?: string }).code = 'run_not_found';
      throw err;
    }

    // Guard precedence (AC-3): active-run wins over active-pass2 — a run can satisfy both.
    if (run.status === 'pending' || run.status === 'running') {
      const err = new Error(`bench_run ${runId} is ${run.status}`);
      (err as Error & { code?: string; currentStatus?: string }).code = 'active_run';
      (err as Error & { code?: string; currentStatus?: string }).currentStatus = run.status;
      throw err;
    }

    if (this.isPass2ActiveForRun(runId)) {
      const err = new Error(`bench_run ${runId} has an in-flight pass-2 verify`);
      (err as Error & { code?: string }).code = 'active_pass2';
      throw err;
    }

    const combosDeleted = this.benchComboRepo.countByRun(runId);

    // AUDIT LINE FIRST (M5): after the delete the row is gone forever — this log is the
    // ONLY surviving evidence of what was destroyed. All fields come free from the
    // step-1 row. `actorId` is added route-side (the orchestrator has no auth context).
    logger.info(
      {
    ...
    // In-memory hygiene BEFORE the delete (audit S3): deleteById must be the LAST
    // statement that can fail, so nothing throws after the row is gone inside a
    // caller's SAVEPOINT. NOTE the maps are NOT transactional — a caller rollback
    // does not restore them. Benign in practice: only ACTIVE runs occupy these
    // maps, and an active run cannot reach this line (guard above). Recorded decision.
    this.cancelledRuns.delete(runId);
    this.progressThrottle.delete(runId);
    this.inFlightControllers.delete(runId);

    this.benchRunRepo.deleteById(runId);

    return { combosDeleted };
  }
  ...
  async enqueueRun(input: BenchRunCreateInput): Promise<{ runId: number }> {
    ...
    for (const fileId of input.fileIds) {
      for (const encoder of encoders) {
        const nativeQualityParam = resolveNativeQualityParam(encoder);
  ...
  private async _executeRun(runId: number): Promise<void> {
    ...
    // 28-08 MH-1: shared cross-combo counter threaded as a mutable ref into
    // _encodeOneCombo (bumped exactly-once on BOTH the success AND the
    // mid-encode-cancel/failure path; NOT on the pre-encode cancel-skip).
    const progress = { completed: 0, total: totalCombos };

    // 47-03 AC-9: combos whose source file was deleted between enqueue and
    // dispatch carry file_id IS NULL (migration 0029, ON DELETE SET NULL).
    // They match NO `run.fileIds` round below, so without this they would stay
    // 'pending' forever and the run would never reach a terminal status. Fail
    // them up front and advance the counter — `totalCombos` already counts
    // them, so skipping the bump would peg the progress bar below 100% on a
    // run that is in fact done.
    //
    // What this deliberately does NOT cover: a file deleted WHILE the run is
    // already going. `pendingCombos` is read ONCE above and kept in memory, so
    // a later SET NULL never reaches that snapshot; those combos still carry
    // their old file_id and land in the existing `if (!file)` branch below
    // (`file ${fileId} not found`). One null-handling site, not two.
    const orphanCombos = pendingCombos.filter((c) => c.file_id === null);
    if (orphanCombos.length > 0) {
      for (const combo of orphanCombos) {
        this.benchComboRepo.markComboFailed(combo.id, 'source_file_deleted');
      }
      progress.completed += orphanCombos.length;
      logger.warn(
        { runId, orphanCombos: orphanCombos.length },
        'bench: combos with deleted source file failed as source_file_deleted',
      );
    }
    ...
    try {
      for (const fileId of run.fileIds) {
        ...
        try {
          extractedPaths = await this._extractFileSamples(file, run, sampleDir, controller.signal);
        } catch (err) {
          if (this.cancelledRuns.has(runId)) break;
          const reason =
        ...
        for (const sampleIdx of sampleIdxSet) {
          ...
          const ctx = {
            ...
            sampleDir,
            sampleDurationSec,
            vmafModel: run.vmaf_model,
            signal: controller.signal,
          };

          // 28-08 D1=C: encode loop stays STRICTLY SERIAL (combo-2 dispatches only
          // after combo-1's encode+vmaf settle). P1 parallel-encode DROPPED — parallel
          // encodes corrupt the encodeSec measurement + HW encoders serialize at the
          // driver anyway. NO Promise.all / window / lanes here. Pinned by AC-3.
          for (const combo of sampleCombos) {
            await this._encodeOneCombo(combo, samplePath, sampleIdx, ctx, progress);
          }
        }
      }

      this._finalizeRun(runId);
    } catch (err) {
  ...
  // 28-08 L3: extract-once-per-file sample helper. THROWS on extractSamples
  // failure (does NOT swallow — the caller's try/catch keeps the
  // SampleExtractorError-vs-generic reason classification + cancel-recheck break).
  private async _extractFileSamples(
    file: FileRow,
    run: BenchRunRow,
    sampleDir: string,
    signal: AbortSignal,
    ...
    const extracted = await extractSamples(file.path, {
      ...
      durationSec: run.sample_duration_seconds,
      scratchDir: sampleDir,
      fileId: file.id,
      signal,
    });
    return new Map(extracted.map((r) => [r.sampleIdx, r.path]));
  }

  // 28-08 L3: per-combo throttled progress emitter (leading-edge 1Hz).
  // Closure scope = per-combo: the orchestrator iterates combos serially (D1=C),
  // so the closure dies between combos and throttle state can't leak.
  // Audit M3: phase change bypasses the throttle (immediate emit).
  private _makeComboProgressEmitter(
    runId: number,
    comboId: number,
  ): (phase: BenchPhase, phasePct: number) => void {
    let lastEmitMs = 0;
    let lastPhase: BenchPhase | null = null;
    return (phase, phasePct) => {
      const now = Date.now();
      const phaseChanged = phase !== lastPhase;
      if (!phaseChanged && now - lastEmitMs < 1000) return; // 1Hz leading-edge
      const [start, end] = BENCH_PHASE_ANCHORS[phase];
  ...
  // 28-08 L3: single-combo encode→vmaf→mark pipeline. Behavior-identical extraction
  // of the former inner combo body. `progress` is a SHARED mutable ref bumped
  // exactly-once on the success AND the mid-encode-cancel/failure path; the
  // pre-encode cancel-skip guard early-returns WITHOUT bumping (asymmetry pinned
  // by AC-7). Dispatched strictly serially by the caller (AC-3, P1 dropped).
  private async _encodeOneCombo(
    combo: BenchComboRow,
    samplePath: string,
    sampleIdx: number,
    ctx: {
      runId: number;
      mode: BenchRunRow['mode'];
      vmafTargets: number[] | undefined;
      sampleDir: string;
    ...
    const { runId, mode, vmafTargets, sampleDir, sampleDurationSec, vmafModel, signal } = ctx;

    // 47-03: file_id became nullable with migration 0029, but this method is
    // only ever reached for combos that matched a `run.fileIds` round in
    // _executeRun — a NULL file_id matches no round, and those combos were
    // already failed as 'source_file_deleted' before the loop. Narrowed ONCE
    // here rather than three times below, so the reasoning lives in one place;
    // deliberately an assertion and not a silent `?? 0`, which would emit a
    // bogus fileId=0 into the progress stream.
    const comboFileId = combo.file_id!;
    ...
    try {
      ...
      const emitComboProgress = this._makeComboProgressEmitter(runId, combo.id);
  ...
  // 28-08 L3: post-loop completion block. `!cancelled` + status==='running' guard,
  // recomputePareto → markComplete → summarize tally → bench.completed emit.
