import { walkFiles, emptyWalkStats, type FileEntry, type WalkStats } from './walker';
// 48-01: process-local last-scan integrity snapshot (diagnostics evidence).
...
type PerShareCounters = {
  ...
  filesFailed: number;
  // 48-01: walker WalkStats, per share. Directory axis — orthogonal to the
  ...
  // 48-02: fifth directory-axis counter, threaded exactly like the other four.
...
// 48-01: cap for the store-level sample lists concatenated across shares —
...
export async function runScan(
  ...
  // 33-01 (audit-SR-4b): resolve sidecar_mode + sidecar_central_path ONCE per
  ...
  // the 26-01 write side (beside / DEFAULT_SIDECAR_CENTRAL_PATH). A settings-read
  // failure soft-degrades to beside (byte-identical to pre-33-01) — never blocks the scan.
  ...
  // 33-01: reuse the single hoisted resolve above (no second settings.get).
  ...
  async function scanOneShare(
    ...
    // 48-01 (D1=A): the walker hands its directory-integrity counters back as
    // the generator's RETURN value, i.e. `next.value` on the `done: true`
    // result — which the manual-iterator drain below already reads. Capture it
    // there BEFORE breaking out; nothing else in this loop can observe it.
    ...
    for (;;) {
      ...
      while (window.length < SCAN_PROBE_CONCURRENCY) {
        ...
        if (next.done) {
          // 48-01: capture ONCE. The outer `for(;;)` re-enters this drain after
          // the final window is processed and calls .next() on the ALREADY
          // exhausted generator, which yields `{value: undefined, done: true}` —
          // assigning unconditionally would overwrite the real stats with zeros.
          // The `??` additionally covers a mocked walker in a foreign test whose
          ...
          try {
            decision = await runSkipPipeline(
              { filePath: entry.path, probe, diskContentHash: hashResult.value },
              {
                ...
                patternsCache,
                // 33-01: mode-aware Step-1 read (central → consult central sidecar
    ...
    return {
      ...
      filesUnchanged,
      filesFailed,
      dirsVisited: ws.dirsVisited,
      dirsSkippedCycle: ws.dirsSkippedCycle,
      dirsSkippedUnreadable: ws.dirsSkippedUnreadable,
      dirsSkippedMaxDepth: ws.dirsSkippedMaxDepth,
      dirsSkippedSystemPrefix: ws.dirsSkippedSystemPrefix,
      cycleSamples: ws.cycleSamples,
      unreadableSamples: ws.unreadableSamples,
      systemPrefixSamples: ws.systemPrefixSamples,
    };
  }
  ...
  // 48-01: directory-axis aggregation. The four counters travel on into
  ...
  if (isMultiShare) {
    ...
    for (const share of shares) {
      ...
      // 48-01 (audit-added M1): a share that walked 10.000 directories and only
      // THEN threw would otherwise be indistinguishable from an empty share.
    ...
    // 48-01 (audit-added M1 / AC-15): this path has NO per-share catch — a
    // walker root-stat failure propagates straight out of runScan, and
    // reconcile.ts:69-77 SWALLOWS it (`auto_scan_reconcile_run_scan_failed`,
  ...
  // 48-01: ONE store write per completed scan, AFTER the vanished-marking so the
  // snapshot describes a fully finished run. The failure counterpart sits in the
  // single-share catch above (AC-15).
  recordScanIntegrity({
    startedAtIso: new Date(startedMs).toISOString(),
