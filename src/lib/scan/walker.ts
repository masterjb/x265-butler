...
export async function* walkFiles(
  ...
  // emit a misleading partial statistic. NEVER at debug level: debug is dropped
  // at the `LOG_LEVEL ?? 'info'` instance gate BEFORE the multistream fan-out, so
  // it would never reach the ring-buffer (the 22-01 → 38-02 dark-surface trap).
