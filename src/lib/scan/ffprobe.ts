...
export type ProbeStream = {
  ...
  channels?: number;
  // 49-01: `disposition.attached_pic === 1` from the SAME -show_streams JSON
  // (NO new ffprobe spawn, NO new argv flag). REQUIRED — not optional — so tsc
  // flags every ProbeStream literal that forgets it instead of letting
  // `undefined` flow silently into analyzeAttachedPictures (48-03-M1 pattern:
  // required field as a compiler gate). Embedded cover art demuxes as a VIDEO
  // stream, so `-map 0:v` matches it and the global `-c:v <enc>` would push a
  // 600x900 yuvj444p mjpeg through the HEVC encoder → job dies.
  attachedPic: boolean;
  // 49-01 additive (OPTIONAL — no fixture churn): per-stream metadata tags,
  // UPPER-normalized exactly like normalizeTags does for format tags. Sole
  // purpose: recognize a cover in OUR OWN mkv output. The matroska muxer does
  // NOT write the attached_pic disposition back — it writes the copied cover as
  // a plain video track that keeps only FILENAME/MIMETYPE (measured, ffmpeg
  // 6.1.1). Without this a second encode over our own output would be fatal
  // again. The mp4 muxer DOES preserve attached_pic.
  tags?: Record<string, string>;
  // 50-01 additive (OPTIONAL — E2, no fixture churn): parsed avg_frame_rate in
  // frames per second, or undefined when ffprobe reported '0/0' / 'N/A' / a
  // shape parseFrameRate cannot read. Deliberately NOT required: the frame gate
  // is fail-OPEN by D2, so the runtime MUST handle `undefined` anyway — a
  // compiler gate would force churn across 109 ProbeStream literals in 11 test
  // files for a state that is legal. Precedent: 49-01 `tags`.
  avgFrameRate?: number;
};
...
interface FfprobeStream {
  ...
  // 49-01 additive: ffprobe emits the disposition object for every stream under
  // -show_streams already. All values are 0/1 integers.
  disposition?: Record<string, number>;
  // 49-01 additive: per-stream tags (Matroska AttachedFile carries
  // filename/mimetype here after the demuxer turns it into a video stream).
  tags?: Record<string, string>;
  // 50-01 additive: ffprobe writes avg_frame_rate for EVERY stream under
  // -show_streams already (audio streams carry the '0/0' sentinel). NO new
  // ffprobe argument, NO second spawn.
  ...
  return new Promise<ProbeResult | null>((resolve) => {
    ...
    child.once('close', (code) => {
      ...
      const streams: ProbeStream[] = (parsed.streams ?? []).map((s, idx) => ({
        ...
        // 49-01: strict === 1, NOT a truthy test. ffprobe writes only 0 or 1
        // here; a truthy test would silently reinterpret a future string-valued
        // disposition. Missing disposition object ⇒ false.
        attachedPic: s.disposition?.attached_pic === 1,
        // 49-01: undefined (not {}) when the stream carries no tags, so the
        // field stays invisible to every pre-49 consumer.
        tags: s.tags ? normalizeTags(s.tags) : undefined,
        // 50-01: parsed from the SAME -show_streams JSON. undefined for audio
        // ('0/0') and for any unreadable shape — see parseFrameRate.
...
// ── 50-01: frame-gate probe layer ─────────────────────────────────────────────

/**
 * 50-01: parse an ffprobe `avg_frame_rate` value into frames per second.
 *
 * TOTAL by contract (AC-11) — returns `undefined` for every shape it cannot
 * read and NEVER NaN / Infinity / 0 / a negative:
 *
 *   '24/1'        → 24
 *   '24000/1001'  → 23.976023976…
 *   '0/0'         → undefined   (the sentinel ffprobe writes for audio streams)
 *   'N/A' / 'n/a' → undefined
 *   '' / undefined→ undefined
 *   '30'          → 30          (no slash ⇒ plain number)
 *   'x/y'         → undefined
 *
 * MEASURED CAVEAT (50-01 M-A, ffmpeg 6.1.1): this value does NOT collapse when
 * the video track is truncated — a Matroska track header keeps `24/1` even when
 * only 17 packets sit in the stream. That is precisely why the frame gate needs
 * a SECOND, counted number (countVideoPackets) and cannot be a one-field check.
 */
...
export type CountVideoPacketsOptions = {
  timeoutMs?: number;
  /**
   * 50-01 (audit MH-2): which VIDEO ordinal to count. NOT optional out of
   * convenience — a hardcoded `v:0` would, on an output whose cover art is video
   * ordinal 0, count the ONE cover packet against an expectation derived from a
   * DIFFERENT stream. Both halves of the comparison must provably mean the same
   * stream. Default 0 covers the trivial single-video-track case only.
   */
  videoOrdinal?: number;
};

// 50-01: the count run demuxes the WHOLE file, unlike the metadata probe. CONTEXT
// M6 measures ~0.07 s for 54 MB / 14 400 packets (≈770 MB/s pure demux, no
// decode) ⇒ ~6 s for a 4.3 GB output on cache-pool I/O. A slow spinning array
// must not run into a false positive here — and a timeout is fail-OPEN anyway (E6).
const COUNT_PACKETS_TIMEOUT_MS = 300_000;

/**
 * 50-01: count the packets of ONE video stream in a finished file.
 *
 * `ffprobe -select_streams v:<ordinal> -count_packets -show_entries
 * stream=nb_read_packets -of csv=p=0` — the only way to learn the REAL number of
 * video packets in a container that reports `nb_frames=N/A` (CONTEXT M5: the
 * Matroska muxer writes no frame count, so no single metadata field can answer
 * this without a second spawn).
 *
 * Returns `null` — never throws — on: an invalid ordinal (no spawn at all), a
 * spawn error (ENOENT), a timeout, a non-zero exit, an over-cap stdout, or an
 * unparsable answer. Every one of those is fail-OPEN at the call site (E6): a
 * failed CHECK is not evidence of a broken output.
 *
 * NO reniceChild (E5): scan/ffprobe.ts has never renice'd — only the four ffmpeg
 * children do (ffmpeg.ts / cropdetect.ts / detection.ts / ffmpeg-version.ts).
 * This spawn mirrors its neighbour in the same module; 0.054 s of demux is not a
 * scheduler concern.
 */
export async function countVideoPackets(
  filePath: string,
  opts: CountVideoPacketsOptions = {},
): Promise<number | null> {
  const timeoutMs = opts.timeoutMs ?? COUNT_PACKETS_TIMEOUT_MS;
  const videoOrdinal = opts.videoOrdinal ?? 0;

  if (!Number.isInteger(videoOrdinal) || videoOrdinal < 0) {
    logger.warn(
      { action: 'count_video_packets_invalid_ordinal', filePath, videoOrdinal },
      'countVideoPackets: invalid video ordinal — no spawn',
    );
    return null;
  }
  ...
  return new Promise<number | null>((resolve) => {
    ...
    const safeResolve = (v: number | null): void => {
      if (resolved) return;
      resolved = true;
      resolve(v);
    };
    ...
    child.on('error', (err) => {
      ...
      logger.warn(
        {
          action: 'count_video_packets_spawn_failed',
          filePath,
          videoOrdinal,
          err: err instanceof Error ? err.message : String(err),
        },
        'countVideoPackets: spawn failed',
      );
      safeResolve(null);
    });

    // Resolve only on `close` (never `exit`) — mirrors ffprobe() audit M2: after
    // a kill the kernel still reaps the child and emits close, so awaiting close
    ...
    child.once('close', (code) => {
      ...
      if (timedOut) {
        logger.warn(
          { action: 'count_video_packets_timeout', filePath, videoOrdinal, timeoutMs },
          'countVideoPackets: timeout, killed',
        );
        safeResolve(null);
        return;
      }
      if (stdoutCapped) {
        logger.warn(
          {
            action: 'count_video_packets_stdout_capped',
            filePath,
            videoOrdinal,
            capBytes: STDOUT_CAP_BYTES,
          },
          'countVideoPackets: stdout exceeded cap, killed',
        );
        safeResolve(null);
        return;
      }
      if (code !== 0) {
        logger.warn(
          {
            action: 'count_video_packets_nonzero_exit',
            filePath,
            videoOrdinal,
            code,
            stderr: stderrTail.toString('utf8'),
          },
          'countVideoPackets: non-zero exit',
        );
        safeResolve(null);
        return;
      }
      ...
      if (raw === '' || !Number.isInteger(n) || n < 0) {
        logger.warn(
          {
            action: 'count_video_packets_unparsable',
            filePath,
            videoOrdinal,
            raw: raw.slice(0, 200),
          },
          'countVideoPackets: unparsable output',
        );
        safeResolve(null);
        return;
      }
