...
export async function runEstimate(
  ...
  for await (const entry of walkFiles(opts.rootPath, {
    ...
    // L7 (28-04): per-file skip-classification extracted into classifyEstimateEntry
    // so the walk loop reads as a clean dispatch. Behavior-identical: the helper
    // encapsulates EXACTLY today's hash → skip-check → ffprobe sequence (incl. the
    // hash-failed and ffprobe-rejected warn-logs); the loop keeps the OUTER concerns
    // (abort, cap, buckets.scanned++) inline and switches on the tagged result.
    const classification = await classifyEstimateEntry(entry, deps, patternsCache);
    switch (classification.bucket) {
      ...
      case 'hash-failed':
        ...
        eligibleSizes.push(classification.size);
        eligibleDurations.push(null);
        buckets.eligible++;
        break;
      case 'eligible':
        // ffprobe duration may be null on reject — still count by size (SR3
        // partial-duration scaling in estimator upscales rawSeconds proportionally).
        eligibleSizes.push(classification.size);
        eligibleDurations.push(classification.duration);
        buckets.eligible++;
        break;
    }
  }
...
// L7 (28-04): single-file classification dispatch extracted from runEstimate's
// walk loop. Returns a small tagged result; the caller maps it to bucket
// increments + eligible-size/duration pushes. Pure readability — byte-identical
// per-entry behavior (same hash → skip-check → ffprobe sequence, same warn-logs).
type EstimateEntryClassification =
  | { bucket: 'sidecar' }
  | { bucket: 'blocklist' }
  | { bucket: 'eligible'; size: number; duration: number | null }
  | { bucket: 'hash-failed'; size: number };

async function classifyEstimateEntry(
  entry: FileEntry,
  deps: EstimateDeps,
  patternsCache: BlocklistRow[],
): Promise<EstimateEntryClassification> {
  let hash: string;
  try {
    ...
    return { bucket: 'hash-failed', size: entry.size };
  }
  ...
  if (skip === 'sidecar') return { bucket: 'sidecar' };
  if (skip === 'blocklist') return { bucket: 'blocklist' };

  // Eligible — ffprobe for duration only.
  ...
  return { bucket: 'eligible', size: entry.size, duration };
}
