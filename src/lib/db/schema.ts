...
export interface ScanResult {
  ...
  // 48-01: DIRECTORY-axis integrity counters from the walker (WalkStats),
  // orthogonal to the files* invariant above — they count directories, never
  // files, so they neither feed nor disturb
  // filesAdded+filesUpdated+filesUnchanged+filesFailed === filesScanned.
  // NON-OPTIONAL by design (precedent: filesVanished): `tsc --noEmit` forces
  // every ScanResult literal to enumerate them instead of drifting silently.
  // Skip SAMPLES, outcome and timestamps deliberately do NOT live here —
  // ScanResult is the HTTP body of /api/scan and must not hand raw filesystem
  // paths to every API consumer; those live in the scan-integrity store.
  dirsVisited: number;
  dirsSkippedCycle: number;
  dirsSkippedUnreadable: number;
  dirsSkippedMaxDepth: number;
  // 48-02: DELIBERATE system-prefix prunes (/proc, /sys, /dev, /run). Same
  // NON-OPTIONAL choice as its four siblings — `tsc --noEmit` enumerating every
  // ScanResult literal that must be updated IS the point, not a regression.
  ...
  byShare?: Array<{
    ...
    filesUnchanged: number;
    filesFailed: number;
    // 48-01: same directory axis, per share.
    dirsVisited: number;
    dirsSkippedCycle: number;
    dirsSkippedUnreadable: number;
    dirsSkippedMaxDepth: number;
...
export interface BenchComboRow {
  id: number;
  run_id: number;
  // 47-03: nullable since migration 0029 (ON DELETE SET NULL). NULL means the
  // source file was deleted from the library after this combo was created —
  // the measurements survive, the file mapping does not. BenchComboCreateInput
  // stays non-null: at enqueue time the file exists by definition.
  file_id: number | null;
  encoder: string;
  preset: string | null;
