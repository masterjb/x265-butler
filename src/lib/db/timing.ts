...
//
// 38-02 (dark-since-22-01 fix): this event was emitted at logger.debug, which
// does not reach the ring-buffer stream → the writer never saw it →
// /api/diagnostics.slowQueries.topN was permanently [] and slow request-path
// queries were invisible in container logs from 22-01 until 38-02.
// Emitting at WARN reaches BOTH stdout and the ring-buffer (≥ `telemetry` (25)
// reaches the ring; `debug` (20) does not — tier table in src/lib/logger.ts).
// NOTE: since 49-04 `telemetry` (25) would ALSO reach the ring — quietly. WARN
// is KEPT DELIBERATELY here (D-C): 38-02 recorded the choice for
