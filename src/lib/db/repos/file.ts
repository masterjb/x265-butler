...
export interface FileRepo {
  ...
  // 47-03 (replaces the 24-04 F6 any-bench-reference guard): is the file
  // referenced by >=1 bench_combo row of an ACTIVE run ('pending'|'running')?
  // Since migration 0029 bench_combo.file_id is ON DELETE SET NULL, so a
  // terminal run ('complete'|'failed'|'cancelled') no longer blocks the delete —
  // its measurements survive with file_id IS NULL. Only a live run does.
  isReferencedByActiveBench(id: number): boolean;
  // 47-03 AC-14: how many bench_combo rows point at this file? Called right
  // before deleteById so the audit line can report how many references the
  // ON DELETE SET NULL severed — a DB-level effect that is otherwise invisible
  // to the application.
  countBenchRefs(id: number): number;
}
...
export function makeFileRepo(db: Db): FileRepo {
  ...
  // 47-03 AC-15: the set of ACTIVE bench_run statuses is enumerated HERE and
  // NOWHERE ELSE. The full domain per migrations/0019_bench_run.sql:9 is
  // 'pending' | 'running' | 'complete' | 'failed' | 'cancelled'; the two below
  // block a library delete, the other three do not.
  // This is an allowlist on the BLOCKING side, i.e. a denylist on the ALLOWING
  // side: a bench_run status added later falls through to "terminal" with no
  // code change and would silently let a delete through that tears a real run
  // apart. Whoever adds a status MUST classify it right here.
  const isReferencedByActiveBenchStmt = db.prepare<[number], { one: number }>(
    `SELECT 1 AS one
       FROM bench_combo bc
       JOIN bench_run br ON br.id = bc.run_id
      WHERE bc.file_id = ? AND br.status IN ('pending','running')
      LIMIT 1`,
  );
  // 47-03 AC-14: plain reference count for the audit trail (ALL runs, terminal
  // included — these are exactly the references ON DELETE SET NULL severs).
  // idx_bench_combo_run_file does not cover file_id as its leading column;
  // accepted at the row counts realistic here, no new index in this plan.
  const countBenchRefsStmt = db.prepare<[number], { c: number }>(
    'SELECT COUNT(*) AS c FROM bench_combo WHERE file_id = ?',
  );
  ...
  return {
    ...
    isReferencedByActiveBench(id: number): boolean {
      // 47-03: narrowed from "referenced at all" to "referenced by a live run".
      // Used BOTH as the route pre-check AND as the in-transaction re-check —
      // after 0029 the FK can no longer raise, so this is the only guard left.
      const row = isReferencedByActiveBenchStmt.get(id);
      return row !== undefined;
    },

    countBenchRefs(id: number): number {
      return countBenchRefsStmt.get(id)!.c;
    },
  };
}
