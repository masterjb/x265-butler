...
export function makeFileRepo(db: Db): FileRepo {
  ...
  function getListStmt(sort: SortKey, dir: SortDir): ListStmt {
    ...
    if (!stmt) {
      ...
      stmt = db.prepare<unknown[], FileRow>(
        // 05-bonus: extra `AND (? = 1 OR status != 'vanished')` clause hides
        // vanished rows by default. Caller passes 1 when includeVanished=true
        // OR when status filter is explicitly 'vanished' (the explicit filter
        // already narrows to status='vanished', so the OR is a no-op there).
        // 07-01: extra `AND (? IS NULL OR file.id = ?)` clause narrows to a
        // single file.id when set; bypassed (NULL) for normal listing.
        // 14-03: SHARE clause appended AFTER idFilter — CASE selector encodes
        // (NULL=all / -1=orphan / N>0=share_id=N). Sentinel encoding lives in
        // the binder, not the SQL — keeps `?` count tractable (3 placeholders).
        // 15-01: pathPrefix STARTS WITH clause — escape-bound, positional
        // (? IS NULL bypass). Appended after SHARE clause to keep prior bind
        // order stable.
        `SELECT * FROM file
         WHERE (? IS NULL OR LOWER(path) LIKE LOWER(?) ESCAPE '\\')
  ...
  function getIterateStmt(sort: SortKey, dir: SortDir): ListStmt {
    ...
    if (!stmt) {
      ...
      const order = dir === 'asc' ? 'ASC' : 'DESC';
      stmt = db.prepare<unknown[], FileRow>(
        // 07-01: idFilter clause mirrors getListStmt for parity (CSV export
        // through iterateAll respects ?file=N if a future caller wires it).
        // 14-03: SHARE clause mirrors getListStmt for parity (CSV export honors
        // ?share=N through iterateAll, same CASE-selector semantics).
        // 15-01: pathPrefix clause mirrors getListStmt for CSV-export parity.
        // 15-02 will surface the field on /api/library/export.csv when the
        // operator request needs it; until then the binder passes null.
        `SELECT * FROM file
         WHERE (? IS NULL OR LOWER(path) LIKE LOWER(?) ESCAPE '\\')
           AND (? IS NULL OR status = ?)
           AND (? = 1 OR status != 'vanished')
  ...
  function getCountStmt(): CountStmt {
    ...
    if (!stmt) {
      stmt = db.prepare<unknown[], { c: number }>(
        // 07-01: COUNT honors idFilter so pagination math (pageCount) stays
        // consistent with the LIST result (1 row → total=1 → pageCount=1).
        // 14-03: COUNT honors SHARE clause for same pagination invariant.
        // 15-01: pathPrefix clause mirrors getListStmt — pagination math
        // (pageCount) stays consistent with the filtered LIST result.
  ...
  return {
    ...
    listPaginated(opts: ListOptions): ListResult {
      ...
      return withQueryTiming('fileRepo.listPaginated', () => {
        ...
        const rows = getListStmt(opts.sort, opts.dir).all(
          likeParam,
          likeParam,
          statusParam,
          statusParam,
