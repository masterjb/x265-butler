...
interface BenchComboRaw {
  id: number;
  run_id: number;
  file_id: number | null; // 47-03: ON DELETE SET NULL since migration 0029
  encoder: string;
  preset: string | null;
...
export class BenchComboRepo {
  ...
  // 47-01 (audit S1): scalar count for the purge response payload.
  // NOT listByRun(id).length — a combo count is encoders × presets × values × files
  // × samples (routinely thousands of rows), and the bulk-purge path would materialize
  // + hydrate up to MAX_BULK=500 such sets inside ONE open write transaction purely to
  // produce integers. Hits the existing idx_bench_combo_run (0020_bench_combo.sql:26).
  countByRun(runId: number): number {
    const row = this.db
      .prepare('SELECT COUNT(*) AS c FROM bench_combo WHERE run_id=?')
