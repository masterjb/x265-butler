...
export function makeStorageRepo(db: Db, deps: StorageRepoDeps): StorageRepo {
  ...
  // 28-01 P5: grouped (GROUP BY share_id) — ONE query for all named shares,
  // replacing the former per-share `WHERE share_id = ?` N+1 in getSharesTable.
  // Scoped to share_id IS NOT NULL so the orphan bucket (share_id IS NULL) can't
  // form a stray NULL-keyed group; orphan stays sourced from orphanAggregateStmt.
  const sharesAggregateGroupedStmt = db.prepare<
    [],
    {
      share_id: number;
      totalSize: number;
  ...
  // 28-01 P5: grouped (GROUP BY f.share_id) — ONE query for all named shares,
  // replacing the former per-share `WHERE f.share_id = ?` N+1. Named-share-scoped
  // (f.share_id IS NOT NULL). A share with no done-smaller savings emits NO group
  // row → the JS Map lookup defaults to 0 (byte-identical to the old COALESCE-zero).
  const savingsByShareGroupedStmt = db.prepare<[], { share_id: number; savings: number }>(
    `SELECT f.share_id AS share_id,
            COALESCE(SUM(j.bytes_in - j.bytes_out), 0) AS savings
     FROM file f
     JOIN job j ON j.file_id = f.id AND j.status = 'done'
     WHERE f.status = 'done-smaller'
       AND f.share_id IS NOT NULL
  ...
  return {
    ...
    getSharesTable(): ShareTableRow[] {
      ...
      // 28-01 P5: batch the per-share size/hevc + savings aggregates into TWO
      // grouped queries (was sharesAggregateStmt.get(id) + savingsByShareStmt.get(id)
      // per share → N+1). Assemble rows by Map lookup below.
      const aggByShare = new Map<number, { totalSize: number; hevcBytes: number }>();
      for (const a of sharesAggregateGroupedStmt.all()) {
        aggByShare.set(a.share_id, { totalSize: a.totalSize, hevcBytes: a.hevcBytes });
      }
      const savingsByShare = new Map<number, number>();
      for (const sv of savingsByShareGroupedStmt.all()) {
        savingsByShare.set(sv.share_id, sv.savings);
      }

      for (const s of shares) {
        // 28-01 M2: default-on-miss is REQUIRED — a named share with zero files
        // (or zero done-smaller savings) produces NO group row, so map.get()
        // returns undefined. The old per-share non-GROUP-BY query always returned
        // one COALESCE-zeroed row; { totalSize: 0, hevcBytes: 0 } / 0 reproduces
        // it byte-identically (reading an undefined agg would yield NaN/crash).
        const agg = aggByShare.get(s.id) ?? { totalSize: 0, hevcBytes: 0 };
        const totalSize = agg.totalSize;
        const hevcBytes = agg.hevcBytes;
        const hevcPercent = totalSize > 0 ? Math.round((hevcBytes / totalSize) * 10000) / 100 : 0;
        const savingsBytes = savingsByShare.get(s.id) ?? 0;
        // largestFolder stays per-share (getTopFoldersImpl) — top-N-per-group is a
        // separate batching problem, out of scope per 28-01 boundaries.
