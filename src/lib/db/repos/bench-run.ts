...
export class BenchRunRepo {
  ...
  // 47-01: dumb row-delete, NO policy. All guards (status / pass-2 busy) live in
  // BenchOrchestrator.purgeRun() — the single policy seam.
  // Deliberately WITHOUT a status filter in SQL: a WHERE-guarded delete collapses
  // "row absent" and "row present but active" into changes=0, which would make the
  // route unable to tell 404 from 409.
  // No OCC/version check either: purge is terminal — there is no lost-update to protect.
  // bench_combo rows drop via run_id ... ON DELETE CASCADE (migrations/0020_bench_combo.sql:7)
  // with PRAGMA foreign_keys=ON (src/lib/db/index.ts:98).
  deleteById(id: number): boolean {
    const result = this.db.prepare('DELETE FROM bench_run WHERE id=?').run(id);
