...
export interface BlocklistRepo {
  ...
  // 50-04: batched file-pinned lookup for the Library list payload. The Library
  // page renders up to 200 rows; the UI needs the blocklist_entry.id for every
  // row it wants to offer an "unblock" action on. One IN-query instead of N
  // findByFileId calls (measured M-4: 0.131ms vs 0.190ms for 200 ids — both
  // irrelevant, the IN-query wins on being a single statement).
  //
  // Pattern entries can never appear in the result: they carry file_id IS NULL
  // and `file_id IN (…)` excludes NULL by SQL semantics. That is a consequence
  // of the predicate, not an extra filter.
  findByFileIds(ids: readonly number[]): BlocklistRow[];
...
export function makeBlocklistRepo(db: Db): BlocklistRepo {
  ...
  // 50-04: chunked at 500 to stay below SQLite SQLITE_MAX_VARIABLE_NUMBER=999,
  // mirroring FileRepo.listByIds (repos/file.ts). The Library page caps at
  // size=200, so the chunk boundary is defensive rather than load-bearing —
  // but a future bulk consumer must not hit the scaling cliff.
  function findByFileIds(ids: readonly number[]): BlocklistRow[] {
    if (ids.length === 0) return [];
    return withQueryTiming('blocklistRepo.findByFileIds', () => {
      const CHUNK = 500;
      const out: BlocklistRow[] = [];
      for (let i = 0; i < ids.length; i += CHUNK) {
        const slice = ids.slice(i, i + CHUNK);
        const placeholders = slice.map(() => '?').join(',');
        const stmt = db.prepare<number[], BlocklistRow>(
          `SELECT * FROM blocklist_entry WHERE file_id IN (${placeholders})`,
        );
