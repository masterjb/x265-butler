...
export async function assembleDiagnostics(): Promise<DiagnosticsPayload> {
  ...
  try {
    ...
    encoders = {
      ...
      outcome: ENCODER_IDS.map((enc) => {
        ...
        // 49-03: absent key ⇒ 'not-probed'. That is the honest label for every
        ...
        // 49-05: only qsv HAS a ratecontrol tier. Everyone else says
  ...
  // 46-02 (D2): NVIDIA GPU model + driver (boot-cached singleton). Evidence-only
  ...
  // 48-01: last-scan directory-integrity evidence. Direct getter call, NOT a
