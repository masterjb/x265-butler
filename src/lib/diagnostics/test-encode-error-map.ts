...
// 45-02 (audit-S1): the SINGLE source of the NVENC API-floor stderr match.
// Consumed by this map (the nvencApiTooNew PatternEntry) AND by detection.ts
// probeEncodeOnce. Keep it here so the two classifiers can NEVER diverge on the
// variant ffmpeg wordings of "Driver does not support the required nvenc API
// version. Required: X.Y Found: A.B". `lc` = already-lowercased stderr.
export function isNvencApiTooNewStderr(lc: string): boolean {
  return (
    lc.includes('required nvenc api version') ||
    (lc.includes('nvenc') && /required:\s*[\d.]+\s+found:\s*[\d.]+/.test(lc))
  );
}

// 46-01: the SINGLE source of the "NVENC could not open the HEVC encoder" stderr match.
// Consumed by this map (the nvencHevcUnsupported PatternEntry) AND by detection.ts
// probeEncodeOnce — keep it here so the two classifiers can NEVER diverge.
// Signature: a hevc_nvenc open that fails with EINVAL and no more-specific NVENC reason
// = the GPU most likely has no HEVC-NVENC encode block (Kepler / Maxwell-1; HEVC NVENC
// needs Maxwell-2 GM206+). `lc` = already-lowercased stderr. The "invalid argument"
// (EINVAL) co-occurrence is REQUIRED so a non-EINVAL "could not open encoder" (a
// different fault) does not get mislabelled as an unsupported-codec hardware limit.
export function isNvencHevcUnsupportedStderr(lc: string): boolean {
  return lc.includes('could not open encoder') && lc.includes('invalid argument');
}

// 49-03 (audit M-1 / AC-19): the SINGLE source of the "this runtime REJECTS the
// forced-IDR option" stderr match. Consumed by detection.ts's forced-IDR confirm
// spawn — same SINGLE-SOURCE invariant that binds isNvencApiTooNewStderr /
// isNvencHevcUnsupportedStderr: never a second inline regex.
//
// WHY THE OPTION-NAME CO-OCCURRENCE IS MANDATORY, NOT DECORATIVE: the confirm
// spawn is the THIRD GPU-session attempt of the same probe chain, and it starts
// while the parallel vaapi probe may still hold the very same Intel iGPU. That
// self-contention IS 39-01 (the AMD card lost until restart). A device-busy /
// OOM / driver-hiccup EXIT_NONZERO must therefore NOT be read as an option
// rejection — it would silently undo the 49-02 fix on a HEALTHY host for the
// whole process lifetime AND assert a runtime limitation to the operator that
// does not exist. Absent the option name, the caller falls OPEN (D4).
//
// Both spellings are matched: `hevc_qsv` spells it `forced_idr` (UNDERSCORE),
// `hevc_nvenc` spells it `forced-idr` (HYPHEN) — verbatim from the binaries'
// own option tables (49-02 M3/M5d). The error wordings are the ones already
// recorded in CLAUDE.md for this class of AVOption rejection. `lc` = an
// already-lowercased stderr tail.
export function isForcedIdrRejectedStderr(lc: string): boolean {
  if (!lc.includes('forced_idr') && !lc.includes('forced-idr')) return false;
  return (
    lc.includes('error setting option') ||
    lc.includes('unrecognized option') ||
    lc.includes('invalid argument')
  );
}
...
export const TEST_ENCODE_ERROR_PATTERNS: readonly PatternEntry[] = [
  ...
  {
    // 45-02: the compile-baked NVENC API floor is HIGHER than the GPU's driver
    // exposes ("Required: 13.1 Found: 13.0") — NOT a too-old driver (Pascal/
    // Maxwell can NEVER get a newer driver), so it is distinct from
    // nvencDriverMismatch and MUST precede it: nvencDriverMismatch's
    // `driver does not support` substring also matches this string. severity
    // 'warning' (recoverable — the dual-binary path or a different GPU works).
    code: 'nvencApiTooNew',
    severity: 'warning',
    vendors: ['nvenc'],
    test: (lc) => isNvencApiTooNewStderr(lc),
  },
  {
  ...
  {
    // 46-01: hevc_nvenc open fails EINVAL with no more-specific NVENC reason → the GPU
    // has no HEVC-NVENC encode block (pre-Maxwell-2: Kepler / Maxwell-1). A hardware-
    // generation limit, not a broken build — severity 'warning' (libx265 works; nothing
    // to fix). MUST come after nvencApiTooNew + nvencDriverMismatch (their strings can
    // also contain "could not open encoder"); this is the nvenc open-failure catch of
    // last resort BEFORE the generic HW fallback.
    code: 'nvencHevcUnsupported',
    severity: 'warning',
    vendors: ['nvenc'],
    test: (lc) => isNvencHevcUnsupportedStderr(lc),
  },
  {
    code: 'vaapiNoDisplay',
    severity: 'error',
