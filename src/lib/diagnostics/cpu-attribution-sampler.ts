...
//  - Emit level ≥ `telemetry` (25) so the line reaches the ring-buffer stream
//    (AC-3). `debug` (20) does NOT reach it — that is the precise 22-01→38-02
//    dark-surface bug. 49-04 moved the QUIET branch from `info` to the new
//    `telemetry` tier: still recorded in the ring (and therefore in
//    /api/diagnostics + the copy-report), but no longer on stdout every 15s,
//    which is what made the container log unreadable (N100 forum report G4).
//    The JANKY branch stays at `warn` and stays loud, deliberately — the spam
//    came from the uninteresting ticks.
...
import { queueCountsFromRows } from '@/src/lib/queue/counts';
 ...
 * (janky), TELEMETRY otherwise. Extracted so the gate decision is unit-testable
 * deterministically (the live histogram p99 is not injectable). BOTH levels are
 * ≥ `telemetry` (25) so the line reaches the ring-buffer stream (AC-3);
 * `debug` (20) would not. The threshold semantics (strictly greater) are
 * unchanged from 40-01 — 49-04 renamed the quiet branch, nothing else.
 */
export function pickCpuAttributionLevel(p99Ms: number, thresholdMs: number): 'warn' | 'telemetry' {
  return p99Ms > thresholdMs ? 'warn' : 'telemetry';
}
...
export function startCpuAttributionSampler(): void {
  ...
  handle.timer = setInterval(() => {
    ...
    try {
      ...
      // sampler.
      // 48-03 (R3): encoding-only. listActive() selects queued+encoding, so the
      // old `.length` claimed 999 activeEncodes while four ffmpeg processes ran.
      // The value flows through the ring buffer into /api/diagnostics.cpuAttribution
      // and the copy-report forum posts are built from — a wrong number there
      // spoils exactly the post-incident reconstruction this surface exists for.
      // Field name unchanged (parser + copy-report read it); only the value.
      let activeEncodes = 0;
      try {
        activeEncodes = queueCountsFromRows(jobRepo().listActive(), 0).encodingJobs;
      } catch {
        activeEncodes = 0;
      }
      ...
      // ≥ telemetry both branches (AC-3): warn when the loop is janky (loud on
      // stdout, unchanged from 40-01), telemetry otherwise (ring only).
      ...
      if (pickCpuAttributionLevel(eventLoopLagP99Ms, lagWarnMs) === 'warn') {
        ...
        logger.telemetry(payload, 'cpu_attribution');
      }
    } catch (err) {
      logger.error(
        {
