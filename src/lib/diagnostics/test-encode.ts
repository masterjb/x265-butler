...
import { closedGopEnabled } from '@/src/lib/encode/keyframe';
...
// probe, same shared const, and the envelope (testsrc 320x240 duration=5, `-t 5`,
// `-f null /dev/null`) stays UNCHANGED. `testsrc` emits rgb24; a modern
// `hevc_qsv` advertises RGB in `pix_fmts`, so ffmpeg inserts no converter and iHD
// rejects the frame → this test encode reported "QSV broken" on a host whose real
// encodes ran fine. libx265/vaapi deliberately get no pin (see profiles.ts).
// 49-03 (D7, AC-13): `forceIdr` is THREADED IN (not global-read) so this builder
// stays pure/deterministic per 30-01 SR-3. runTestEncode resolves the SAME value
// buildArgs resolves — otherwise the test encode would validate an argv the
...
export interface RunTestEncodeOptions {
  ...
  // selector picks by det.activeFromAuto (nvenc → jellyfin ffmpeg-nvenc, else BtbN)
  // so the diagnostics test-encode agrees with the real encode on Pascal hosts.
  ffmpegPath?: string;
  timeoutMs?: number;
}
...
export async function runTestEncode(opts: RunTestEncodeOptions): Promise<RunTestEncodeResult> {
  ...
  // env lever AND runtime verdict. Without this the test encode would validate an
  // argv the production path no longer emits on a degraded host, i.e. the
  // diagnosis would contradict production again, one layer over.
  const args = buildTestEncodeArgs(
    det.activeFromAuto,
    det.vaapiDevice,
    det.qsvRateControl,
    closedGopEnabled() && isForcedIdrSupported(det.activeFromAuto),
  );
  ...
  const child = spawn(opts.ffmpegPath ?? ffmpegBinaryFor(det.activeFromAuto), args, {
