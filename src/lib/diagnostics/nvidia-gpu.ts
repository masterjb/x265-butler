...
export async function probeNvidiaGpu(deps: NvidiaGpuDeps = {}): Promise<NvidiaGpuBlock> {
  ...
  // One resolved-once log at the point the FRESH probe completes — info level so
  // it survives the `LOG_LEVEL ?? 'info'` gate → ring-buffer (the 22-01→38-02
  // dark-surface lesson). source + count only; NEVER the GPU name (kept to the
