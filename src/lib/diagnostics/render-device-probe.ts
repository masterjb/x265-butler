...
// 29-02 — re-export the single-source group-fix predicate. The definition lives
// in the node:fs-free leaf render-device-group.ts so the 'use client' diagnostics
// component can import it without pulling this module's `node:fs` into the client
// bundle. Server call-sites + unit tests that import from here keep working.
export { groupFixRelevant } from './render-device-group';
