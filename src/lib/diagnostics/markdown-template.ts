...
import type { DiagnosticsPayload, NvidiaGpuSource } from './types';
import { groupFixRelevant } from './render-device-group';

// 46-02 (D2, SR-2): every NvidiaGpuSource maps to a distinct italic fallback
// line for the empty case. A `Record<NvidiaGpuSource,string>` (NOT a
// `<source>`-interpolated default) so a future 6th union member is a COMPILE
// error, not a silent generic string. `present`-but-zero reuses the no_gpu copy.
const NVIDIA_GPU_EMPTY_FALLBACK: Record<NvidiaGpuSource, string> = {
  present: '_No NVIDIA GPU detected._',
...
export function renderDiagnosticsMarkdown(payload: DiagnosticsPayload): string {
  ...
  // 49-03: the forced-IDR (closed-GOP) verdict. Only rows where something was
  // actually established are printed — a 'not-probed' line per encoder would be
  // noise on every single report. The 'unsupported' row carries the CONSEQUENCE,
  // not just the label: this host ships open GOPs, so the playback-artefact fix
  // does not apply to it, and that is the fact the next forum report needs.
  const idrRows = payload.encoders.outcome.filter((o) => o.forcedIdr !== 'not-probed');
  if (idrRows.length > 0) {
    parts.push('- closed-GOP (forced-IDR) support:');
    for (const o of idrRows) {
      parts.push(
        o.forcedIdr === 'unsupported'
          ? `  - ⚠️ \`${o.encoder}\`: runtime REJECTED the forced-IDR option — auto-degraded to open GOPs (the closed-GOP fix does NOT apply on this host)`
          : `  - \`${o.encoder}\`: runtime accepted the forced-IDR option`,
      );
    }
  }
  // 49-05: the QSV ratecontrol tier. Printed only where it MEANS something:
  //   - 'not-applicable' rows (libx265/nvenc/vaapi) never print — they have no tier.
  //   - an 'unresolved' row whose own outcome is 'missing' never prints either
  //     (AC-4b): that host has no QSV at all, and an alarming line about absent
  //     hardware would be new noise in exactly the artefact 49-05 exists to make
  //     analysable. The JSON field stays 'unresolved'; this is a print decision.
  // The 'unresolved' line that DOES print carries the CONSEQUENCE, not just the
  // label — getActiveQsvRateControl() falls back to 'icq-full' (detection.ts:1160),
  // so the encode still ships `-global_quality` and the stored crf_qsv value is
  // being read on the ICQ scale. "unknown" alone would not answer the question the
  // next forum report asks.
  const rcRows = payload.encoders.outcome.filter(
    (o) =>
      o.rateControl !== 'not-applicable' &&
      !(o.rateControl === 'unresolved' && o.outcome === 'missing'),
  );
  if (rcRows.length > 0) {
    parts.push('- QSV ratecontrol tier:');
    for (const o of rcRows) {
      if (o.rateControl === 'unresolved') {
        parts.push(
          `  - ⚠️ \`${o.encoder}\`: tier UNRESOLVED — the boot probe validated neither tier; the encode still falls back to \`-global_quality\` (ICQ), so the stored crf_qsv value is being read on the ICQ scale`,
        );
      } else if (o.rateControl === 'icq-full') {
        parts.push(
          `  - \`${o.encoder}\`: \`icq-full\` — emits \`-global_quality <crf> -low_power 0\` (ICQ scale)`,
        );
      } else {
        parts.push(`  - \`${o.encoder}\`: \`cqp\` — emits \`-q:v <crf>\` (constant-QP scale)`);
      }
    }
  }
  parts.push('');
  ...
  // 46-02 (D2): NVIDIA GPU model + driver — immediately after `## CPU` (both
  // hardware-capability surfaces). Lets a nvenc report be evidence-checked
  // against the exact card/gen instead of hedged.
  const nvidiaGpu = payload.nvidiaGpu;
  parts.push('## NVIDIA GPU');
  if (nvidiaGpu.gpus.length > 0) {
    for (const gpu of nvidiaGpu.gpus) {
      parts.push(`- ${gpu.name} — driver ${gpu.driverVersion}`);
    }
  } else {
    parts.push(NVIDIA_GPU_EMPTY_FALLBACK[nvidiaGpu.source]);
  }
  parts.push('');
  ...
  // 48-01 — append `## Scan Integrity` after `## Forced-polling shares` (new
  // TRAILING H2). Append-only: the section order above is NOT reordered.
  // Surfaces the walker's directory-skip counters, i.e. the evidence for the
  // "143 of 546 directories indexed" class of silent library loss. BOTH states
  // render explicitly — a missing section would be indistinguishable from an
  // older container build and therefore worthless as evidence.
  parts.push('## Scan Integrity');
  parts.push('');
  const lastScan = payload.scanIntegrity.lastScan;
  if (lastScan === null) {
    parts.push('_No scan has completed in this process yet._');
  } else {
    const anySkips =
      lastScan.dirsSkippedCycle > 0 ||
      lastScan.dirsSkippedUnreadable > 0 ||
      lastScan.dirsSkippedMaxDepth > 0;
    parts.push(
      `- outcome: \`${lastScan.outcome}\` · root: ${lastScan.rootPath === null ? '_multi-share_' : `\`${lastScan.rootPath}\``}`,
    );
    parts.push(`- started: ${lastScan.startedAtIso} · finished: ${lastScan.finishedAtIso}`);
    parts.push(
      `- directories visited: ${lastScan.dirsVisited} · skipped: cycle ${lastScan.dirsSkippedCycle}, unreadable ${lastScan.dirsSkippedUnreadable}, max-depth ${lastScan.dirsSkippedMaxDepth}, system-prefix ${lastScan.dirsSkippedSystemPrefix}`,
    );
    // 48-02 (audit-fix S5 / AC-18): system-prefix is rendered in the counters
    // line but deliberately NOT folded into `anySkips` above — it is an
    // INTENTIONAL prune, so it must not cancel the "no directories were
    // skipped" reassurance 48-01 shipped for UNINTENDED loss. 0 is always
    // printed; an absent field would be indistinguishable from an older build.
    // audit-added S6: max-depth counts the truncated ROOTS, not the directories
    // lost underneath them — spell that out so `3` is not read as "3 missing".
    parts.push(
      '- _(max-depth counts TRUNCATED ROOTS, not the directories below them — one truncated root can hide thousands)_',
    );
    // audit-added M1: a failed scan must never read as a clean one.
    if (lastScan.outcome === 'failed') {
      parts.push('- ⚠️ **the last scan FAILED — these numbers describe an aborted run**');
    }
    if (lastScan.sharesFailed > 0) {
      parts.push(`- ⚠️ **${lastScan.sharesFailed} share(s) threw during this scan**`);
    }
    if (!anySkips) {
      parts.push('- no directories were skipped');
    }
    if (lastScan.byShare.length > 0) {
      parts.push('');
      // 48-02 (audit-added M5): `System-prefix` column — header, separator AND
      // cell. AC-9 requires the PER-SHARE counter to be visible; a field that
      // reaches the payload but never renders is not evidence.
      parts.push(
        '| Share | Root | Failed | Visited | Cycle | Unreadable | Max-depth | System-prefix |',
      );
      parts.push('|---|---|---|---|---|---|---|---|');
      for (const s of lastScan.byShare) {
        parts.push(
          `| ${s.name} | \`${s.rootPath}\` | ${s.failed ? '⚠️ yes' : 'no'} | ${s.dirsVisited} | ${s.dirsSkippedCycle} | ${s.dirsSkippedUnreadable} | ${s.dirsSkippedMaxDepth} | ${s.dirsSkippedSystemPrefix} |`,
        );
      }
    }
    if (lastScan.cycleSamples.length > 0 || lastScan.unreadableSamples.length > 0) {
      parts.push('');
      // audit-added S5: same disclosure class as the blocklist path table above
      // (raw /mnt/user/<share>/… paths in a forum-paste surface) → same notice.
      parts.push(
        '<!-- Operator: paths below are verbatim. Redact mount/user prefixes before posting if sensitive. -->',
      );
      parts.push('');
      if (lastScan.cycleSamples.length > 0) {
        parts.push(`- skipped (cycle, first ${lastScan.cycleSamples.length}):`);
        for (const p of lastScan.cycleSamples) parts.push(`  - \`${p}\``);
      }
      if (lastScan.unreadableSamples.length > 0) {
        parts.push(`- skipped (unreadable, first ${lastScan.unreadableSamples.length}):`);
        for (const p of lastScan.unreadableSamples) parts.push(`  - \`${p}\``);
      }
    }
  }
  parts.push('');
