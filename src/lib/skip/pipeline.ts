...
export type PipelineDeps = {
  ...
  // 33-01 (optional, additive — mirrors the blocklistRepo?/patternsCache?/logger?
  // idiom): when sidecarMode==='central' Step 1 consults the central sidecar tree
  // (with beside fallback, D1=B) instead of beside-only. Callers resolve these from
  // settings and pass them; legacy/04-01 callers omit both → Step 1 defaults to a
  // beside read = byte-identical to pre-33-01 (AC-4). Pipeline stays pure-input
  // (no settingRepo coupling — D2=A).
...
export async function runSkipPipeline(
  ...
  // 33-01: central-mode reads the central-resolved sidecar first (beside fallback,
  // D1=B); absent/'beside'/'off' is byte-identical to pre-33-01 (readSidecar = beside).
  // audit-SR-1: the central-root default comes from the SHARED const, never a
  // re-typed literal — read-default MUST equal the 26-01 write-default (AC-7).
  const sidecar =
