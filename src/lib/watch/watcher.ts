...
function buildChokidarOpts(usePolling: boolean, intervalMs?: number): Record<string, unknown> {
  ...
  return {
    ignored: pruneActive ? [DOTFILE_RE, systemMatcher] : [DOTFILE_RE],
...
export async function startWatcher(deps: WatcherDeps): Promise<void> {
  ...
  // finds the cause here (info ≥ the `LOG_LEVEL ?? 'info'` instance gate, so it
  // reaches the ring-buffer and the copy-report).
