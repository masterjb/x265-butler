...
} from '../fs/system-paths';
import { recordWatchError, resetWatchErrorAggregator } from './watch-error-aggregator';
// 50-03: the ONE media-eligibility predicate, shared with the scan walker, the
...
interface WatcherState {
  ...
  epoch: number;
  // 50-03 (AC-20): per-share tally of add-events the extension gate discarded,
  // drained by the next flushBatch window for that share. A filtered event
  // produces NO `file` row, so without this it would leave no trace anywhere.
  ingestFilteredByShare: Map<number, { count: number; samples: string[] }>;
  // 50-03 (AC-20): 48-01 pattern — the LINES are capped per watcher start, the
  // COUNTER is not. A large first import must not flush the 1000-line ring.
  ingestFilteredLogLines: number;
  ingestFilteredTotal: number;
  // 50-03 (AC-19): shares already warned about an empty extensions_csv. One
  // warn per share and watcher start, never per event.
  emptyExtensionsWarned: Set<number>;
}
...
function freshState(): WatcherState {
  return {
    ...
    status: freshStatus(),
    epoch: 0,
    ingestFilteredByShare: new Map(),
    ingestFilteredLogLines: 0,
    ingestFilteredTotal: 0,
    emptyExtensionsWarned: new Set(),
  };
}
...
export function resetWatcherState(): void {
  ...
  // 48-02 (AC-12): the aggregation window is process-global, so it must die
  // with the watcher state — otherwise a torn-down watcher fires a summary for
  // a previous life and keeps an (unref'd, but still pending) timer around.
...
function resolvePollingForShare(share: ShareRow): number {
  ...
  state.status.pollingShares[share.name] = {
    ...
    // 42-03: measured fields stay null until the chokidar 'ready' event fires
    // (attachReadyDiag) — the snapshot is well-formed before ready (AC-2).
    actualWatchedPaths: null,
    actualPathMultiplier: null,
    actualStatsPerSec: null,
  };
  return res.ms;
}

// 42-03: chokidar's TRUE watched-path count from getWatched(), the ground truth
// the PATHS_PER_FILE=10 estimate only approximates. getWatched() returns
// Record<dir, string[]> (absolute dir key → immediate child basenames, files AND
// subdirs). The poller stat()s the set of (#dirs + #files) each tick; a subdir
// appears BOTH as its own key and as a child of its parent, so dedup children
// that are themselves keys to avoid double-counting. Returns null on the
// no-getWatched (test factory / older chokidar) or throw paths — never breaks the
// watcher for a diagnostics number. onUnmeasured fires ONCE per call (per 'ready',
// not per tick) so a silently-null measured surface (the 22-01→38-02 dark-surface
// bug this lineage already paid for) is impossible (audit-added S1).
function countWatchedPaths(
  inst: FSWatcher,
  onUnmeasured?: (reason: 'no-getWatched' | 'threw', err?: string) => void,
): number | null {
  if (typeof inst.getWatched !== 'function') {
    onUnmeasured?.('no-getWatched');
    return null;
  }
  try {
    const watched = inst.getWatched();
    const dirSet = new Set(Object.keys(watched));
    let fileCount = 0;
    for (const [dir, children] of Object.entries(watched)) {
      for (const c of children) {
        if (!dirSet.has(join(dir, c))) fileCount++;
      }
    }
    return dirSet.size + fileCount;
  } catch (e) {
    onUnmeasured?.('threw', e instanceof Error ? e.message : String(e));
    return null;
  }
}

// 42-03: on a forced-polling instance's 'ready' (initial scan complete), read
// chokidar's real watched-path count and write the measured fields into the
// pollingShares snapshot. Attached ONLY to forced-polling instances (the inotify
// mainInstance has no poll stat-storm). Every 'ready' emits EXACTLY one log line —
// watch_poll_actual_paths_measured (success) XOR watch_poll_actual_paths_unmeasured
// (warn) — so the measured surface is never silently null (audit-added S1).
function attachReadyDiag(inst: FSWatcher, share: ShareRow, deps: WatcherDeps): void {
  inst.on('ready', () => {
    const entry = state.status.pollingShares[share.name];
    if (!entry) return; // share torn down between wire + ready
    const actualWatchedPaths = countWatchedPaths(inst, (reason, err) => {
      deps.log.warn(
        { action: 'watch_poll_actual_paths_unmeasured', share: share.name, reason, err },
        'watcher poll diag: getWatched() unavailable/threw — actualWatchedPaths stays null',
      );
    });
    if (actualWatchedPaths === null) return;
    entry.actualWatchedPaths = actualWatchedPaths;
    // 42-03 audit-S2: divides by effectiveIntervalMs (= the resolved `interval`).
    // This is the realized stat-cadence ONLY while 42-02's `interval ==
    // binaryInterval` pin holds (buildChokidarOpts sets both equal). If a future
    // change unpins them, this single-rate division silently mis-reports a media
    // tree — the next editor of buildChokidarOpts must re-check this line.
    entry.actualStatsPerSec =
      entry.effectiveIntervalMs > 0
        ? Math.round((actualWatchedPaths / entry.effectiveIntervalMs) * 1000)
        : 0;
    entry.actualPathMultiplier =
      entry.watchedFileCount > 0
        ? Math.round((actualWatchedPaths / entry.watchedFileCount) * 10) / 10
        : null;
    deps.log.info(
      {
        action: 'watch_poll_actual_paths_measured',
        share: share.name,
        actualWatchedPaths,
        actualStatsPerSec: entry.actualStatsPerSec,
        actualPathMultiplier: entry.actualPathMultiplier,
        estimatedRealPaths: entry.realPaths,
        estimatedStatsPerSec: entry.computedStatsPerSec,
      },
      'watcher poll diag: measured chokidar watched-path count vs PATHS_PER_FILE=10 estimate',
    );
  });
}
...
// 48-02 (Bundle B, AC-7/AC-7b): the system-prefix ignore matcher.
//
// A FUNCTION over LIVE state, deliberately NOT a snapshot computed at
// construction time, because BOTH of the alternatives are broken:
//
//  (a) "omit the matcher when ANY watched root is under a prune prefix" — the
//      main inotify instance watches an ARRAY of share roots at once, so one
//      /run/media/usb1 share would disarm the prune for a CO-WATCHED '/' share
//      and re-open exactly the crash this plan closes.
//  (b) a per-path snapshot — `ignored` is fixed at instance construction, but
//      addShareToWatcher() calls `state.mainInstance.add(sharePath)` on the
//      ALREADY-BUILT instance and cannot rebuild `ignored` at all.
//
// So the exemption is per-PATH and read at CALL time: a path is ignored iff it
// is under a prune prefix AND it does not belong to a watched share whose OWN
// root is under a prune prefix. '/' is never under a prune prefix, so a
// '/'-rooted share can never buy the exemption.
function systemMatcher(p: string): boolean {
  return (
    isUnderPruneSystemPrefix(p) &&
    !state.shares.some(
      (s) => isUnderPruneSystemPrefix(s.path) && (p === s.path || p.startsWith(`${s.path}/`)),
    )
  );
}

// 48-02 (AC-16, audit M4): `followSymlinks: false` is NOT a free consistency
// fix — chokidar stops descending symlinked directories, so an install whose
// media tree is reached THROUGH a symlink loses auto-scan for those files
// SILENTLY. Behaviour changes on the watch path get a no-redeploy revert lever
// in this project (42-02 precedent), so this one gets its own — deliberately
// NOT governed by SCAN_PRUNE_SYSTEM_PATHS (one lever per behaviour change).
// Memoized + logged once per watcher start.
let followSymlinksMemo: { value: boolean; source: 'env' | 'default' } | null = null;

function resolveFollowSymlinks(): { value: boolean; source: 'env' | 'default' } {
  if (followSymlinksMemo !== null) return followSymlinksMemo;
  const raw = process.env.WATCH_FOLLOW_SYMLINKS;
  followSymlinksMemo = { value: raw === '1', source: raw === undefined ? 'default' : 'env' };
  return followSymlinksMemo;
}

// Test-only companion to the memo above — production code NEVER calls this.
export function __resetFollowSymlinksMemoForTests(): void {
  followSymlinksMemo = null;
}
...
function buildChokidarOpts(usePolling: boolean, intervalMs?: number): Record<string, unknown> {
  ...
  // chokidar v4 (4.0.3): `ignored: Matcher | Matcher[]` with
  // `Matcher = string | RegExp | MatchFunction | MatcherObject`
  // (node_modules/chokidar/index.d.ts:29+38-43) — the existing RegExp stays
  // legal alongside the function matcher.
  const pruneActive = isSystemPruneEnabled();
  return {
    // 50-03 (E2b/AC-2): SIDECAR_IGNORE_RE is UNCONDITIONAL — it is deliberately
    // NOT behind WATCH_INGEST_FILTER_DISABLED. This app writes those files
    // itself, so a lever that restores the self-feeding is not a safety net.
    ignored: pruneActive
      ? [DOTFILE_RE, SIDECAR_IGNORE_RE, systemMatcher]
      : [DOTFILE_RE, SIDECAR_IGNORE_RE],
    followSymlinks: resolveFollowSymlinks().value,
    persistent: true,
    ignoreInitial: true,
    atomic: true,
...
export async function startWatcher(deps: WatcherDeps): Promise<void> {
  ...
  // 48-02 (AC-16): resolved value + source, ONCE per watcher start — an
  // operator whose symlinked library stopped being auto-scanned after 2.45.0
  // finds the cause here (info ≥ `telemetry` (25), so it reaches the
  // ring-buffer and the copy-report; `debug` (20) would not).
  const followSymlinks = resolveFollowSymlinks();
  deps.log.info(
    {
      action: 'watch_follow_symlinks_resolved',
      followSymlinks: followSymlinks.value,
      source: followSymlinks.source,
    },
    'watcher: followSymlinks resolved (WATCH_FOLLOW_SYMLINKS=1 restores pre-48-02 behaviour)',
  );

  // 50-03 (AC-12b): resolved value + source, ONCE per watcher start — never on
  // the event path. Same placement and reasoning as the line above: `info` is
  // >= `telemetry` (25), so it reaches the ring-buffer and the copy-report.
  // The resolver itself is pure and takes no logger; the line lives here.
  const ingestFilter = resolveIngestFilterEnabled();
  deps.log.info(
    {
      action: 'watch_ingest_filter_resolved',
      enabled: ingestFilter.enabled,
      source: ingestFilter.source,
    },
    'watcher: ingest media-eligibility filter resolved (WATCH_INGEST_FILTER_DISABLED=1 restores the pre-50-03 ingest; the sidecar ignore stays on regardless)',
  );

  // 48-02 (AC-17, audit S3): Guard A only blocks NEW writes — it does NOT
  // rewrite rows that were stored before this plan shipped (a silent migration
  // of an operator's share row would be a one-way door). This WARN is the ONLY
  // channel that tells the already-broken operator WHY their library is
  // half-watched. Once per START, never per event.
  for (const share of state.shares) {
    if (isForbiddenSharePath(share.path)) {
      deps.log.warn(
        {
          action: 'share_path_forbidden_prefix_stored',
          share: share.name,
          path: share.path,
        },
        'watcher: stored share path points at a system directory — edit it in Settings → Paths (runtime prune limits the damage but this is not a valid media root)',
      );
    }
    // 48-02 (AC-6 analogue on the watch path): a share whose OWN root is under
    // a prune prefix is EXEMPT from the prune (else it would vanish entirely).
    // Same action name as the walker's so one grep finds both surfaces.
    if (isSystemPruneEnabled() && isUnderPruneSystemPrefix(share.path)) {
      deps.log.warn(
        {
          action: 'scan_root_under_system_prefix',
          surface: 'watcher',
          share: share.name,
          rootPath: share.path,
        },
        'watcher: share root lies under a system prefix — system-prefix prune exempted for this share only',
      );
    }
  }
  ...
  for (const share of forcedPollingShares) {
    ...
    wireInstanceHandlers(inst, deps);
    attachReadyDiag(inst, share, deps);
...
// 50-03 (AC-20): 48-01 shape — cap the LINES, never the COUNTER.
const INGEST_FILTERED_LOG_CAP = 20;
const INGEST_FILTERED_SAMPLE_CAP = 5;

function recordIngestFiltered(shareId: number, absPath: string): void {
  state.ingestFilteredTotal++;
  let entry = state.ingestFilteredByShare.get(shareId);
  if (!entry) {
    entry = { count: 0, samples: [] };
    state.ingestFilteredByShare.set(shareId, entry);
  }
  entry.count++;
  if (entry.samples.length < INGEST_FILTERED_SAMPLE_CAP) entry.samples.push(absPath);
}

// 50-03: the flush timer must be armed on BOTH paths — buffer insert AND
// discard. A window that contained ONLY filtered events creates no buffer at
// all, and that is precisely the window whose evidence line matters most.
function scheduleFlush(shareId: number, deps: WatcherDeps): void {
  if (state.flushTimers.has(shareId)) return;
  const window = readSettingInt('autoScan.batchWindow', DEFAULTS.batchWindowMs);
  const timer = setTimeout(() => {
    state.flushTimers.delete(shareId);
    void flushBatch(shareId, deps);
  }, window);
  state.flushTimers.set(shareId, timer);
}
...
function onAddEvent(absPath: string, deps: WatcherDeps): void {
  ...
  // 50-03 (E3/AC-4): the extension gate sits BEFORE the batch buffer and thus
  // before flushBatch's 60/min admission — which today admits BEFORE anything
  // is filtered. A burst of sidecars / .nfo / .srt / .jpg could otherwise push
  // real `add` events out over `auto_scan_rate_cap_drop`. The check is
  // synchronous and free (`path.extname`); the SIZE check needs a stat and
  // therefore stays in the ingest, where a stat happens anyway.
  //
  // No log per event on purpose (a media folder would spam forever) — the
  // discards are tallied here and written as ONE aggregate line per flush
  // window in flushBatch (AC-20).
  if (ingestFilterEnabled()) {
    // Resolved per event, NOT cached in `state`: a share edit would otherwise
    // leave the cache silently stale.
    const allowed = resolveAllowedExtensions(share.extensions_csv);
    if (allowed.fellBack && !state.emptyExtensionsWarned.has(share.id)) {
      state.emptyExtensionsWarned.add(share.id);
      deps.log.warn(
        {
          action: 'watch_share_extensions_empty',
          shareId: share.id,
          share: share.name,
          fallback: [...allowed.set].sort(),
        },
        'watcher: share has an empty extensions_csv — falling back to the default media extensions (the share keeps its watch ingest; fix it in Settings → Shares)',
      );
    }
    if (!hasAllowedExtension(absPath, allowed.set)) {
      recordIngestFiltered(share.id, absPath);
      scheduleFlush(share.id, deps);
      return;
    }
  }
...
// 50-03 (AC-20): ONE aggregate line per flush window that discarded anything.
// Runs at the very TOP of flushBatch — before the empty-buffer early return —
// because a window holding only filtered events never allocates a buffer.
function emitIngestFilteredBatch(shareId: number, deps: WatcherDeps): void {
  const entry = state.ingestFilteredByShare.get(shareId);
  if (!entry) return;
  state.ingestFilteredByShare.delete(shareId); // close the window cleanly
  if (entry.count === 0) return;
  if (state.ingestFilteredLogLines >= INGEST_FILTERED_LOG_CAP) return; // count on, log off
  state.ingestFilteredLogLines++;
  deps.log.info(
    {
      action: 'auto_scan_ingest_filtered_batch',
      shareId,
      filtered: entry.count,
      samples: entry.samples,
    },
    'auto-scan: add-events discarded by the media-eligibility filter (extension not in the share allowlist) — add the extension in Settings → Shares if this is your media',
  );
}

async function flushBatch(shareId: number, deps: WatcherDeps): Promise<void> {
  emitIngestFilteredBatch(shareId, deps);
...
function onWatcherError(err: Error, deps: WatcherDeps): void {
  ...
  // 48-02 (Bundle C, AC-11): the ENOSPC branch returned ABOVE — it is a
  // distinct, rare, actionable condition with its own retry ladder and must
  // never be swallowed by an aggregation window.
  //
  // AC-10: the first error of a window is logged IMMEDIATELY with the UNCHANGED
  // line shape (existing greps + tests survive); the rest are counted and
  // collapsed into one summary at window close.
  if (recordWatchError({ message: err.message, code }, deps.log) === 'emit') {
    deps.log.warn(
...
export async function stopWatcher(): Promise<void> {
  ...
  resetWatchErrorAggregator(); // 48-02 (AC-12)
