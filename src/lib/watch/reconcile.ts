...
export interface ReconcileDeps {
  ...
  runScan: (opts: ScanOptions, repo: FileRepo, log?: AppLogger) => Promise<ScanResult>;
...
export async function runBootReconcile(deps: ReconcileDeps): Promise<ReconcileResult> {
  ...
  // 28-06 R10: bound the orphan re-enqueue so a pathological crash-backlog cannot
  ...
  // audit-added SR-1: malformed values (NaN/0/neg/empty) are SILENTLY coerced to
  // 1000 with NO log.warn (consistent with ingestConcurrency() — documented in
  // CLAUDE.md, 28-06). Read per-call (reconcile is 6h-periodic — cheap).
  const cap = ((): number => {
    const n = Number(process.env.RECONCILE_ORPHAN_CAP);
    return Number.isFinite(n) && n >= 1 ? Math.floor(n) : 1000; // NaN/0/neg/empty → 1000
  })();
  const totalOrphans = orphanIds.length;
  // audit-added SR-4: "first C" is SQLite return-order (findOrphanFileIds has NO
