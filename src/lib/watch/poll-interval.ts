...
// memoized + warn-once, resolved value logged at info (≥ `telemetry` (25)
// reaches the ring-buffer; `debug` (20) does not — the 22-01→38-02
// dark-surface bug; see the tier table in src/lib/logger.ts).
//
// 42-02 (strace-confirmed 2026-06-25 post-ship): chokidar's forced-polling path
// has TWO stat knobs — `interval` (non-binary paths) and `binaryInterval`
// (default 300 ms, for binary-extension paths: .mkv/.mp4/.jpg/.png/...). This
// resolver computes ONE value; watcher.ts:buildChokidarOpts pins BOTH knobs to it.
// 42-01 scaled only `interval`, so a media library — where ~every watched path IS
// a binary extension — kept polling at the 300 ms default even with
// WATCH_POLL_INTERVAL_MS set (operator strace: env=60000 yet ~59 k statx/s). NOTE
// 42-01 AC-9 named the "second stat source" as `awaitWriteFinish`; that was a
// mis-identification — the real second source is `binaryInterval`. awaitWriteFinish
// (~1 s) stays fixed: it polls only the bounded in-flight write-stabilizing set,
// NOT the whole tree, so a small accepted residual — not the storm.
