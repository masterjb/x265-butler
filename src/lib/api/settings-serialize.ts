...
export type EditableSettings = {
  ...
  // default-seed) → fresh/upgraded install byte-identical pre-43.
  force_10bit: 'true' | 'false';
  // 43-03: preserve source VUI color tags on output (surfaces 43-03 backend).
...
export type FormValues = {
  ...
  auto_crop: boolean;
  crop_override: string;
  // 43-01: form-side force-10bit toggle (bool). Serialized → 'true'/'false'.
  force_10bit: boolean;
  // 43-03: form-side color-passthrough toggle (bool). Serialized → 'true'/'false'.
...
export function serializeForApi(values: Partial<FormValues>): Partial<EditableSettings> {
  ...
  // 43-01: force_10bit bool → string (mirror auto_crop).
  if (values.force_10bit !== undefined) out.force_10bit = values.force_10bit ? 'true' : 'false';
  // 43-03: color_passthrough bool → string (mirror force_10bit).
