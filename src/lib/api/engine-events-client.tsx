...
export interface ActiveJob {
  ...
  /** 43-02: ffmpeg speed multiplier, drives the per-job ETA. null until first progress / non-derivable. */
  speed: number | null;
  encoder: string | null;
}
...
export interface QueueCounts {
  /**
   * queued + encoding — the unchanged wire field. 48-03: this is NOT what the UI
   * labels "encoding"; render `encodingJobs` for that.
   */
  activeJobs: number;
  pendingJobs: number;
  /** 48-03: encoding-only. `activeJobs` stays queued+encoding (wire compatibility). */
  encodingJobs: number;
  /**
   * 48-03 (audit M4): false until the FIRST live value has arrived (SSE
   * queue.updated or a rebootstrap response). Consumers with a bootstrap/SSR seed
   * must gate on THIS, not on `encodingJobs > 0` — after the 48-03 switch,
   * `encodingJobs === 0` is a perfectly normal live state (between two encodes,
   * throttled dispatch, paused queue with 995 waiting), so a value test would
   * bring the stale mount-time number back every single time.
   */
...
interface EngineEventsStore {
  // 36-02: multi-slot active-job map keyed by jobId. Replaces the single
  // `activeJob: ActiveJob | null` slot (02-04) which broke at encode_parallelism
  // >= 2 — a second slot's job.started OVERWROTE the first, and a completing slot
  // cleared the bar for a still-running sibling. A keyed map renders one bar per
  // concurrent encode (mirrors the existing `pass2: Record<number, …>` precedent).
...
const INITIAL_STORE: EngineEventsStore = {
  ...
  recentJobs: [],
  paused: false,
  counts: { activeJobs: 0, pendingJobs: 0, encodingJobs: 0, hasLiveCounts: false },
  disconnected: false,
...
interface SelectorCache {
  // 36-02: three fields tracking the ONE activeJobs map ref. `activeJobsRef` is
  // the last-seen map identity (the ref-guard both hooks share); `activeJobsSorted`
  // is the derived jobId-ascending array (reference-stable until the map changes);
  // `activeJobSingle` is the derived lowest-jobId entry for the back-compat
  // useActiveJob() selector. ATOMICITY: all three are re-stamped together in a
  // single immutable spread whenever the map ref changes — never update one
  // without re-stamping `activeJobsRef`, or useSyncExternalStore tears.
  activeJobsRef: Record<number, ActiveJob>;
  activeJobsSorted: ActiveJob[];
  activeJobSingle: ActiveJob | null;
  recentJobs: JobRow[];
  paused: boolean;
  counts: QueueCounts;
  disconnected: boolean;
...
export function EngineEventsProvider({
  ...
  const selectorCache = useRef<SelectorCache>({
    activeJobsRef: store.getState().activeJobs,
    activeJobsSorted: Object.values(store.getState().activeJobs).sort((a, b) => a.jobId - b.jobId),
    activeJobSingle:
      Object.values(store.getState().activeJobs).sort((a, b) => a.jobId - b.jobId)[0] ?? null,
    ...
    try {
      ...
      store.setState((prev) => ({
        ...
        counts: {
          ...
          // 48-03 (audit S6): /api/queue/status has carried encodingJobs since
          // 05-09, so this fallback is NOT about a partial 5xx (`if (!res.ok)
          // return` catches that above) — it covers SKEW: a new client bundle in
          // the browser talking to a still-old server process (rolling container
          // restart, reverse proxy across two instances). Do not delete it as
          // unreachable.
          encodingJobs: data.encodingJobs ?? 0,
          hasLiveCounts: true,
        },
        disconnected: false,
      }));
...
function handleEngineEvent(event: EngineEvent, store: StoreInstance): void {
  store.setState((prev) => {
    switch (event.type) {
      case 'job.started':
        // 36-02: upsert the keyed slot (was: overwrite the single slot, which
        // wiped a concurrent sibling at parallelism >= 2).
        return {
          ...prev,
          activeJobs: {
            ...prev.activeJobs,
      ...
      case 'job.progress': {
        // 36-02: UPSERT with create-on-unknown (AC-6). Was: early-return when
        // the single slot's jobId didn't match — which DROPPED a sibling's
        // progress at parallelism >= 2. Now each jobId owns its own slot, and an
        // unknown jobId CREATES the entry (encoder=null until a later job.started
        // — or never, on a mid-join reconnect) so a client connecting mid-encode
        // reconstructs every parallel bar from the server's per-active-job
        // progress replay (route.ts:90). Mirrors the bench `combo_progress`
        // null-tolerance pattern.
        //
        // KNOWN-LIMITATION (audit S-zombie): create-on-unknown means a
        // job.progress arriving AFTER its own terminal event would RE-CREATE a
        // slot that then receives no further terminal → a phantom bar until
        // reload (the mirror of the bug being fixed). Accepted because (a) a
        // single SSE connection delivers a job's events in order, so progress
        // never follows that job's terminal within one stream, and (b) the
        // server replay on reconnect is ACTIVE-jobs-only — a completed job is
        // never replayed. Do NOT "harmonize" this with the bench
        // `pass2_progress` guard (which early-returns on unknown): the
        // create-on-unknown divergence is deliberate and is what enables AC-6.
        const existing = prev.activeJobs[event.jobId];
        return {
          ...prev,
          activeJobs: {
            ...prev.activeJobs,
      ...
      case 'job.failed':
      case 'job.cancelled': {
        // 36-02: key-targeted delete. The 05-13 UAT fix (clear only on jobId
        // match — skipping a QUEUED job emits job.cancelled for a non-active
        // jobId, which must NOT touch the encoding job) is now satisfied
        // STRUCTURALLY: deleting only the matching key leaves every sibling slot
        // intact, so a completing job removes its OWN bar and the others keep
        // updating (THE bug fix, AC-2). A no-op delete on an unknown jobId
        // returns the prev ref unchanged (no spurious re-render).
        if (!prev.activeJobs[event.jobId]) return prev;
        const next = { ...prev.activeJobs };
        delete next[event.jobId];
        return { ...prev, activeJobs: next };
      }
      case 'queue.updated':
        return {
          ...prev,
          counts: {
            ...
            // 48-03: `?? 0` is deliberately redundant against the required wire
            // type from Task 1 — it covers bundle skew, where an old client bundle
            // still running after a container update reads a frame without the
            // field. Do not remove it as dead code.
            encodingJobs: event.encodingJobs ?? 0,
            // audit M4: a live value has arrived — from here on the bootstrap/SSR
            // seed must never be shown again, not even when the value is 0.
            hasLiveCounts: true,
          },
          paused: event.paused,
        };
...
// 36-02: shared recompute for the activeJobs-map-derived cache fields. Called by
// BOTH useActiveJobs and useActiveJob; re-stamps all three map-tracking fields
// together (atomicity invariant — see SelectorCache comment) only when the map
// ref actually changes, so getSnapshot returns reference-stable values across
// no-change renders (no useSyncExternalStore tear / loop).
function syncActiveJobsCache(
  s: EngineEventsStore,
  cache: React.MutableRefObject<SelectorCache>,
): void {
  if (s.activeJobs !== cache.current.activeJobsRef) {
    const sorted = Object.values(s.activeJobs).sort((a, b) => a.jobId - b.jobId);
    cache.current = {
      ...cache.current,
      activeJobsRef: s.activeJobs,
      activeJobsSorted: sorted,
      activeJobSingle: sorted[0] ?? null,
    };
  }
}

// 36-02: Narrow hook — reference-stable jobId-ascending array of all active
// (encoding) jobs. Re-renders only when the activeJobs map ref changes.
export function useActiveJobs(): ActiveJob[] {
  ...
  const getSnapshot = useCallback(() => {
    syncActiveJobsCache(store.getState(), selectorCache);
    return selectorCache.current.activeJobsSorted;
  }, [store, selectorCache]);

  return useSyncExternalStore(subscribe, getSnapshot, getSnapshot);
}

// Narrow hook: lowest-jobId active job (or null). KEPT for back-compat (32-xx
// dashboard + any single-summary consumer). 36-02: now DERIVED from the
// activeJobs map (was the single slot). Shares the one cache with useActiveJobs
// so the two stay mutually consistent (single-job map → useActiveJob ===
// useActiveJobs[0]).
export function useActiveJob(): ActiveJob | null {
  ...
  const getSnapshot = useCallback(() => {
    syncActiveJobsCache(store.getState(), selectorCache);
    return selectorCache.current.activeJobSingle;
  }, [store, selectorCache]);
