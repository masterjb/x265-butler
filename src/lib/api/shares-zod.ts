...
import { isForbiddenSharePath } from '@/src/lib/fs/system-paths';
...
// 48-02 (Bundle A): EXPORTED — `app/api/onboarding/complete/route.ts` consumes
// this exact object for its `scan_root` field. The wizard route owns a SECOND
// schema and calls `shareRepo().create()` directly, so Guard A does not reach it
// otherwise; reusing the object (instead of copying the refines) is what makes
// the two doors provably identical (AC-4b).
export const pathSchema = z
  ...
  // 48-02: MUST sit AFTER the double-slash-collapse transform — otherwise
  // '//sys' slips past the prefix test. zod 4 (^4.3.6 installed, NOT v3): a
  // `.refine()` after `.transform()` sees the TRANSFORMED value and the issue
  // still carries `path: ['path']` inside the object schema AND through
  // `.optional()`, so `fieldErrorsFromZod` maps it to `fieldErrors.path`
  // (AC-1/AC-2). In-file precedent: `extensionsCsvSchema` below.
  .refine((p) => !isForbiddenSharePath(p), { message: 'path_forbidden_prefix' });
