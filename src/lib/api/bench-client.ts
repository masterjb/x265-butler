...
// 47-01: name + return shape preserved deliberately — both UI callsites
// (app/[locale]/bench/bench-client.tsx:350, .../runs/[id]/run-detail-client.tsx:178)
// and their component-test mocks consume THIS wrapper, so only the transport moved
// (DELETE /api/bench/{id} → POST /api/bench/{id}/cancel). No caller edits required.
export async function cancelBenchRun(
  runId: number,
): Promise<{ cancelled: true } | { error: string }> {
  const res = await fetch(`/api/bench/${runId}/cancel`, { method: 'POST' });
  const json = (await res.json()) as { cancelled?: true; error?: string };
  if (!res.ok) return { error: json.error ?? 'unknown_error' };
  return { cancelled: true };
}

// 47-01: purge (row-delete of the bench_run + CASCADE of its bench_combo rows).
// NOT cancel — an active run answers 409. Consumed by the 47-02 history-table UI.
export async function purgeBenchRun(
  runId: number,
): Promise<{ deleted: true; combosDeleted: number } | { error: string }> {
  ...
  if (!res.ok) return { error: json.error ?? 'unknown_error' };
  return { deleted: true, combosDeleted: json.combosDeleted ?? 0 };
}

// 47-01: bulk purge. 200-always partial-success envelope (library parity) — a non-ok
// response is an envelope-level failure (415/400/500), NOT a per-id rejection; those
// arrive in `failed`. Content-Type is mandatory: the route's CSRF guard 415s without it.
export async function bulkDeleteBenchRuns(
  ids: number[],
): Promise<
  { successCount: number; failed: Array<{ id: number; reason: string }> } | { error: string }
> {
  const res = await fetch('/api/bench/bulk-delete', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  if (!res.ok) return { error: json.error ?? 'unknown_error' };
  return { successCount: json.successCount ?? 0, failed: json.failed ?? [] };
}
